...
import com.mediatek.phone.calloption.PhoneCallOptionHandler;
...
import com.mediatek.phone.SmartBookUtils;
...
public class OutgoingCallBroadcaster extends Activity
    ...
    */
    ...
    public class OutgoingCallReceiver extends BroadcastReceiver {
        ...
        public boolean doReceive(Context context, Intent intent) {
            ...
            if (number == null) {
                ...
                return false;
            /* M: performance tracing @{
             * Move the code into else.
             * Original Code:
    ...
    private void startSipCallOptionHandler(Context context, Intent intent,
        ...
        /* M: using CallOptionHandler to handle newIntent directly*/
        PhoneGlobals.getInstance().callController.placeCall(newIntent);
        PhoneLog.i(TAG, "gemini NOT handled, so do nothing, please check MO process!!!");
        /*
    ...
    protected void onCreate(Bundle icicle) {
        ...
        /// M: For SmartBook @{
        SmartBookUtils.setActivityOrientation(this);
        /// @}
    ...
    private void processIntent(Intent intent) {
        ...
        /* M: Performance @{
         * isExactEmergencyNumber is no used;
        ...
        final boolean isPotentialEmergencyNumber = PhoneUtils.isEmergencyNumber(number);
        ...
        if (Intent.ACTION_CALL.equals(action)) {
            if (isPotentialEmergencyNumber) {
                ...
                try {
                    ...
                    PhoneLog.e(TAG, "Activity for DialtactsActivity isn't found.");
                }
                finish();
                return;
            }
        ...
        if (TextUtils.isEmpty(number)) {
            if (intent.getBooleanExtra(EXTRA_SEND_EMPTY_FLASH, false)) {
                ...
                /* M: VoiceMail GEMINI @{
                 * when dialing a VoiceMail number, the data of intent
                 * is 'voicemail:', do not need to dial out@{ */
                callNow = !isVoiceMailNumber;
                /* }@ */
            }
        }
        ...
        if (!callNow) {
            ...
            // finish ourself directly instead of sending an intent.
            finish();
        }
        /* }@ */
    }
    ...
    protected void onStart() {
        super.onStart();
        /// M: For ALPS00921390. @{
        // After lock->unlock screen, all related dialogs(includes sim status
        // indicator progress dialog) have been dismissed at the point of activity
        // onStop(), so when activity restart, we should check whether need to
        // re-show sim status indicator progress dialog .
        if (mCallOptionHandler != null) {
            mCallOptionHandler.updateSimStatusIndicatorProgressDialog();
        }
        /// @}
        PhoneLog.d(TAG, "onStart of OutgoingCallBroadcaster!");
    }

    // As original design, the lifetime of OutgoingCallBroadcaster is so short,
    // but we add too many handle in its context, so this maybe resume with new
    // intent, in this case we do some special handle, make sure some task must be
    // executed(for example the emergency call, see detail of ALPS00298859).
    protected void onNewIntent(Intent intent) {
        PhoneLog.d(TAG, "onNewIntent: intent = " + intent);
        if ((Intent.ACTION_CALL_EMERGENCY.equals(intent.getAction())
                || intent.getBooleanExtra(Constants.EXTRA_VT_MAKE_VOICE_RECALL, false))
                && PhoneGlobals.getInstance().mCM.getState() == PhoneConstants.State.IDLE) {
            PhoneLog.d(TAG,
                    "onNewIntent: ACTION_CALL_EMERGENCY and phone idle, finish and restart!");
            finish();
    ...
    protected void onResume() {
        super.onResume();
        PhoneLog.d(TAG, "onResume of OutgoingCallBroadcaster!");
    }
    ...
    /// M: CallOptionHandler @{
    private PhoneCallOptionHandler mCallOptionHandler;
    ...
    private void doCallOptionHandle(Intent intent) {
        ...
        mCallOptionHandler = new PhoneCallOptionHandler(new PhoneCallOptionHandlerFactory());
        mCallOptionHandler.doCallOptionHandle(intent, this, this);
    }
}
