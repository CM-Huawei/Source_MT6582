 ...
 *
 * Explanation for PhoneGlobals's Call states and why it is required:
 * IDLE - When no call is going on. This is just required as default state to reset the PhoneGlobals
 ...
 *        in a CONF_CALL state and user rejects a CW, which basically tells the PhoneGlobals that the
 *        Call is back to a single call, the FW's state still would remain ACTIVE or OFFHOOK and
 *        isGeneric would still be true. At this condition PhoneGlobals does need to enable the
 ...
 *        items accordingly. FW currently does not differentiate this condition hence PhoneGlobals
