...
interface ISmartcardService {
    ...
    byte[] getSelectResponse(long hChannel, out SmartcardError error);    
     ...
     /**
     * Query access control policy of NFC when HCI_EVT_TRANSACTION
     */
    boolean[] isNFCEventAllowed(in String reader, 
    							in byte[] aid,
    							in String[] packageNames, 
    							ISmartcardServiceCallback callback,
    							out SmartcardError error);
    							
    int nfcSmartCardSrvTest(int test);
}
