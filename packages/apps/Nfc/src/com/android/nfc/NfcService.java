...
public class NfcService implements DeviceHostListener {
    ...
    public NfcService(Application nfcApplication) {
        ...
        Log.i(TAG, "isNfcProvisioningEnabled = " + isNfcProvisioningEnabled);
        ...
        Log.i(TAG, "mInProvisionMode = " + mInProvisionMode);
        ...
        /// M: {
        mIsHceCapable = false; /// pm.hasSystemFeature(PackageManager.FEATURE_NFC_HOST_CARD_EMULATION);
        /// }
        if (mIsHceCapable) {
    ...
    int doOpenSecureElementConnection() {
        mEeWakeLock.acquire();
        /// M : @ {        
        Log.i(TAG, "doOpenSecureElementConnection");
        int result = 0;
        try {            
            mDeviceHost.disableDiscovery();
            
            result = mSecureElement.doOpenSecureElementConnection();
    ...
    class EnableDisableTask extends AsyncTask<Integer, Void, Void> {
        ...
        boolean enableInternal() {
			...
			/// M: to make it looks faster, we update the state earlier @ {
            ...
            if (mIsHceCapable) {
				...
				mScreenState = checkScreenState();/// force the screen state to update
            }
			/// }
        ...
        boolean disableInternal() {
            ...
            WatchDogThread watchDog = new WatchDogThread("disableInternal", mDeviceHost.getTimeout(6605));
            watchDog.start();
            ...
            synchronized (NfcService.this) {
                ...
                result = mDeviceHost.deinitializeEx(mDisableCardMode);
            ...
            try {
                ...
                try {
                    ...
                    try {
                        /// M: @ {
                        /*
                        TODO: Before open connection, the eSE needs to be selected.
                        
                              But the vendor thinks the wipe should not be supported.
                              Therefore, MTK make the wipe command is null and doesn't
                              add the eSE selection.
                        */
                        /// }
    ...
    void applyRouting(boolean force) {
        ...
        synchronized (this) {
            ...
            WatchDogThread watchDog = new WatchDogThread("applyRouting", mDeviceHost.getTimeout(6605));
            if (mInProvisionMode) {
                mInProvisionMode = Settings.Secure.getInt(mContentResolver,
                        Settings.Global.DEVICE_PROVISIONED, 0) == 0;
                if (!mInProvisionMode) {
                ...
                if (mDeviceHost.enablePN544Quirks() && mScreenState == SCREEN_STATE_OFF) {
                    ...
                    if (POLLING_MODE > SCREEN_STATE_OFF) {
                        if (force || mNfcPollingEnabled) {
                            ...
                            mNfcPollingEnabled = false;
                            mDeviceHost.disableDiscoveryEx();
                            maybeDisconnectTarget();
                        }
                    }
    ...
    public LlcpServerSocket createLlcpServerSocket(int sap, String sn, int miu, int rw,
		...
		try {
			serverSocket = mDeviceHost.createLlcpServerSocket(sap, sn, miu, rw, linearBufferLength);
		} catch (LlcpException e) {
			Log.d(TAG, "createLlcpServerSocket fail, wait for a 1000 ms to avoid busy polling");
			try {
				Thread.sleep(1000);
			} catch (Exception e1) {}
			throw e;
    ...
    final class NfcServiceHandler extends Handler {
        ...
        private void sendSeBroadcast(Intent intent) {
            ...
            synchronized(this) {
				...
				if (NfcRuntimeOptions.isGsmaEvtBroadcast()) {
                    Log.d(TAG, "NfcRuntimeOptions.isGsmaEvtBroadcast(): True");
                    byte[] aid = null;
                    boolean broadcastResult;
                    aid = intent.getByteArrayExtra(EXTRA_AID);
                    if(null != aid){
                        broadcastResult = EvtTransactionHandle.getInstance().processEvt(aid);
                        Log.d(TAG, "GSMA NFC Handset intent broadcastResult:"+broadcastResult);
                    }
                }
                else
                    Log.d(TAG, "NfcRuntimeOptions.isGsmaEvtBroadcast(): False");

				if (NfcRuntimeOptions.isEvtBroadcastAcBypass()) {
					Log.d(TAG, "bypass EVT broadcast AC, send to all");
					try {
						List<ResolveInfo> resolveInfos = mContext.getPackageManager().queryBroadcastReceivers(intent, 0);
						for (ResolveInfo r : resolveInfos) {
							Log.d(TAG, "send broadcast to " + r.activityInfo.packageName);
							intent.setPackage(r.activityInfo.packageName);
							mContext.sendBroadcast(intent);
						}
					} catch (Exception e) {
						Log.d(TAG, "exception during dispatching EVT");
						e.printStackTrace();
					} finally {
						return;
					}
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(
                ...
                if (isAirplaneModeOn) {
					/// M: @ {
                    new EnableDisableTask(NfcRuntimeOptions.isNoCardEmuInFlyMode() ? true : false).execute(TASK_DISABLE);
					/// }
	...
	private void initAddon(HandoverManager handoverManager) {
		MtkHandoverManager.createSingleton(mContext,handoverManager,mP2pLinkManager);
        ...
        //EvtTransactionHandle.createSingleton(mContext);
        ...
        filter.addAction("com.mediatek.nfc.IPO_DELAY_DONE");
	...
	private final BroadcastReceiver mAddonBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
			...
			if (action.equals("android.intent.action.ACTION_BOOT_IPO")) {
				...
				if (mPrefs.getBoolean(PREF_NFC_ON, NFC_ON_DEFAULT)) {
					Log.d(TAG, "start IpoBootDelayTimer");
                    new IpoBootDelayTimer(context).start();
				}
            } else if (action.equals("com.mediatek.nfc.IPO_DELAY_DONE")) {
                Log.d(TAG, "should turn on NFC");
				MtkNfcAddonSequence.getInstance().applyIpoSequence();
				sIsWfdOngoing = false;
				new EnableDisableTask().execute(TASK_ENABLE);
                Log.d(TAG, "sendBroadcast android.intent.action.AFTER_BOOT_IPO.NFC_BindHandoverService");
                Intent bindHandoverServiceIntent = new Intent("android.intent.action.AFTER_BOOT_IPO.NFC_BindHandoverService");
                context.sendBroadcast(bindHandoverServiceIntent);
                    
                    Log.d(TAG, "sendBroadcast android.intent.action.AFTER_BOOT_IPO.NFC_BindSmartCardService");
                    Intent bindSmartCardServiceIntent = new Intent("android.intent.action.AFTER_BOOT_IPO.NFC_BindSmartCardService");
                    context.sendBroadcast(bindSmartCardServiceIntent);
            ...
            }else if(action.equals("com.mediatek.nfc.CreateEvtTransactionHandle")){
                Log.d(TAG, "Rec. CreateEvtTransactionHandle");
    ...
    static class IpoBootDelayTimer implements Runnable {
        private Context mContext;
        public IpoBootDelayTimer(Context context) {
            mContext = context;
        }
        ...
        public void start() {
            try {
                ...
                e.printStackTrace();
                mContext.sendBroadcast(new Intent("com.mediatek.nfc.IPO_DELAY_DONE"));             
            }
        }
