...
public class BluetoothHeadsetHandover implements BluetoothProfile.ServiceListener {
	...
	/// M: @ {
	static final int STATE_WAITING_FOR_UUID = 8;
	/// }
	...
	/// M: @ {
	static final int MSG_WAIT4UUID_TIMEOUT = 3;
	/// }
    ...
    void nextStepConnect() {
        switch (mState) {
			...
			/// M: @ {
                // fall-through
            case STATE_BONDING:
				if (!isUuidReady(mDevice)) {
					mState = STATE_WAITING_FOR_UUID;
					mHandler.sendEmptyMessageDelayed(MSG_WAIT4UUID_TIMEOUT, 2500);
					break;
				}
			case STATE_WAITING_FOR_UUID:
			/// }
    ...
    void handleIntent(Intent intent) {
		...
		/// M: # {
		} else if (BluetoothDevice.ACTION_UUID.equals(action) && mState == STATE_WAITING_FOR_UUID) {
			if (isUuidReady(device)) {
				nextStepConnect();
			}
		/// }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_NEXT_STEP:
                    ...
                    break;
				/// M: @ {
				case MSG_WAIT4UUID_TIMEOUT:
					Log.d(TAG, "MSG_WAIT4UUID_TIMEOUT");
					if (mState == STATE_WAITING_FOR_UUID) {
						nextStepConnect();
					}
					break;
				/// }
	...
	/// M: @ {
	private boolean isUuidReady(BluetoothDevice device) {
		boolean isHandsFreeReady = false, isA2dpReady = false;
		try {
			ParcelUuid[] uuids = device.getUuids();
			Log.d(TAG, "isUuidReady:");
			for (ParcelUuid uuid : uuids) {
				Log.d(TAG, "  " + uuid);
				if (uuid.equals(BluetoothUuid.Handsfree)) {
					isHandsFreeReady = true;
					Log.d(TAG, "isUuidReady/handsfree is ready");
				} else if (uuid.equals(BluetoothUuid.AudioSource)) {
					isA2dpReady = true;
					Log.d(TAG, "isUuidReady/audiosource is ready");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		return (isHandsFreeReady && isA2dpReady);
	}
	/// }
}
