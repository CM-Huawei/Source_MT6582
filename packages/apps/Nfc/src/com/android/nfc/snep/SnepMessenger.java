...
public class SnepMessenger {
    ...
    public SnepMessenger(boolean isClient, LlcpSocket socket, int fragmentLength) {
        mSocket = socket;
		/// M: @ {
		int remoteMiu = mSocket.getRemoteMiu();
		int localMiu = mSocket.getLocalMiu();
        mFragmentLength = Math.min(remoteMiu, localMiu);
        Log.d(TAG, "remoteMiu: " + remoteMiu + ", localMiu = " + localMiu + ", we're using " + mFragmentLength);
		/// }
        mIsClient = isClient;
    }
