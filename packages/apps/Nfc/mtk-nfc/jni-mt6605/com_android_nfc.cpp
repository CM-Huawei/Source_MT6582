...
bool nfc_cb_data_init(nfc_jni_callback_data* pCallbackData, void* pContext)
{
   ...
   pCallbackData->status = NFC_STATUS_FAIL;
...
nfc_jni_native_monitor_t* nfc_jni_init_monitor(void)
{
   ...
   if(nfc_jni_native_monitor != NULL)
   {
      ...
      for ( index = 0; index < NFC_LLCP_SERVICE_NUM ; index++)
      {
         if(pthread_mutex_init(&nfc_jni_native_monitor->incoming_socket_mutex[index], NULL) == -1)
         {
            ALOGE("NFC Manager incoming socket mutex creation returned 0x%08x", errno);
            return NULL;
         }
   
         if(pthread_cond_init(&nfc_jni_native_monitor->incoming_socket_cond[index], NULL) == -1)
         {
