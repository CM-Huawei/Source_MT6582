...
final class CachedBluetoothDevice implements Comparable<CachedBluetoothDevice> {
    ...
    // Following constants indicate the user's choices of Phone /message access settings
    ...
    private void fetchName() {
        ...
        Xlog.d(TAG, "fetchName, AlaisName is " + mName);

        if (TextUtils.isEmpty(mName)) {
            mName = mDevice.getAddress();
            if (DEBUG) Log.d(TAG, "Device has no name (yet), use address: " + mName);
        }
        Xlog.d(TAG, "fetchName, Return Name " + mName);
    }
    ...
    boolean isBusy() {
        ...
        Xlog.d(TAG, mName + " bond state is " + getBondState());
    ...
    private void fetchBtClass() {
        ...
        if (mBtClass == null) {
            Xlog.d(TAG, "fetchClass, mBtClass is null");
        } else {
            int mClass = mBtClass.getMajorDeviceClass();
            Xlog.d(TAG, "fetchClass, mBtClass is " + mClass);
        }
    }
    ...
    private boolean updateProfiles() {
        ...
        if (uuids == null) {
            Xlog.d(TAG, "Bluetooth device get uuid is null");
            return false;
        }

        ParcelUuid[] localUuids = mLocalAdapter.getUuids();
        if (localUuids == null) {
            Xlog.d(TAG, "Bluetooth Adapter get uuid is null");
            return false;
        }

        Xlog.d(TAG, mName + " update profiles");
    ...
    void onBondingStateChanged(int bondState) {
        Xlog.d(TAG, "onBondingStateChanged to " + bondState);
        ...
        if (bondState == BluetoothDevice.BOND_BONDED) {
            Xlog.d(TAG, "Bond state changed to bonded, mConnectAfterPairing is " + mConnectAfterPairing);
    ...
    List<LocalBluetoothProfile> getConnectableProfiles() {
        Xlog.d(TAG, mName + " mprofile size is " + mProfiles.size());
        ...
        Xlog.d(TAG, mName + " conectable profile size is " + connectableProfiles.size());
