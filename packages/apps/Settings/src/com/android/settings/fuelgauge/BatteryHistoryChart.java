...
public class BatteryHistoryChart extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mLargeMode) {
            ...
            if (mDurationStringWidth > width) {
                splitLongString(width);
                canvas.drawText(mFirstDurationStr, textStartX, -mTextAscent + (mLineWidth/2),
                        mTextPaint);
                canvas.drawText(mSecondDurationStr, textStartX, -mTextAscent + (mLineWidth/2) 
                        + ((mTextDescent-mTextAscent)), mTextPaint);
            } else {
            // @}
            ...
            if (layoutRtl) durationHalfWidth = -durationHalfWidth;
            // {@ M : long string solution
            float x = (width/2) - (mDurationStringWidth/2);
            if (x < 0) {
                if (layoutRtl) {
                    x = width;
                } else {
                ...
                splitLongString(width);
                canvas.drawText(mFirstDurationStr, x,
                        (height/2) - ((mTextDescent-mTextAscent)) - mTextAscent, mTextPaint);
                canvas.drawText(mSecondDurationStr, x,
                        (height/2) -mTextAscent, mTextPaint);
            } else {
            // @}
    ...
    // {@ M : long string solution
    private String mFirstDurationStr = "";
    private String mSecondDurationStr = "";

    private void splitLongString(int width) {
        mFirstDurationStr = "";
        mSecondDurationStr = "";
        String spaceString = " ";
        String[] splitStrs = mDurationString.split(spaceString);
        int index = 0;
        for (int i = 0; i < splitStrs.length; i++) {
            mSecondDurationStr  = mSecondDurationStr + splitStrs[i] + spaceString;
            int textWidth = (int)mTextPaint.measureText(mSecondDurationStr);
            if (textWidth > width) {
                index = i;
                break;
            } else {
                mFirstDurationStr = mSecondDurationStr;
            }
        }
        mSecondDurationStr = "";
        for (int i = index; i<splitStrs.length; i++) {
            mSecondDurationStr = mSecondDurationStr + splitStrs[i] + spaceString;
        }
    }
    // @}
