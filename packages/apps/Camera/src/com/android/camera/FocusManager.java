...
public class FocusManager implements Camera.OnOrientationListener , Camera.OnParametersReadyListener {
    ...
    public void doSnap() {
        Log.i(TAG, "mInitialized =" + mInitialized + " mState=" + mState);
        if (!mInitialized) { return; }
    ...
    public void onAutoFocus(boolean focused) {
        Log.i(TAG, "onAutoFocus focused=" + focused + " mState=" + mState + " mFocusMode=" + mFocusMode);
    ...
    public void onAutoFocusMoving(boolean moving) {
        ...
        if (mState != STATE_IDLE && mState != STATE_UNKNOWN) { return; }

        //if current focus mode is INFINISTY, then don't show AF box
        if (getCurrentFocusMode(mContext).equals(Parameters.FOCUS_MODE_INFINITY)) {return;}
    ...
    public void onLongPress(int x, int y) {
        ...
        // Convert the coordinates to driver format.
        int[] pts = calculateTapPoint(x, y);
        if (mListener != null) {
            mListener.stopObjectTracking();
            mListener.startObjectTracking(pts[0], pts[1]);
            
            RelativeLayout.LayoutParams params =
                    (RelativeLayout.LayoutParams) getFrameview().getLayoutParams();
            int len = Math.min(mPreviewWidth, mPreviewHeight) / 3;
            params.width = len;
            params.height = len;
            int left = Util.clamp(x - len / 2, 0, mPreviewWidth - len);
            int top = Util.clamp(y - len / 2, 0, mPreviewHeight - len);
    ...
    public String getFocusMode() {
        Log.d(TAG, "getFocusMode() mOverrideFocusMode=" + mOverrideFocusMode
    ...
    //when in the MT mode, and take MT picture,will found in the process of MT,
    //camera UI is showing.the reason is :when call shutter down,will do a focus
    //when the onAutoFocus callback,will restore the view state,so the UI is abnormal.
    //in this case we set the needAutoFocusCall is false,not do this time focus
    
    public void setNeedAutoFocus(boolean isNeed) {
        mNeedAutoFocus = isNeed;
    }
    ...
    private boolean needAutoFocusCall() {
        ...
        boolean needAutoFocus = !(focusMode.equals(Parameters.FOCUS_MODE_INFINITY)
                ...
                || focusMode.equals(Parameters.FOCUS_MODE_EDOF)) && mNeedAutoFocus;
    ...
    public FocusManager(Camera context, ComboPreferences preferences, String[] defaultFocusModes,
        ...
        Log.d(TAG, "FocusManager(" + continous + ")");
        if (defaultFocusModes != null) {
            for (int i = 0, len = defaultFocusModes.length; i < len; i++) {
                Log.d(TAG, "FocusManager() defaultFocusModes[" + i + "]=" + defaultFocusModes[i]);
            }
        }
    }
