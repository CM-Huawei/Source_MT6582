...
public class FrameManager extends ViewManager implements Camera.OnOrientationListener{
 ...
 // For object tracking
    ...
    private static final int[] OBJECT_TRACKING_ICON = new int[] {
        R.drawable.ic_object_tracking,
        R.drawable.ic_object_tracking_succeed,
        R.drawable.ic_object_tracking_failed };

    private boolean mEnableFaceBeauty;
    private Camera mContext;
    private FrameView mFrameView = null;

    public FrameManager(Camera context) {
        super(context);
    ...
    public void initializeFrameView(boolean isOtStated) {
        CameraInfo info = CameraHolder.instance().getCameraInfo()[mContext
                .getCameraId()];
        if (isOtStated) {
    ...
    public void setView(FrameView view) {
        ...
        if (mContext.getFocusManager() != null) {
            mFrameView.setFocusIndicatorRotateLayout(mContext.getFocusManager()
                    .getFocusLayout());
        }
