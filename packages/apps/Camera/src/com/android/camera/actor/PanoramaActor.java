...
public class PanoramaActor extends PhotoActor {
    ...
    public PanoramaActor(Camera context) {
        super(context);
        mS3DMode = mCamera.isStereoMode();
        Log.i(TAG, "PanoramaActor initialize");
    ...
    public int getMode() {
    	if (mS3DMode) {
    		return ModePicker.MODE_PANORAMA_SINGLE_3D;
    	} else {
    ...
    public void onCameraParameterReady(boolean startPreview) {
        ...
        if (mS3DMode) {
        	mPanoramaHandler.sendEmptyMessage(MSG_LOCK_ORIENTATION);
        }
    }
    ...
    private boolean startCapture() {
        if (mCamera.getCameraDevice() != null && mCaptureState == IDLE && !mStopping) {
            ...
            Log.i(TAG, "start mCaptureState: " + mCaptureState);
            return false;
        }
    }
    ...
    private void stopAsync(final boolean isMerge) {
        Log.i(TAG, "stopAsync mStopping: " + mStopping);
    ...
    private final class PanoramaCallback implements AUTORAMACallback {
        public void onCapture(byte[] jpegData) {
            ...
            if (mCurrentNum == NUM_AUTORAMA_CAPTURE || mCaptureState == MERGING) {
                Log.d(TAG, "autorama done");
                mJpegImageData = jpegData;
                mCaptureState = IDLE;
                onHardwareStopped(true);
            } else if (mCurrentNum >= 0 && mCurrentNum < NUM_AUTORAMA_CAPTURE) {
                playShutterSound();
            	if (mCurrentNum == 1 && mS3DMode) {
            		mCamera.getShutterManager().setOkButtonEnabled(true);
            	}
    ...
    private void safeStop() {
        ...
        Log.i(TAG, "check stopAsync thread state, if running,we must wait");
        checkStopProcess();
        synchronized (holder) {
            stopPreview();
        }
    ...
    private void doStop(boolean isMerge) {
        Log.i(TAG, "doStop isMerge " + isMerge);
    ...
    private void onHardwareStopped(boolean isMerge) {
        Log.i(TAG, "onHardwareStopped isMerge: " + isMerge);
    ...
    private void onCaptureDone(boolean isMerge) {
        Log.i(TAG, "onCaptureDone isMerge " + isMerge + " mCameraState=" + mCamera.getCameraState());
    ...
    private void stopCapture(boolean isMerge) {
        Log.i(TAG, "stopCapture isMerge = " + isMerge);
    ...
    public void onShutterButtonLongPressed(ShutterButton button) {
        Log.d(TAG, "PanoramaActor.onShutterButtonLongPressed(" + button + ")");
    ...
    public void onShutterButtonClick(ShutterButton button) {
        Log.i(TAG, "PanoramaActor.onShutterButtonClick(" + button + ")");
    ...
    public boolean capture() {
        ...
        if (mS3DMode) {
        	mCamera.getShutterManager().setOkButtonEnabled(false);
        }
    ...
    public void showGuideString(int step) {
        ...
        switch (step) {
        case GUIDE_SHUTTER:
            guideId = mS3DMode ? R.string.panorama3d_guide_shutter
                    : R.string.panorama_guide_shutter;
            break;
        case GUIDE_MOVE:
            guideId = R.string.panorama_guide_choose_direction;
            break;
        case GUIDE_CAPTURE:
            guideId = mS3DMode ? R.string.panorama_guide_capture 
                    : R.string.panorama3d_guide_capture;
            break;
        default:
            break;
        }
    ...
    class PanoramaCategory extends CameraCategory {
        ...
        public void shutterUp() {
            Log.d(TAG,"PanoramaCategory.shutterUp");
            mShutterPressed = false;
