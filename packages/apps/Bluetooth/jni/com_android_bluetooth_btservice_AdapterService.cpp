...
static jmethodID method_adapterResetIndCallback;
...
static void adapter_reset_ind_callback() {
    if (!checkCallbackThread()) {
       ALOGE("Callback: '%s' is not called on the correct thread", __FUNCTION__);
       return;
    }
    ...
    callbackEnv->CallVoidMethod(sJniCallbacksObj, method_adapterResetIndCallback);
...
static void classInitNative(JNIEnv* env, jclass clazz) {
	...
	method_adapterResetIndCallback= env->GetMethodID(jniCallbackClass,
                                                    "adapterResetIndCallback", "()V");
