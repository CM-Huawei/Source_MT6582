...
public class HidService extends ProfileService {
    ...
    boolean disconnect(BluetoothDevice device) {
        enforceCallingOrSelfPermission(BLUETOOTH_PERM, "Need BLUETOOTH permission");
        log("begin to disconnect,device=" + device);
    ...
    int getConnectionState(BluetoothDevice device) {
        ...
        log("getConnectionState:device=" + device + " state=" + state);
        return state;
    }
    ...
    public  int getPriority(BluetoothDevice device) {
        ...
        log("getPriority:priority=" + priority);
        return priority;
    }
    ...
    //M:fix bug:ALPS01008739,in the case unCheck the input device check box in setting,input device 
    //should not be allowed to connect success,but now input device can connect success through authorize,so in this case,
    //should disconnect the improper connection
    private boolean okToBroadcastConnectState(BluetoothDevice device, int halState, int prevState) {
        if (halState == CONN_STATE_CONNECTED
                && (prevState == BluetoothInputDevice.STATE_DISCONNECTED || prevState == BluetoothInputDevice.STATE_CONNECTING)
                && (!okToConnect(device))) {
            if (DBG)
                log("Incoming HID connection rejected");
            disconnectHidNative(Utils.getByteAddress(device));
            return false;
        } else if (halState == BluetoothInputDevice.STATE_CONNECTING && BluetoothProfile.PRIORITY_OFF == getPriority(device)) {
            if (DBG)
                log("when authorize priority off:not show connecting");
            return false;
        } else if (halState == BluetoothInputDevice.STATE_DISCONNECTING
                && BluetoothProfile.PRIORITY_OFF == getPriority(device)) {
            if (DBG)
                log("when authorize priority off:not show disconnecting");
            return false;
