    ...
    private String parseMmsHeaders(String hdrPart) {
        ...
        for(int i = 0, c = headers.length; i < c; i++) {
            ...
            if(headerType.contains("FROM")) {
                ...
                if(D) Log.w(TAG, "[parseMmsHeaders] Skipping unknown header: " + headerType + " (" + header + ")");
            }
        }
        return null;
    }
    ...
    private void parseMmsMimePart(String partStr) {
        ...
        for(String header : headers) {
            ...
            if(headerType.contains("CONTENT-TYPE")) {
                ...
                Log.d(TAG, "[parseMmsMimePart] *** CONTENT-TYPE: " + newPart.contentType);
            }
                ...
                if(D) Log.w(TAG, "[parseMmsMimePart] Skipping unknown part-header: " + headerType + " (" + header + ")");
            }
        }
        // Now for the body
    ...
    private void parseMms(String message) {
        ...
        else
        {
            ...
            if (mimeParts.length == 1) {
                String bound = boundary.replace("\"", "");
                mimeParts = messageBody.split("--" + bound);
            }
            /// M: remove -1
            for(int i = 0; i < mimeParts.length - 1; i++) {
                String part = mimeParts[i];
                if (part != null && (part.length() > 0) && part.contains(":"))
