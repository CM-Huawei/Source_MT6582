...
public class BluetoothMapContent {
    private static final String TAG = "[MAP]BluetoothMapContent";
    ...
    static final String[] THREADID_PROJECTION = new String[] {
        Threads._ID,
    };

    static final String[] SIMID_PROJECTION = new String[] {
    ...
    public BluetoothMapContent(final Context context) {
        ...
        if (mResolver == null) {
            Log.d(TAG, "[BluetoothMapContent] getContentResolver failed");
        }
    }

    private void addSmsEntry() {
        if (D) Log.d(TAG, "[addSmsEntry] *** Adding dummy sms ***");
            ...
            try {
                if (c.moveToFirst()) {
                    do {
                        ...
                        if (type == MMS_TO) {
                            if (D) Log.d(TAG, "[printMmsAddr]   recipient: " + add + " (type: " + type + ")");
                        } else if (type == MMS_FROM) {
                            if (D) Log.d(TAG, "[printMmsAddr]   originator: " + add + " (type: " + type + ")");
                        } else {
                            if (D) Log.d(TAG, "[printMmsAddr]   address other: " + add + " (type: " + type + ")");
                        }
    ...
    private void printMmsPartImage(long partid) {
        ...
        try {
            ...
            if (D) Log.d(TAG, "[printMmsPartImage] " + sb.toString());
    ...
    private void printMmsParts(long id) {
        ...
        if (D) Log.d(TAG, "[printMmsParts]   parts:");
        if (c != null) {
            try {
                if (c.moveToFirst()) {
                    do {
                        ...
                        if (D) Log.d(TAG, "[printMmsParts]     _id : " + partid +
                            "\n     ct : " + ct +
                            "\n     partname : " + name +
                            "\n     charset : " + charset +
                            "\n     filename : " + filename +
    ...
    public void dumpMmsTable() {
        if (D) Log.d(TAG, "[dumpMmsTable] **** Dump of mms table ****");
        Cursor c = mResolver.query(Mms.CONTENT_URI,
                MMS_PROJECTION, null, null, "_id DESC");
        if (c != null) {
            try {
                if (D) Log.d(TAG, "[dumpMmsTable] c.getCount() = " + c.getCount());
                c.moveToPosition(-1);
                while (c.moveToNext()) {
                    printMms(c);
            ...
            Log.d(TAG, "[dumpMmsTable] query failed");
        }
    }

    public void dumpSmsTable() {
        addSmsEntry();
        if (D) Log.d(TAG, "[dumpSmsTable] **** Dump of sms table ****");
        Cursor c = mResolver.query(Sms.CONTENT_URI,
                SMS_PROJECTION, null, null, "_id DESC");
        if (c != null) {
            try {
                if (D) Log.d(TAG, "[dumpSmsTable] c.getCount() = " + c.getCount());
                c.moveToPosition(-1);
                while (c.moveToNext()) {
                    printSms(c);
                }
            ...
            Log.d(TAG, "[dumpSmsTable] query failed");
        }
    }
    ...
    public void dumpMessages() {
        ...
        if (D) Log.d(TAG, "[dumpMessages] message listing size = " + msgListingSize("inbox", ap));
    ...
    private void setDateTime(BluetoothMapMessageListingElement e, Cursor c,
        ...
        if (D) Log.d(TAG, "[setDateTime]: " + date);
        e.setDateTime(date);
    }
    ...
    private BluetoothMapMessageListingElement element(Cursor c, FilterInfo fi,
        ...
        if (D) Log.d(TAG, "[element]: BluetoothMapMessageListingElement");
    ...
    private boolean matchRecipientMms(Cursor c, FilterInfo fi, String recip) {
        ...
        if (phone != null && phone.length() > 0) {
            if (phone.matches(recip)) {
                if (D) Log.d(TAG, "[matchRecipientMms] match recipient phone = " + phone);
                res = true;
            } else {
                String name = getContactNameFromPhone(phone);
                if (name != null && name.length() > 0 && name.matches(recip)) {
                    if (D) Log.d(TAG, "[matchRecipientMms] match recipient name = " + name);
                    res = true;
                } else {
                    res = false;
                }
    ...
    private boolean matchRecipientSms(Cursor c, FilterInfo fi, String recip) {
        ...
        if (msgType == 1) {
            ...
            if (phone != null && phone.length() > 0 && phone.matches(recip)) {
                if (D) Log.d(TAG, "[matchRecipientSms] match recipient phone = " + phone);
                res = true;
            } else if (name != null && name.length() > 0 && name.matches(recip)) {
                if (D) Log.d(TAG, "[matchRecipientSms] match recipient name = " + name);
                res = true;
            } else {
                res = false;
            }
    ...
    private boolean matchRecipient(Cursor c, FilterInfo fi, BluetoothMapAppParams ap) {
        ...
        if (recip != null && recip.length() > 0) {
            ...
            if (fi.msgType == FilterInfo.TYPE_SMS) {
                ...
                if (D) Log.d(TAG, "[matchRecipient] Unknown msg type: " + fi.msgType);
                res = false;
            }
        } else {
            res = true;
    ...
    private boolean matchOriginatorMms(Cursor c, FilterInfo fi, String orig) {
        ...
        if (phone != null && phone.length() > 0) {
            if (phone.matches(orig)) {
                if (D) Log.d(TAG, "[matchOriginatorMms] match originator phone = " + phone);
                res = true;
            } else {
                String name = getContactNameFromPhone(phone);
                if (name != null && name.length() > 0 && name.matches(orig)) {
                    if (D) Log.d(TAG, "[matchOriginatorMms] match originator name = " + name);
                    res = true;
                } else {
                    res = false;
                }
    ...
    private boolean matchOriginatorSms(Cursor c, FilterInfo fi, String orig) {
        ...
        if (msgType == 1) {
            ...
            if (phone !=null && phone.length() > 0) {
                if (phone.matches(orig)) {
                    if (D) Log.d(TAG, "[matchOriginatorSms] match originator phone = " + phone);
                    res = true;
                } else {
                    String name = getContactNameFromPhone(phone);
                    if (name != null && name.length() > 0 && name.matches(orig)) {
                        if (D) Log.d(TAG, "[matchOriginatorSms] match originator name = " + name);
                        res = true;
                    } else {
                        res = false;
                    }
   ...
   private boolean matchOriginator(Cursor c, FilterInfo fi, BluetoothMapAppParams ap) {
        ...
        if (orig != null && orig.length() > 0) {
            ...
            if (fi.msgType == FilterInfo.TYPE_SMS) {
                ...
                Log.d(TAG, "[matchOriginator] Unknown msg type: " + fi.msgType);
                res = false;
            }
        } else {
            res = true;
    ...
    private String setWhereFilter(String folder, FilterInfo fi, BluetoothMapAppParams ap) {
        ...
        if (D) Log.d(TAG, "[setWhereFilter] where: " + where);
    ...
    private void setFilterInfo(FilterInfo fi) {
        ...
        if (tm != null) {
            ...
            if (D) Log.d(TAG, "[setFilterInfo] phone type = " + fi.phoneType +
    ...
    public byte[] getMessage(String handle, BluetoothMapAppParams appParams) throws UnsupportedEncodingException{
        ...
        Log.d(TAG, "[getMessage]: type = " + type + " id = " + id);
        switch(type) {
        case SMS_GSM:
        case SMS_CDMA:
    ...
    public byte[] getSmsMessage(long id, int charset) throws UnsupportedEncodingException{
        ...
        if (c != null && c.moveToFirst()) {
            try {
                if (V)
                    Log.d(TAG, "[getSmsMessage] c.count: " + c.getCount());
                ...
                if (type == 1) // Inbox message needs to set the vCard as originator
                    ...
                    // Other messages sets the vCard as the recipient
    ...
    private byte[] readMmsDataPart(long partid) {
        ...
        try {
            ...
            // do nothing for now
            Log.w(TAG, "[readMmsDataPart] Error reading part data",e);
        } finally {
            try {
                os.close();
                is.close();
            ...
            try {
                if (c.moveToFirst()) {
                    do {
                        ...
                        if(D) Log.d(TAG, "[extractMmsParts]     _id : " + partId +
                                "\n     ct : " + contentType +
                                "\n     partname : " + name +
                                "\n     charset : " + charset +
                                "\n     filename : " + filename +
                        ...
                        try {
                            ...
                            Log.d(TAG, "[extractMmsParts] NumberFormatException = ",e);
                            part.data = null;
                            part.charsetName = null;
                        } catch (UnsupportedEncodingException e) {
                            Log.d(TAG, "[extractMmsParts] UnsupportedEncodingException = ",e);
                            part.data = null;
                            part.charsetName = null;
                        } finally {
                        }
    ...
    public static final Uri MAP_CONTENT_URI = Uri.withAppendedPath(
            MmsSms.CONTENT_URI, "conversations/map");

    public static final Uri MMSSMS_CONTENT_URI = Uri.withAppendedPath(
            MmsSms.CONTENT_URI, "conversations");

    public long getThreadIdByNumber(String number) {
        if (TextUtils.isEmpty(number)) {
            return -1;
        }
        Log.d(TAG, "[getThreadId]: number = " + number);
        long threadId = -1;
        Cursor c = mResolver.query(MAP_CONTENT_URI, THREADID_PROJECTION, number, null, null);
        if (c != null) {
            try {
                if (c.getCount() == 1 && c.moveToFirst()) {
                    threadId = c.getLong(c.getColumnIndex(Threads._ID));
                }
            } finally {
               c.close();
            }
    ...
    public long getSimIdByThread(long threadId) {
        Log.d(TAG, "[getSimIdByThread]: threadId = " + threadId);
        long simId = -1;
        Uri uri = Uri.withAppendedPath(MMSSMS_CONTENT_URI, String.valueOf(threadId));
        Cursor c = mResolver.query(uri, SIMID_PROJECTION, null, null, null);
        if (c != null) {
            try {
                if (c.moveToLast()) {
                    String type = c.getString(c.getColumnIndex(MmsSms.TYPE_DISCRIMINATOR_COLUMN));
                    if (type.equals("sms")) {
                        simId = c.getLong(c.getColumnIndex(Sms.SIM_ID));
                    } else if (type.equals("mms")) {
                        simId = c.getLong(c.getColumnIndex(Mms.SIM_ID));
                    }
                }
            } finally {
               c.close();
