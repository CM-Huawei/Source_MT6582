...
public class BluetoothMapSmsPdu {
    ...
    private static final String TAG = "[MAP]BluetoothMapSmsPdu";
...
//            }
        ...
        public void gsmDecodeUserDataHeader() {
            ...
            if(gsmSubmitHasUserDataHeader() == true) {
                ...
                if(encoding == SmsConstants.ENCODING_7BIT)
                {
                    ...
                    try {
                        ...
                        Log.w(TAG, "[gsmDecodeUserDataHeader] unable to read userDataHeader", e);
                    }
            ...
            if(V) {
                Log.v(TAG, "[gsmDecodeUserDataHeader] encoding:" + encoding);
                Log.v(TAG, "[gsmDecodeUserDataHeader] msgSeptetCount:" + msgSeptetCount);
                Log.v(TAG, "[gsmDecodeUserDataHeader] userDataSeptetPadding:" + userDataSeptetPadding);
                Log.v(TAG, "[gsmDecodeUserDataHeader] languageShiftTable:" + languageShiftTable);
                Log.v(TAG, "[gsmDecodeUserDataHeader] languageTable:" + languageTable);
                Log.v(TAG, "[gsmDecodeUserDataHeader] userDataMsgOffset:" + userDataMsgOffset);
            }
        }
        ...
        private void gsmWriteDate(ByteArrayOutputStream header, long time) throws UnsupportedEncodingException {
            ...
            if(V) Log.v(TAG, "[gsmWriteDate] Generated time string: " + timeStr);
...
/*        private void gsmSubmitExtractUserData() {
        ...
        public void gsmChangeToDeliverPdu(long date, String originator)
        {
            ...
            try {
                ...
                Log.e(TAG, "[gsmChangeToDeliverPdu] IOException", e);
    ...
    private static String gsmParseSubmitPdu(byte[] data) {
        ...
        if ((dataCodingScheme & 0x80) == 0) {
            ...
            if (userDataCompressed) {
                Log.w(TAG, "[gsmParseSubmitPdu] 4 - Unsupported SMS data coding scheme "
                ...
                case 1: // 8 bit data
                case 3: // reserved
                    Log.w(TAG, "[gsmParseSubmitPdu] 1 - Unsupported SMS data coding scheme "
            ...
            if (dataCodingScheme == 0x84) {
                ...
                Log.w(TAG, "[gsmParseSubmitPdu] 5 - Unsupported SMS data coding scheme "
                        + (dataCodingScheme & 0xff));
            }
        } else {
            Log.w(TAG, "[gsmParseSubmitPdu] 3 - Unsupported SMS data coding scheme "
                    + (dataCodingScheme & 0xff));
        }
        ...
        try {
            ...
            case  SmsConstants.ENCODING_8BIT:
                Log.w(TAG, "[gsmParseSubmitPdu] Unknown encoding type: " + encodingType);
                messageBody = null;
                break;
            ...
            case  SmsConstants.ENCODING_7BIT:
                ...
                Log.i(TAG, "[gsmParseSubmitPdu] Decoded as 7BIT: " + messageBody);

                break;

            case  SmsConstants.ENCODING_16BIT:
                messageBody = new String(pdu.getData(), pdu.getUserDataMsgOffset(), pdu.getUserDataMsgSize(), "utf-16");
                Log.i(TAG, "[gsmParseSubmitPdu] Decoded as 16BIT: " + messageBody);
                break;
            ...
            case SmsConstants.ENCODING_KSC5601:
                ...
                Log.i(TAG, "[gsmParseSubmitPdu] Decoded as KSC5601: " + messageBody);
                break;
            }
        } catch (UnsupportedEncodingException e) {
            Log.e(TAG, "[gsmParseSubmitPdu] Unsupported encoding type???", e); // This should never happen.
            return null;
        }
