...
class HeadsetPhoneState {
    ...
    /// M: GEMINI support @{
    private int[] mServicePool = {HeadsetHalConstants.NETWORK_STATE_NOT_AVAILABLE,
    ...
    /// M: GEMINI support @{
    private int[] mSignalPool = {0,0,0,0};
    /// @}
            ...
            if (!mListening) {
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT) {
                    for(int simID = PhoneConstants.GEMINI_SIM_1; simID<(PhoneConstants.GEMINI_SIM_1 + simNum); simID++){
                        TelephonyManagerEx.getDefault().listen(mPhoneStateListenerGemini,
    ...
    private PhoneStateListener mPhoneStateListenerGemini = new PhoneStateListener() {
        ...
        public void onServiceStateChanged(ServiceState serviceState) {
            ...
            int defaultSim = com.android.internal.telephony.PhoneConstants.GEMINI_SIM_1;

            if(serviceState.isCdma(serviceState.getRadioTechnology()))
            {
                mServiceState = serviceState;
            }
            ...
            {
                IBluetoothHeadsetPhone btPhoneProxy = mStateMachine.getBluetoothPhoneProxy();               
                if(null == btPhoneProxy){
                    Log.d(TAG, "updateServiceForGemini: btPhoneProxy is null");
                }
                else
                {
                    try{                
                        defaultSim = btPhoneProxy.getDefaultSIM();
            ...
            {
                Log.d(TAG, "updateServiceForGemini: mStateMachine is null");
            }

            mServicePool[serviceState.getMySimId()] = (serviceState.getState() == ServiceState.STATE_IN_SERVICE) ?
                                HeadsetHalConstants.NETWORK_STATE_AVAILABLE :
                                HeadsetHalConstants.NETWORK_STATE_NOT_AVAILABLE;

            int tempService = HeadsetHalConstants.NETWORK_STATE_NOT_AVAILABLE;

            for(int service : mServicePool){
                if(service == HeadsetHalConstants.NETWORK_STATE_AVAILABLE){
                    tempService = service;
                    break;
                }
            }

            mService = tempService;

            if(defaultSim == serviceState.getMySimId())
            {
                setRoam(serviceState.getRoaming() ? HeadsetHalConstants.SERVICE_TYPE_ROAMING
                                                  : HeadsetHalConstants.SERVICE_TYPE_HOME);
            }

            Log.d(TAG, "updateServiceForGemini, mService="+mService + " tempService="+tempService + " simNum="+serviceState.getMySimId() + " defaultSim="+ defaultSim);
        ...
        public void onSignalStrengthsChanged(SignalStrength signalStrength) {
            if (signalStrength.isGsm()) {
                mSignalPool[signalStrength.getMySimId()] = gsmAsuToSignal(signalStrength);
            } else {
                mSignalPool[signalStrength.getMySimId()] = cdmaDbmEcioToSignal(signalStrength);
            }

            int tempSignal = 0;

            for(int signal : mSignalPool){
                if(signal > tempSignal){
                    tempSignal = signal;
                }
            }

            Log.d(TAG, "updateSignalForGemini, mSignal="+mSignal + " tempSignal="+tempSignal + " simNum="+signalStrength.getMySimId());

            if(mSignal != tempSignal){
                mSignal = tempSignal;
                sendDeviceStateChanged();
            }
        }
