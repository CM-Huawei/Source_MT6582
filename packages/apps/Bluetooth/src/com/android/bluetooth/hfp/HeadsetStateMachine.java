...
final class HeadsetStateMachine extends StateMachine {
    ...
    /// M: Video Call Support @{
    private static final String PHONE_PACKAGE = "com.android.phone"; 
    private static final String OUTGOING_CALL_BROADCASTER = "com.android.phone.OutgoingCallBroadcaster";
    private static final int INTERNATIONAL_DIAL_OPTION_IGNORE = 2;
    /// @}
    ...
    ///M: DAUL TALK SUPPORT : to judge if connected from AudioOn state @{ 
    IState mPrevState;
    /// }@

    ///M: ALPS01067036, to avoid othre module to request connection status while still process in state machine,
    ///    we try to log the Entering State from Pending and consider this isConnected() @{
    IState mEnteringState;
    /// }@
    ...
    private class Pending extends State {
        ...
        @Override
        public void exit() {
            log("Exit Pending: " + getCurrentMessage().what);
        ///M: DAUL TALK SUPPORT @{ 
            mPrevState = getCurrentState();
        ///}@
        }
            ...
            case HeadsetHalConstants.CONNECTION_STATE_CONNECTED:
                if ((mCurrentDevice != null) && mCurrentDevice.equals(device)) {
                    ...
                    broadcastConnectionState(mTargetDevice, BluetoothProfile.STATE_CONNECTED,
                                             BluetoothProfile.STATE_CONNECTING);
                    log("outgoing connection request done");
                    synchronized (HeadsetStateMachine.this) {
                        mCurrentDevice = mTargetDevice;
                        mTargetDevice = null;
                        transitionTo(mConnected);
                    ///M: ALPS01067036 @{ 
                        mEnteringState = mConnected;
                    ///}@
                    }
                } else if (mIncomingDevice != null && mIncomingDevice.equals(device)) {
    ...
    private class Connected extends State {
        ...
        public void enter() {
            ...
            ///M: DAUL TALK SUPPORT : inform BluetoothPhoneService to do first call restriction for dual talk projects
            if(mPrevState != mAudioOn){
                if (mPhoneProxy != null) {
                    try {
                        mPhoneProxy.restrictMultitalks();
        ...
        @Override
        public void exit() {
            log("Exit Connected: " + getCurrentMessage().what);
        ///M: DAUL TALK SUPPORT @{ 
            mPrevState = getCurrentState();
        ///}@
        }
            ...
            if (mPhoneProxy != null) {
                try {
                    ...
                    if(isVirtualCallInProgress()){
                        log("update virtual call again after SLC connected to trigger SCO connection");
    ...
    private class AudioOn extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            boolean retValue = HANDLED;
            switch(message.what) {
                ///M: [Google issue] [ALPS01032298] CONNECT msg in AudioOn state should be deferred to next state for some cases @{ 
                case CONNECT:
                {
                    BluetoothDevice device = (BluetoothDevice) message.obj;
                    ///if DISCONNECT_AUDIO is being processed
                    if(BluetoothHeadset.STATE_AUDIO_DISCONNECTED == mAudioState)
                    {
                        if (DBG) Log.d(TAG,"Try to connect [" + device + "] when disconnecting SCO of [" + mCurrentDevice +"]");
                        ///Defer the message only when trying to connect to a different device
                        if (!mCurrentDevice.equals(device)) {
                            deferMessage(message);
                        }
                    }
        ...
        @Override
        public void exit() {
            log("Exit AudioOn: " + getCurrentMessage().what);
        ///M: DAUL TALK SUPPORT @{ 
            mPrevState = getCurrentState();
        ///}@
        }
    ...
    int getConnectionState(BluetoothDevice device) {
        ...
        log("getConnectionState: current state" + getCurrentState() + ", entering state:" + mEnteringState +", device:" + device);

        if (getCurrentState() == mDisconnected) {
            return BluetoothProfile.STATE_DISCONNECTED;
        }

        log("getConnectionState: mTargetDevice=" + mTargetDevice + ", mCurrentDevice=" + mCurrentDevice + ", mIncomingDevice=" + mIncomingDevice);
    ...
    private boolean isVirtualCallInProgress() {
        log("isVirtualCallInProgress : " + mVirtualCallStarted);
    ...
    private void processDialCall(String number) {
        ...
        if ((number == null) || (number.length() == 0)) {
            ...
            dialNumber = callNum.strNum;
            isVtCall = callNum.isVtCall;
            /// @}
            if (dialNumber == null) {
            ...
            /// M: Memory Dial support @{
            if (DBG) log("processDialCall, memory dial is supported by MTK");

            String args = number.substring(1);
            log("ATD memory dial : " + args);

            try{
                dialNumber = getPhoneNumberByIndex(Integer.parseInt(args), true);
            } catch(NumberFormatException e ){
                Log.e(TAG,"[ERR] wrong memory index : " + args);
            }
            /// @}
            
            if (dialNumber == null) {
                if (DBG) log("processDialCall, memory dial number null");
                atResponseCodeNative(HeadsetHalConstants.AT_RESPONSE_ERROR, 0);
                return;
            }
        } else {
        ...
        processOutgoingCall(dialNumber, isVtCall);
    ...
    private void processKeyPressed() {
        if (mPhoneState.getCallState() == HeadsetHalConstants.CALL_STATE_INCOMING) {
            ...
            AtPhonebook.CallNumber callNum = mPhonebook.getLastDialledNumber(new AtPhonebook.CallNumber());
            /// @}
            if (callNum.strNum == null) {
                if (DBG) log("processKeyPressed, last dial number null");
                return;
            }
            
            /// M: Vedio Call support @{
            processOutgoingCall(callNum.strNum, callNum.isVtCall);
            /// @}
        }
    }
    
    /// M: Vedio Call support @{
    private void processOutgoingCall(String strDialNum, boolean isVtCall){
        if (mPhoneProxy != null) {
            Intent intent = new Intent(Intent.ACTION_CALL_PRIVILEGED,
                                       Uri.fromParts(SCHEME_TEL, strDialNum, null));
            /// For Gemini
            intent.setClassName(PHONE_PACKAGE, OUTGOING_CALL_BROADCASTER);
            intent.putExtra(Intent.EXTRA_IS_FORBIDE_DIALOG, true);
            intent.putExtra(Intent.EXTRA_INTERNATIONAL_DIAL_OPTION, INTERNATIONAL_DIAL_OPTION_IGNORE);        
            /// For Video Call
            intent.putExtra(Intent.EXTRA_IS_VIDEO_CALL, isVtCall);
            /// @}
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            
            log("processOutgoingCall: number=" + strDialNum + " isVtCall="+isVtCall);
            mService.startActivity(intent);
        } else {
            Log.e(TAG, "Handsfree phone proxy null for processOutgoingCall");
        }    
    }
    /// @}
    ...
    private String getPhoneNumberByIndex(final int index, boolean orderByName) {
        Context context = (Context)mService;
        log("getPhoneNumberByIndex(" + index + ", " + String.valueOf(orderByName) + ")");
        if (index < 1) {
            log("getPhoneNumberByIndex : Invalid index value : " + index);
            return null;
        }
    ...
    private String querySuperPrimaryPhone(long contactId) {
        ...
        try {
            ...
            c = context.getContentResolver().query(dataUri,
    ...
    IBluetoothHeadsetPhone getBluetoothPhoneProxy(){
        return mPhoneProxy;
    }
    ...
    boolean isConnected() {
        ...
        log("isConnected : currentState:" + currentState + ", mEnteringState:" + mEnteringState);

        ///M: ALPS01067036 @{ 
        return (currentState == mConnected || currentState == mAudioOn || (currentState == mPending && mEnteringState == mConnected) );
        ///}@
    }
