...
public class AtPhonebook {
    ...
    public static class CallNumber{
        ...
        public CallNumber(String strNum, boolean isVtCall){
            this.strNum = strNum;
    ...
    public String getLastDialledNumber() {
        return getLastDialledNumber(new CallNumber()).strNum;
    }
    /// @}
            ...
            case TYPE_READ: // Read
                ...
                if (pbr == null) {
                    ...
                    if ("SM".equals(mCurrentPhonebook)){
                        atCommandErrorCode = SIMstatusToCME();
                        break;
                    }
                    /// @}
                    atCommandErrorCode = BluetoothCmeError.OPERATION_NOT_SUPPORTED;
        ...
        switch (type) {
            ...
            case TYPE_SET: // Set & read
                ...
                if (mCpbrIndex1 != -1) {
                   ...
                   log("handleCpbrCommand - mCpbrIndex1 != -1");
                   atCommandErrorCode = BluetoothCmeError.OPERATION_NOT_ALLOWED;
                   mStateMachine.atResponseCodeNative(atCommandResult, atCommandErrorCode);
                   break;
                }
                ...
                if ((atString.split("=")).length < 2) {
                   log("handleCpbrCommand - length < 2");
                    mStateMachine.atResponseCodeNative(atCommandResult, atCommandErrorCode);
                    break;
                }
    ...
    /// M: Vedio Call support @{
    public CallNumber getLastDialledNumber(CallNumber callNum){
        ...
        try{
            ...
            strNum = cursor.getString(column);
            if(FeatureOption.MTK_VT3G324M_SUPPORT){
                column = cursor.getColumnIndexOrThrow(Calls.VTCALL);
                isVtCall = (1 == cursor.getInt(column)) ? true : false;
                log("getLastDialledNumber: column=" + column + " getInt(column)= " + cursor.getInt(column));
            }            
            cursor.close();
        }catch(Exception e){
            log(e.toString());
        ...
        callNum.strNum = strNum;
        callNum.isVtCall = isVtCall;
    ...
    private int SIMstatusToCME(){
        ...
        int errCode = BluetoothCmeError.SIM_FAILURE;

        if(null == btPhoneProxy){
            log("SIMstatusToCME: btPhoneProxy is null");
            return errCode;
        }

        try{
            int status = TelephonyManager.getDefault().getSimState();
        
            log("SIMstatusToCME : ");
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                // M: GEMINI API should be put in TelephonyManagerEx
                status = TelephonyManagerEx.getDefault().getSimState(btPhoneProxy.getDefaultSIM());
            }
        
            log("SIMstatusToCME: status = " + status);
            switch (status) {
            case TelephonyManager.SIM_STATE_UNKNOWN:
            case TelephonyManager.SIM_STATE_ABSENT:
                errCode = BluetoothCmeError.SIM_MISSING;
            case TelephonyManager.SIM_STATE_PIN_REQUIRED:
                errCode = BluetoothCmeError.SIM_PIN_REQUIRED;
            case TelephonyManager.SIM_STATE_PUK_REQUIRED:
                errCode = BluetoothCmeError.SIM_PUK_REQUIRED;
            case TelephonyManager.SIM_STATE_NETWORK_LOCKED:
                errCode = BluetoothCmeError.PIN_REQUIRED;
            default:
                errCode = BluetoothCmeError.SIM_FAILURE;
            }
