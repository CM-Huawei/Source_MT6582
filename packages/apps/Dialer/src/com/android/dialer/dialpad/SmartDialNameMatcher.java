...
public class SmartDialNameMatcher {
    ...
    public SmartDialMatchPosition matchesNumber(String phoneNumber, String query, boolean useNanp) {
        ...
        ///M: fix dialer search je issue @{.
        if (phoneNumber == null || TextUtils.isEmpty(phoneNumber)) {
            LogUtils.d(TAG, "matchesNumber return null.phoneNumber ="+phoneNumber);
            return null;
        }
        ///M: @}
        constructEmptyMask(builder, phoneNumber.length());
        mPhoneNumberMatchMask = builder.toString();
        /// M: Support for MTK Dialer search {@
        int offset = phoneNumber.indexOf(query);
        if (offset < 0) {
            offset = 0;
        }
        /// M: @}
        ...
        SmartDialMatchPosition matchPos = matchesNumberWithOffset(phoneNumber, query, offset);
        if (matchPos == null) {
    ...
    /**-----------------------------------------------MTK-- -------------------------------------*/
    private static final String TAG = "SmartDialNameMatcher";
}
