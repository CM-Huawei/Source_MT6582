 ...
 */
public class CallLogReceiverEx extends BroadcastReceiver {
    private static final String TAG = "CallLogReceiver";

    @Override
    public void onReceive(Context context, Intent intent) {
        if (VoicemailContract.ACTION_NEW_VOICEMAIL.equals(intent.getAction())) {
            Intent serviceIntent = new Intent(context, CallLogNotificationsServiceEx.class);
            serviceIntent.setAction(CallLogNotificationsServiceEx.ACTION_UPDATE_NOTIFICATIONS);
            serviceIntent.putExtra(
                    CallLogNotificationsServiceEx.EXTRA_NEW_VOICEMAIL_URI, intent.getData());
