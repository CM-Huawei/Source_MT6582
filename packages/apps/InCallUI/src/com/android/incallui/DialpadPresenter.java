...
public class DialpadPresenter extends Presenter<DialpadPresenter.DialpadUi>
    ...
    //-----------------------MTK--------------------
    /**
     * If current call is VT, processDtmf
     *
     * @param c
     */
    private void processDtmfForVtIfNeeded(char c) {
        if (FeatureOptionWrapper.isSupportVT()) {
            if (!VTUtils.isVTIdle()) {
                Log.d(this, "processDtmfForVtIfNeeded()... c: " + String.valueOf(c));
                CallCommandClient.getInstance().onUserInput(String.valueOf(c));
            }
        }
    }
