...
public class InCallPresenter implements CallList.Listener {
    ...
    public void setActivity(InCallActivity inCallActivity) {
        ...
        if (inCallActivity != null) {
            ...
            */
            ...
            /// M: For SmartBook (ALPS01377629) @{
            // when connect to smart book, InCallActivity will be relaunched,
            // but finish() may be not called(flag will not be set), then isFinishing() will return false;
            /* Origin code:
            if (mInCallActivity != null && mInCallActivity.isFinishing()) {
            */
            if (mInCallActivity != null) {
            /// @}
                updateListeners = true;
    ...
    public void onCallListChange(CallList callList) {
        ...
        // incoming call screen and privacy protect don't open.
        /// M: ALPS01414144 @{
        /* Google code
         if (!newState.isIncoming()) {
         */
        if (!newState.isIncoming() && (!InCallUtils.isprivacyProtectOpen())) {
        /// @}
        ...
        if (mInCallActivity != null) {
            mInCallActivity.enableHomeKeyDispatched(newState.isIncoming());
        }
        /// @}
        ...
        /// M: For VT MO drop back @{
        // always update InCallActivity, just like onPhoneStateChange() on JB.
        if (mInCallActivity != null) {
            mInCallActivity.updateInCallActivity();
        }
        /// @}
    }
    ...
    public void onIncomingCall(Call call) {
        ...
        if (mInCallActivity != null && mInCallActivity.getInVoiceAnswerVideoCall()) {
            mInCallActivity.setInVoiceAnswerVideoCall(false);
        }
        /// @}
    ...
    public void onUiShowing(boolean showing) {
        /// M: for ALPS01270689 @{
        // to sync UI state to Telephony
        CallCommandClient.getInstance().onUiShowing(showing);
        /// @}
    ...
    public void bringToForeground(boolean showDialpad) {
        ...
        ///M: we remove this "mIsActivityPreviouslyStarted" to resolve when current is 
        // incall status, disable or enable
        // dialer app through setting, click dialer icon could not launch incall screen. @{
    ...
    private void maybeShowErrorDialogOnDisconnect(Call call) {
        ...
        if (isActivityStarted() && call.getState() == Call.State.DISCONNECTED) {
            ...
            boolean isVTCause = false;
            if (VTUtils.isVTCall(call)) {
                isVTCause = mInCallActivity.maybeShowErrorDialogOnDisconnectForVT(call.getDisconnectCause(),
                        InCallUtils.isIncomingCall(call), call.getNumber(), call.getSlotId());
    ...
    private InCallState startOrFinishUi(InCallState newState) {
        ...
        if (showCallUi) {
            ...
            if (isActivityStarted()) {
                mInCallActivity.dismissPendingDialogsAndHideConferenceFragment();
                /// M: for ALPS01255734 @{
                // dismiss all pop up menu when a new call incoming
                mInCallActivity.closeOptionsMenu();
                /// @}
            }
            startUi(newState);
 ...
 // true for checking, false for recording
        ...
        if (mountedStorageCount > 1) {
            ...
            if (PhoneRecorderUtils.STORAGE_TYPE_SD_CARD == storageType) {
                ...
                String toast = isForCheckingOrRecording ? mContext.getResources().getString(
                        R.string.vt_sd_not_enough) : mContext.getResources().getString(
                        R.string.vt_recording_saved_sd_full);
    ...
    public void showStorageFullDialog(final int resid, final boolean isSDCardExist) {
        ...
        CharSequence msg = mContext.getResources().getText(resid);
        ...
        CharSequence cancelButtonText = isSDCardExist ? mContext.getResources().getText(
                R.string.alert_dialog_dismiss) : mContext.getResources().getText(R.string.ok);
        AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(mContext).setMessage(msg)
                .setIcon(android.R.drawable.ic_dialog_alert)
                .setTitle(mContext.getResources().getText(R.string.reminder))
                .setNegativeButton(cancelButtonText, cancelClickListener)
                .setOnCancelListener(cancelListener);
        if (isSDCardExist) {
            dialogBuilder.setPositiveButton(
                    mContext.getResources().getText(R.string.vt_change_my_pic), oKClickListener);
        }
    ...
    public void onSuppServiceFailed(String message) {
        if (mInCallActivity != null) {
            mInCallActivity.showSuppMessageDialog(message);
        } else {
            Log.d(this, "Activity is null, do nothing.");
        }
    }

    public void setInVoiceAnswerVideoCall(boolean value) {
        if(mInCallActivity != null) {
            String number = mCallList.getIncomingCall().getNumber();
            if (value) {
              CallCommandClient.getInstance().setVTVoiceAnswerRelated(true, number);
            }
            mInCallActivity.setInVoiceAnswerVideoCall(value);
        }
    }
    ...
    // Dualtalk start.@{
    public DualtalkCallInfo mDualtalkCallInfo;

    public void updateDualtalkCallInfo(DualtalkCallInfo info) {
        mDualtalkCallInfo = info;
    }
    // Dualtalk end. @}

    public void showInCallMTK() {
        mContext.startActivity(getInCallIntent(false));
    }
}
