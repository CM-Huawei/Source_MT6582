...
public class ContactInfoCache implements ContactsAsyncHelper.OnImageLoadCompleteListener {
    ...
    /// M: for ALPS01278985 @{
    // just update the caller number presentation.
    // If presentation is allowed, return null.
    // TODO: if any server information coming, we have to update them here.
    static ContactCacheEntry updateCallerInformation(Context context, Call call) {
        final ContactCacheEntry entry = getInstance(context).getInfo(call.getCallId());
        final int numberPresentation = call.getIdentification().getNumberPresentation();

        if (entry != null) {
            // We do have a valid "name" in the CallerInfo. Display that
            // in the "name" slot, and the phone number in the "number" slot.
            if (numberPresentation != Call.PRESENTATION_ALLOWED) {
                // This case should never happen since the network should never send a name
                // AND a restricted presentation. However we leave it here in case of weird
                // network behavior
                entry.name = getPresentationString(context, numberPresentation);
                Log.d(TAG, "  presentation not allowed!" + " display name = " + entry.name);
            } else {
                // do not return any valid entry, because we do not need to update call card if
    ...
    public static ContactCacheEntry buildCacheEntryFromCall(Context context,
        ...
        /// M: Judge whether is sip call by calltype. @{
        if (Call.CALL_TYPE_SIP == identification.getCallType()) {
            entry.isSipCall = true;
        }
        /// @}
    ...
    private void findInfoQueryComplete(CallIdentification identification,
        ...
        /// M: Judge whether is sip call by calltype. @{
        if (Call.CALL_TYPE_SIP == identification.getCallType()) {
            cacheEntry.isSipCall = true;
            // For ALPS01383271. @{
            // When no name, but we do have a valid CNAP name, so use that.
            if (TextUtils.isEmpty(cacheEntry.name)
                    && !TextUtils.isEmpty(identification.getCnapName())) {
...
//                    }
        ...
        /// M: Add for show geo description always. @{
