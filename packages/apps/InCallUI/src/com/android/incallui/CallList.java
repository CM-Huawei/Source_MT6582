 ...
 */
public class CallList implements IInCallScreen {
    ...
    public void onDisconnect(Context context, Call call) {
    /// @}
        ...
        if (updated) {
            ...
            /// M: for VT Auto Dropback @{
            if (VTUtils.isVTCall(call) && !InCallUtils.isIncomingCall(call)
                    && VTInCallScreenFlags.getInstance().mVTAutoDropBack) {
                VTUtils.handleAutoDropBack(context, call);
            }
            /// @}
        }
    }
    ...
    public void onIncoming(Call call, List<String> textMessages) {
        ...
        /// M: For support Dualtalk. @{
        if (FeatureOptionWrapper.isSupportDualTalk()) {
            initIncomingCallListForDualtalk();
        }
        /// @}
    }
    ...
    public void onUpdate(List<Call> callsToUpdate) {
        ...
        /// M: For support Dualtalk. @{
        mInComingCallList = new ArrayList<Call>();
        /// @}
        /// M: ALPS01264365 update voice commmand.
        CallCommandClient.getInstance().updateVoiceCommand();
        for (Call call : callsToUpdate) {
            ...
            /// M: For support Dualtalk. @{
            if (FeatureOptionWrapper.isSupportDualTalk()) {
                addIncomingCallForDualtalk(call);
            }
            /// @}
    ...
    public Call getIncomingCall() {
        ...
        /// M: For Dualtak feature. @{
        if (FeatureOptionWrapper.isSupportDualTalk() && mInComingCallList != null
                && mInComingCallList.size() >= 2) {
            call = mInComingCallList.get(0);
        }
        /// @}
    ...
    public void onUpdateRecordState(final int state, final int customValue) {
        ...
        PhoneRecorderUtils.updateRecorderState(state);
        for (Listener listener : mListeners) {
    ...
    public void onSuppServiceFailed(String message) {
        InCallPresenter.getInstance().onSuppServiceFailed(message);
    }
    ...
    public HashMap<Integer, Call> getCallMap(){
        return mCallMap;
    }

    /// For Dualtalk start. @{
    public boolean isGsmPhoneFirst() {
        return getGsmActiveCall() != null && getCdmaActiveCall() != null;
    }

    public Call getActiveCallWithPhoneType(int phoneType) {
        for (Call call : mCallMap.values()) {
            if (call.getPhoneType() == phoneType && call.getState() == Call.State.ACTIVE) {
                return call;
            }
        }
        return null;
    ...
    public Call getGsmActiveCall() {
        return getActiveCallWithPhoneType(PhoneConstants.PHONE_TYPE_GSM);
    }

    public Call getCdmaActiveCall() {
        return getActiveCallWithPhoneType(PhoneConstants.PHONE_TYPE_CDMA);
    }

    private List<Call> mInComingCallList;
    ...
    private void addIncomingCallForDualtalk(Call call) {
        if (InCallPresenter.getInstance().mDualtalkCallInfo != null
                && InCallPresenter.getInstance().mDualtalkCallInfo.getHasMultipleRingingCall()
                && call.isIncoming()) {
            mInComingCallList.add(call);
            Log.d(this, "addIncomingCall; mInComingCallList = " + mInComingCallList);
        }
    }

    private void initIncomingCallListForDualtalk() {
        if (mCallMap == null) {
            Log.d(this, "initIncomingCallListForDualtalk; mCallMap is null.");
            return;
        }
        if (InCallPresenter.getInstance().mDualtalkCallInfo != null
                && !InCallPresenter.getInstance().mDualtalkCallInfo.getHasMultipleRingingCall()) {
            Log.d(this, "initIncomingCallListForDualtalk; only one incoming call and no need do this.");
            return;
        }
        mInComingCallList = new ArrayList<Call>();
        for (Call call : mCallMap.values()) {
    ...
    public void clearDisconnectStateForVT() {
        Log.i(this, "clearDisconnectVTCall()...");
        Call disconnectingCall = getDisconnectingCall();
        Call disconnectedCall = getDisconnectedCall();
        if (disconnectingCall != null && disconnectingCall.isVideoCall()) {
            finishDisconnectedCall(disconnectingCall);
        }
        if (disconnectedCall != null && disconnectedCall.isVideoCall()) {
            finishDisconnectedCall(disconnectedCall);
        }
    }
