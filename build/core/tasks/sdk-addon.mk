...
ifneq ($(filter banyan_addon banyan_addon_x86,$(TARGET_PRODUCT)),)
  ...
  ifeq ($(BUILD_MTK_SDK),bsp)
    staging := $(intermediates)/addon-bsp-mtk-$(last_released_sdk_version)
    full_target := $(HOST_OUT_SDK_ADDON)/addon-bsp-mtk-$(last_released_sdk_version).zip
  endif

  ifeq ($(BUILD_MTK_SDK),bsp)
$(full_target): PRIVATE_MTK_SDK_API_ADDON_ZIP := $(HOST_OUT_SDK_ADDON)/addon-bsp-mtk-$(last_released_sdk_version).zip
  else
