...
#include "venc_drv_if_public.h" // for MCI buffer 
#endif
...
#ifndef ANDROID_DEFAULT_CODE			
    ALOGI("createAudioSource,mAudioBitRate=%d",mAudioBitRate);
#endif
...
status_t StagefrightRecorder::setupSurfaceMediaSource() {
	...
	if ((err = checkVideoEncoderCapabilities(&encoderSupportsCameraSourceMetaDataMode)) != OK) {
		return err;
	}
#endif
...
#ifndef ANDROID_DEFAULT_CODE //for CTS vr test with parameter of 1920*1080 which is not 16 align
	checkVideoEncoderBufferLimit(); //qeury mVideoBufSizeLimit 
	//change videoSize recording to video buffer limitation
	videoSize.width = mVideoBufSizeLimit.width;
	videoSize.height = mVideoBufSizeLimit.height;
	ALOGI("setupCameraSource,mVideoBufSizeLimit.width =%d,mVideoBufSizeLimit.height=%d",mVideoBufSizeLimit.width,mVideoBufSizeLimit.height);
#endif
    if (mCaptureTimeLapse) {
...
#ifndef ANDROID_DEFAULT_CODE
				mPreviewSurface, encoderSupportsCameraSourceMetaDataMode, mSupportMCIbuffer);
#else
...
status_t StagefrightRecorder::setupVideoEncoder(
	...
	if(meta->findCString(kKeyColorEffect,&colorEffect)) {
		enc_meta->setCString(kKeyColorEffect,colorEffect);
	}
#endif
    ...
    *source = encoder;
	...
	if (mLivePhotoTagValue > 0) {
		...
		reinterpret_cast<OMXCodec *>(encoder.get())->
		vEncSetSenario(OMXCodec::kLivePhotoEffect);
	}
...
status_t StagefrightRecorder::setupMPEG4Recording(
    ...
    *totalBitRate = 0;
	...
	if(mEnableLivePhoto) {
	    ...
	    writer->addSource(mpLivePhotoSource);
	}
	else
        writer->addSource(encoder);
#else
...
status_t StagefrightRecorder::reset() {
	...
	mVideoBufSizeLimit.width = mVideoWidth;
	mVideoBufSizeLimit.height = mVideoHeight;
...
#ifndef ANDROID_DEFAULT_CODE
void StagefrightRecorder::checkVideoEncoderBufferLimit(){
	...
	mVideoBufSizeLimit.width = tInputBuflimit.u4Stride;
	mVideoBufSizeLimit.height = tInputBuflimit.u4SliceHeight;
