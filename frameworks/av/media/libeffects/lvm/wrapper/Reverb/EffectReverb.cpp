...
extern "C" int EffectCreate(const effect_uuid_t *uuid,
    ...
    if (memcmp(&desc->type, SL_IID_PRESETREVERB, sizeof(effect_uuid_t)) == 0) {
        ...
        ALOGD("EffectCreate - PRESET,prest:%d,curPreset:%d,nextPreset:%d",pContext->preset,pContext->curPreset,pContext->nextPreset);
    }else{
...
int Reverb_LoadPreset(ReverbContext   *pContext)
{
    ...
    ALOGD("Reverb_LoadPreset,pContext->curPreset:%d",pContext->curPreset);
...
int Reverb_setParameter (ReverbContext *pContext, void *pParam, void *pValue){
    ...
    if (pContext->preset) {
        if (param != REVERB_PARAM_PRESET) {
            ALOGE("param != reverb_param_preset,param:%d",param);
            return -EINVAL;
        }
        ...
        ALOGD("Reverb_setParameter, param_preset,curpreset %d,nextpreset:%d", pContext->curPreset,pContext->nextPreset);
        return 0;
    }
