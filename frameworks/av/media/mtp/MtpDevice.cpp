...
#define MAX_USBFS_BUFFER_SIZE   65536
bool MtpDevice::sendObject(MtpObjectInfo* info, int srcFD) {
    ...
    if (sendRequest(MTP_OPERATION_SEND_OBJECT)) {
		...
		mData.reset();
        char buffer[MAX_USBFS_BUFFER_SIZE];
		memset(buffer,0,MAX_USBFS_BUFFER_SIZE);
	...
	MtpObjectHandle MtpDevice::copyObject(MtpObjectHandle handle)
	{
		 MtpObjectInfo* info = getObjectInfo(handle);
		 MtpObjectHandle newHandle;
		   mRequest.reset();
		   mRequest.setParameter(1, handle);
		   mRequest.setParameter(2, info->mStorageID);
		   mRequest.setParameter(3, info->mParent);
		   if (!sendRequest(MTP_OPERATION_COPY_OBJECT))
			   return NULL;
		   MtpResponseCode ret = readResponse();
			ALOGD("copyObject MtpResponseCode: %x \n",ret);
		   if (ret == MTP_RESPONSE_OK) {
				 newHandle=mResponse.getParameter(1);
				 return newHandle;
		   }
		   return NULL;
}
