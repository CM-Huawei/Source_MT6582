...
//Added Modification for ALPS00255822, bug from WHQL test
    else if (mFormFlag == kFormLongString)
    {
        mLongStringMaxLength = packet.getUInt32();
    }
//Added Modification for ALPS00255822, bug from WHQL test
}
...
void MtpProperty::write(MtpDataPacket& packet) {
    ...
    switch (mType) {
        ...
        case MTP_TYPE_STR:
            ...
            if (deviceProp)
            {
                //keep the string length is 4 bytes
                int strLength = strlen(mCurrentValue.str);
    ...
    //Added Modification for ALPS00255822, bug from WHQL test
    //There is no group code of DeviceProperty
    if (!deviceProp)
        packet.putUInt32(mGroupCode);

    //if (!deviceProp)  //There is formFlag of DeviceProperty 
    sxlog_printf(ANDROID_LOG_VERBOSE, "MtpProperty",
                 "%s, line %d: , mFormFlag = 0x%x\n", __func__, __LINE__, mFormFlag);
    ...
    {
        packet.putUInt32(mLongStringMaxLength);
    }
    //Added Modification for ALPS00255822, bug from WHQL test
}
        ...
        int value = *values++;
            switch (mType) {
                ...
                case MTP_TYPE_UINT64:
                    ...
                    break;
                //Added Modification for ALPS00255822, bug from WHQL test
                //Add for WHQL test passed and support 800X800 only for temporary solution
                case MTP_TYPE_STR:
                    if(mCode == MTP_DEVICE_PROPERTY_IMAGE_SIZE)
                    {
                        uint16_t  stringBuffer[] = {'8','0','0','x','8','0','0', 0};
                        int length = 8;
                        //mEnumValues[i].str = strdup(buffer);
                        mEnumValues[i].str = (char*)malloc(length+1);
                        memset(mEnumValues[i].str, 0, length+1);
                        for(int x=0;x<length;x++)
                        {
                            mEnumValues[i].str[x] = stringBuffer[x];
                        }
                        sxlog_printf(ANDROID_LOG_VERBOSE, "MtpProperty",
                                     "%s: mEnumValues[i].str = %s", __func__, mEnumValues[i].str);
                    }
                    else
                        sxlog_printf(ANDROID_LOG_ERROR, "MtpProperty",
                                     "unsupported type for MtpProperty::setEnum");
                    break;
                //Added Modification for ALPS00255822, bug from WHQL test
                default:
...
//Added Modification for ALPS00255822, bug from WHQL test
void MtpProperty::setFormLongString(uint32_t  maxLength) {
     mFormFlag = kFormLongString;
     mLongStringMaxLength = maxLength;
}
//Added Modification for ALPS00255822, bug from WHQL test
