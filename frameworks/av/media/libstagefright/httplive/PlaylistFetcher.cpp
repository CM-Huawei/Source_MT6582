...
void PlaylistFetcher::postMonitorQueue(int64_t delayUs) {
    ...
    if(mPlaylist != NULL && mPlaylist->isComplete())
    {
       ...
       if(mSeqNumber > lastSeqNumberInPlaylist)
          needToCheckBWC = false;
    }
    
    if(needToCheckBWC && mSession->needToSwitchBandwidth(mNotify->dup(),mSeqNumber))
    {
         cancelMonitorQueue();
         return;
    }
...
void PlaylistFetcher::startAsync(
        ...
        const sp<AnotherPacketSource> &videoSource,
        const sp<AnotherPacketSource> &subtitleSource,
        int64_t startTimeUs,int32_t seqNumber) {
#endif
...
#ifndef ANDROID_DEFAULT_CODE
    int32_t seqNumber = -1;
    CHECK(msg->findInt32("seqNumber", (int32_t *)&seqNumber));
    if(seqNumber >= 0)
    {
        mSeqNumber = seqNumber;
        mStartup = true;
        ALOGD("seqnumber:%d,mStreamTypeMask:%d",mSeqNumber,mStreamTypeMask);
    }
#endif    
    postMonitorQueue();
...
void PlaylistFetcher::onMonitorQueue() {
    ...
    if (mStreamTypeMask == LiveSession::STREAMTYPE_SUBTITLES) {
...
#ifndef ANDROID_DEFAULT_CODE
        if(mPrepareSent &&  mPlaylist != NULL && mPlaylist->isComplete())
           downloadMore = (bufferedDurationUs < kMinBufferedDurationUs * 2);//20s
        else
#endif
...
#ifndef ANDROID_DEFAULT_CODE
            else if(mPacketSources.valueAt(i)->getFormat() == NULL)
            {
                first = false;
                minBufferedDurationUs = 0ll;
                finalResult = OK;
                break;
            }
#endif            
        ...
        if(mPrepareSent &&  mPlaylist != NULL && mPlaylist->isComplete())
          downloadMore =
            !first && (minBufferedDurationUs < kMinBufferedDurationUs * 2);//20s
        else
#endif            
        downloadMore =
...
void PlaylistFetcher::onDownloadNext() {
    ...
    if (mSeqNumber < 0) {
        ...
        if (mPlaylist->isComplete() || mPlaylist->isEvent()) {
...
#ifndef ANDROID_DEFAULT_CODE
            /* Spec. 6.3.3 the client SHOULD NOT choose a segment which
               starts less than three target durations from the end of the Playlist file */
            int32_t targetDurationSecs;
            CHECK(mPlaylist->meta()->findInt32("target-duration", &targetDurationSecs));
            int64_t targetDurationUs = targetDurationSecs * 1000000ll;

            ssize_t n = (ssize_t)(mPlaylist->size());
            mSeqNumber = lastSeqNumberInPlaylist;
            int i = 1;
            sp<AMessage> itemMeta;
            int64_t itemDurationUs;
            int64_t accuItemDurationUs = 0;
            while ((n-i) >= 0)
            {
                CHECK(mPlaylist->itemAt(n - i, NULL /* uri */, &itemMeta));
                CHECK(itemMeta->findInt64("durationUs", &itemDurationUs));

                accuItemDurationUs += itemDurationUs;
                if(accuItemDurationUs > 3*targetDurationUs)
                {
                    mSeqNumber = firstSeqNumberInPlaylist + n - i;
                    break;
                }
                i++;
            }
            if((n-i) < 0)
            {
                // To the 1st ts segment, the accumulated duration is not enough to 3*target duration
                mSeqNumber = firstSeqNumberInPlaylist;
            }
            ALOGD("Start with Seq(%d), targetDuration(%d), Penultimate(%d)", mSeqNumber,
                targetDurationSecs, i);
#else
    ...
    ALOGD("fetching segment %d from (%d .. %d)",
          mSeqNumber, firstSeqNumberInPlaylist, lastSeqNumberInPlaylist);

    ALOGD("fetching '%s'", uri.c_str());

    sp<ABuffer> buffer;
#ifndef ANDROID_DEFAULT_CODE
    int32_t targetDurationSecs;
    mPlaylist->meta()->findInt32("target-duration", &targetDurationSecs);
    int64_t fetchStartTimeUs = ALooper::GetNowUs();    
    status_t err = mSession->fetchFile2(mNotify->dup(),uri.c_str(), &buffer, range_offset, range_length,targetDurationSecs,mSeqNumber);
    int64_t fetchEndTimeUs = ALooper::GetNowUs();
    ALOGI("fetchFile '%s',spend time = %.2f,err:%d",uri.c_str(),(fetchEndTimeUs - fetchStartTimeUs) / 1E6,err);
    if (err != OK) {
         ...
         if(err == ECANCELED) {
             ...
             ALOGD("need switch to lower bandwidth playlist due to bandwidth decrease");
             return;
        } else {
    ...
    if (mPlaylist->isVendorSource() && buffer->size() == 0) {
        ...
        explicitDiscontinuity = true;
        ALOGD("receive discontinuity from vendor source");
    }
#endif
...
#ifndef ANDROID_DEFAULT_CODE
        if(mSeqNumber == 0 && mStartup && explicitDiscontinuity)
        {
            ALOGD("cancel queueDiscontinuity while playlist start with explicitDiscontinuity");
        }
        else if(seekDiscontinuity || explicitDiscontinuity) {
#else            
...
#ifndef ANDROID_DEFAULT_CODE
        if ((timeUs + 500000ll) < segmentStartUs + itemDurationUs) {
            break;
        }
#else        
...
#ifndef ANDROID_DEFAULT_CODE
              sp<AnotherPacketSource> sourceMeta =
              static_cast<AnotherPacketSource *>(mTSParser->getSource(ATSParser::METADATA).get());
              sp<ABuffer> accessUnitMeta;

              if ((sourceMeta != NULL) && sourceMeta->hasBufferAvailable(&finalResult))
              {
                 status_t errMeta = sourceMeta->dequeueAccessUnit(&accessUnitMeta);
                 if (errMeta == OK)
                 {
                    AString mimeType;
                    sp<ABuffer> buffer;

                    if(((accessUnitMeta)->meta()->findString("mime", &mimeType)) &&
                     ((accessUnitMeta)->meta()->findBuffer("pictureBuffer", &buffer)))
                    {
                       sp<AMessage> notify = mNotify->dup();
                       notify->setInt32("what", kWhatPicture);
                       notify->setBuffer("buffer", accessUnitMeta);
                       notify->post();
                    }
                 }
                 return OK;
