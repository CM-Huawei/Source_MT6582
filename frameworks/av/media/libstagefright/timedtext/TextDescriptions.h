...
public:
    enum
    {
        ...
        OUT_OF_BAND_TEXT_ASS          = 0x07,
        OUT_OF_BAND_TEXT_SSA          = 0x08,
        OUT_OF_BAND_TEXT_TXT          = 0x09,
        OUT_OF_BAND_TEXT_MPL          = 0x0A,
        OUT_OF_BAND_TEXT_SMI          = 0x0B,
        OUT_OF_BAND_TEXT_SUB          = 0x0C,
        OUT_OF_BAND_TEXT_IDX          = 0x0D,
#endif
...
private:
    ...
    static status_t extractVOBSUBGlobalDescriptions(
        const uint8_t *data, ssize_t size, Parcel *parcel, int depth) ;
#if 0
    static status_t extractVOBSUBLocalDescriptions(
        ...
        int timeMs, Parcel *parcel, int depth);
#endif
    static status_t extractMPLLocalDescriptions(
        const uint8_t *data, ssize_t size, int timeMs, Parcel *parcel) ;

    static status_t extractSUBLocalDescriptions(
        const uint8_t *data, ssize_t size, int timeMs, Parcel *parcel) ;

    static status_t extractSMILocalDescriptions(
        const uint8_t *data, ssize_t size, int timeMs, Parcel *parcel) ;

    static status_t extractIDXLocalDescriptions(
