    ...
    uint32_t flags, int timeMs, Parcel *parcel)
{
    ...
    case IN_BAND_TEXT_VOBSUB:
        ...
        if (flags & GLOBAL_DESCRIPTIONS)
        {
            ALOGE("[RY] return extractVOBSUBGlobalDescriptions");
            return extractVOBSUBGlobalDescriptions(data, size, parcel, 0);
        }
        break;

    case OUT_OF_BAND_TEXT_SRT:
        if (flags & LOCAL_DESCRIPTIONS)
        {
            return extractSRTLocalDescriptions(data, size, timeMs, parcel);
        }
        break;
    case OUT_OF_BAND_TEXT_MPL:
        if (flags & LOCAL_DESCRIPTIONS)
        {
            return extractMPLLocalDescriptions(data, size, timeMs, parcel);
        }
        break;

    case OUT_OF_BAND_TEXT_SMI:
        if (flags & LOCAL_DESCRIPTIONS)
        {
            return extractSMILocalDescriptions(data, size, timeMs, parcel);
        }
    case OUT_OF_BAND_TEXT_SUB:
        if (flags & LOCAL_DESCRIPTIONS)
        {
            return extractSUBLocalDescriptions(data, size, timeMs, parcel);
        }
        break;
    }
#else
    ...
    uint32_t flags, int timeMs, Parcel *parcel)
{
    ...
    switch (SP_Type)
    {
    case OUT_OF_BAND_TEXT_IDX:
        if (flags & LOCAL_DESCRIPTIONS)
        {
            return extractIDXLocalDescriptions(fd, width, height, timeMs, parcel);
        }
        break;
    case IN_BAND_TEXT_VOBSUB:
        if (flags & LOCAL_DESCRIPTIONS)
...
status_t TextDescriptions::extractVOBSUBGlobalDescriptions(
    const uint8_t *data, ssize_t size, Parcel *parcel, int depth)
{
    /* parcel->writeInt32(KEY_STRUCT_TEXT);
     // write the size of the text sample
...
status_t TextDescriptions::extractMPLLocalDescriptions(
    const uint8_t *data, ssize_t size, int timeMs, Parcel *parcel)
{
    parcel->writeInt32(KEY_LOCAL_SETTING);
    parcel->writeInt32(KEY_START_TIME);
...
status_t TextDescriptions::extractSMILocalDescriptions(
    const uint8_t *data, ssize_t size, int timeMs, Parcel *parcel)
{
    parcel->writeInt32(KEY_LOCAL_SETTING);
    parcel->writeInt32(KEY_START_TIME);
...
status_t TextDescriptions::extractSUBLocalDescriptions(
    const uint8_t *data, ssize_t size, int timeMs, Parcel *parcel)
{
    parcel->writeInt32(KEY_LOCAL_SETTING);
    parcel->writeInt32(KEY_START_TIME);
...
status_t TextDescriptions::extractIDXLocalDescriptions(
