...
void TimedTextPlayer::start() {
    ALOGE("[PANDA]  TimedTextPlayer::start");
    (new AMessage(kWhatStart, id()))->post();
}

void TimedTextPlayer::pause() {
    ALOGE("[PANDA]  TimedTextPlayer::pause");
    (new AMessage(kWhatPause, id()))->post();
}

void TimedTextPlayer::resume() {
    ALOGE("[PANDA]  TimedTextPlayer::resume");
    (new AMessage(kWhatResume, id()))->post();
}

void TimedTextPlayer::seekToAsync(int64_t timeUs) {
    ALOGE("[PANDA]  TimedTextPlayer::seekToAsync");
...
void TimedTextPlayer::setDataSource(sp<TimedTextSource> source) {
    ALOGE("[PANDA]  TimedTextPlayer::setDataSource");
...
#ifndef ANDROID_DEFAULT_CODE
            ALOGI("seektimeUs:%lld Us", seekTimeUs);
#endif
