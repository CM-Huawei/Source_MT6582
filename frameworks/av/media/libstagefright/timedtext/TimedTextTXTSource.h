...
#ifndef TIMED_TEXT_TXT_SOURCE_H_
#define TIMED_TEXT_TXT_SOURCE_H_
...
class TimedTextTXTSource : public TimedTextSource {
public:
    TimedTextTXTSource(const sp<MediaSource>& mediaSource);	//For internal SP
    TimedTextTXTSource(const sp<DataSource>& dataSource);		//For external SP
...
private:
	...
	void adjustPreSubEndTime(int64_t curStartTimeUs, MultiTextInfo info);
    ...
    DISALLOW_EVIL_CONSTRUCTORS(TimedTextTXTSource);
};

};

#endif  // TIMED_TEXT_TXT_SOURCE_H_
#endif
