...
void CameraSource::dataCallbackTimestamp(int64_t timestampUs,
    ...
    Mutex::Autolock autoLock(mLock);
#ifndef ANDROID_DEFAULT_CODE
    if (!mStarted || (mNumFramesReceived == 0 && timestampUs < mStartTimeUs) || mPaused) {
    ...
    if (mNumFramesReceived == 0) {
        ...
        if (mStartTimeUs > 0) {
            if (timestampUs < mStartTimeUs) {
                ...
                releaseOneRecordingFrame(data);
#ifndef ANDROID_DEFAULT_CODE 
				ALOGW("timestampUs=%lld < mStartTimeUs=%lld drop frame",timestampUs,mStartTimeUs);
#endif
                return;
            }
            mStartTimeUs = timestampUs - mStartTimeUs;
#ifndef ANDROID_DEFAULT_CODE 
			ALOGI("the first video frame,time offset to mStartTimeUs=%lld",mStartTimeUs);
#endif
        }
    }
    ++mNumFramesReceived;
