...
/*debug
    ...
    if( (OMX_ACODEC_COLOR_CONVERT & pBuffer->nFlags) == OMX_ACODEC_COLOR_CONVERT )
    {
        OMX_MTK_PLATFORM_PRIVATE *ptrPlatformPrivate = (OMX_MTK_PLATFORM_PRIVATE *)pBuffer->pPlatformPrivate;
        ALOGV("mM4UMVABufferPa %p", ptrPlatformPrivate->mM4UMVABufferPa);
    }
#endif //ANDROID_DEFAULT_CODE
*/
    findDispatcher(node)->post(msg);
