...
#ifndef ANDROID_DEFAULT_CODE
        ssize_t sz = gFileSourceProxy.read(mFd, offset + mOffset, data, size);
        if (sz >= 0) {
			return sz;
        }
		else {
...
sp<DecryptHandle> FileSource::DrmInitialization(const char *mime) {
	...
	memset(buffer, 0, 256);
	memset(linkto, 0, 256);
	sprintf(buffer, "/proc/%d/fd/%d", gettid(), mFd);
	int len = 0;
	...
	ALOGI("zxy link:%s", linkto);
	mDecryptHandle = mDrmManagerClient->openDecryptSession(linkto, mime);
#endif
    }
