   ...
   if (mPause) {
    ...
    while ((size_remaining > 0)&&(err==M4NO_ERROR)) {
        ...
        if (mInputBuffer == NULL) {
			...
			ALOGD("mPause =%d, size_remaining=%d, mBGAudioStoryBoardSkimTimeStamp=%lld,mPositionTimeMediaUs=%lld,mBGAudioStoryBoardCurrentMediaBeginCutTS=%lld,mSeekTimeUs=%lld,mAudioMixSettings->uiAddCts=%d",
			mPause, size_remaining, mBGAudioStoryBoardSkimTimeStamp,mPositionTimeMediaUs,mBGAudioStoryBoardCurrentMediaBeginCutTS,mSeekTimeUs,mAudioMixSettings->uiAddCts);

#ifndef ANDROID_DEFAULT_CODE
                    if (((mBGAudioStoryBoardSkimTimeStamp * 1000) +
                          (mPositionTimeMediaUs - ((mBGAudioStoryBoardSkimTimeStamp + mBGAudioStoryBoardCurrentMediaBeginCutTS)/100)*100000)) >=					
#else
			if (((mBGAudioStoryBoardSkimTimeStamp* 1000) +
...
#ifndef ANDROID_DEFAULT_CODE
                        if (mInputBuffer->meta_data() == NULL)
                        {
...
#ifndef ANDROID_DEFAULT_CODE							
                            bgFrame.m_dataAddress = NULL;
#endif
                        }
                        if (mixFrame.m_dataAddress) {
                            free(mixFrame.m_dataAddress);
#ifndef ANDROID_DEFAULT_CODE													
                            mixFrame.m_dataAddress = NULL;	
#endif
                        }
                    } else {
                        // No mixing;
