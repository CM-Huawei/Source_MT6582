...
/*** DEFINITIONS ***/
#ifdef VE_OMX_ENC
#define VIDEOEDITOR_FORCECODEC kAudUseOMXForVE
#else
...
#ifdef VE_OMX_ENC
	if(pEncoderContext->mEncoder != NULL){
...
M4OSA_ERR VideoEditorAudioEncoder_open(M4OSA_Context pContext,
...
#ifdef VE_OMX_ENC
	if(NULL == pEncoderContext->mEncoder.get()){
		ALOGW("Audio encoder create unsuccess!");
	}
#endif
	...
	metaPara->setInt32(kKeyInputBufferNum, 1);
	result = pEncoderContext->mEncoder->start(metaPara.get());
#else
    ...
    if( M4ENCODER_kAAC == pEncoderContext->mCodecParams->Format ) {
		...
		int32_t nbBuffer = pEncoderContext->mEncoderSource->storeBuffer(inputBuffer);
#endif
