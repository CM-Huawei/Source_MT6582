...
M4OSA_ERR VideoEditor3gpReader_open(M4OSA_Context pContext,
    ...
    if (pC->mDataSource == NULL) {
        ALOGW("VideoEditor3gpReader_open Datasource error");
    ...
    if (pC->mExtractor == NULL) {
        ALOGW("VideoEditor3gpReader_open extractor error");
        return M4ERR_PARAMETER;
    }
    ...
    if (isDRMProtected) {
        ALOGW("VideoEditorMp3Reader_open error - DRM Protected");
        return M4ERR_UNSUPPORTED_MEDIA_TYPE;
    }
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGV("VideoEditor3gpReader_jump time us %lld ", time64);
#endif //ANDROID_DEFAULT_CODE
...
#ifndef ANDROID_DEFAULT_CODE
        ALOGV("VideoEditor3gpReader_jump AUDIO time us %lld ", time64);
#endif //ANDROID_DEFAULT_CODE
...
#ifndef ANDROID_DEFAULT_CODE
        ALOGV("VideoEditor3gpReader_jump VIDEO time us %lld ", time64);
#endif //ANDROID_DEFAULT_CODE
    } else {
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGV("VideoEditor3gpReader_jump time ms before seekset %lld ", time64);
#endif //ANDROID_DEFAULT_CODE
...
M4OSA_ERR VideoEditor3gpReader_getNextAu(M4OSA_Context context,
    ...
    M4OSA_DEBUG_IF1((context == 0), M4ERR_PARAMETER,
...
#ifndef ANDROID_DEFAULT_CODE
    Mutex::Autolock autolock(pC->mLock);

    ALOGV("VideoEditor3gpReader_getNextAu locked");
#endif //ANDROID_DEFAULT_CODE
    if (pStreamHandler == (M4_StreamHandler*)pC->mAudioStreamHandler) {
        ALOGV("VideoEditor3gpReader_getNextAu pC->mAudioSource %p, pC->mAudioAu %x", pC->mAudioSource.get(), &pC->mAudioAu);
#ifndef ANDROID_DEFAULT_CODE
        if( NULL == pC->mAudioSource.get() )
        {
            ALOGW("VideoEditor3gpReader_getNextAu pC->mAudioSource NULL");
        ...
        if (pC->mAudioSeeking == M4OSA_TRUE) {
            ALOGV("VideoEditor3gpReader_getNextAu audio seek time: %lld",
                pC->mAudioSeekTime);
            options.setSeekTo(pC->mAudioSeekTime);
            error = pC->mAudioSource->read(&mMediaBuffer, &options);
#ifndef ANDROID_DEFAULT_CODE
            ALOGV("VE3gpReader_getNextAu mAudioSource MediaBuffer %x , error %d", mMediaBuffer, error);
#endif //ANDROID_DEFAULT_CODE
            if (mMediaBuffer != NULL) {
                mMediaBuffer->meta_data()->findInt64(kKeyTime,
                    (int64_t*)&tempTime64);
#ifndef ANDROID_DEFAULT_CODE
                ALOGV("VideoEditor3gpReader_getNextAu seek tempTime64 =%lld, kKeyTime=%d , pC->mAudioSeekTime=%lld, range_length=%d",
                    tempTime64 , kKeyTime,pC->mAudioSeekTime, mMediaBuffer->range_length());
#endif //ANDROID_DEFAULT_CODE
            }
#ifdef ANDROID_DEFAULT_CODE
            ALOGV("VideoEditor3gpReader_getNextAu seek tempTime64 =%lld, kKeyTime=%d , pC->mAudioSeekTime=%lld, range_length=%d",
        ...
        ALOGV("VideoEditor3gpReader_getNextAu pC->mVideoSource %p, pC->mVideoAu %x", pC->mVideoSource.get(), &pC->mVideoAu);

#ifndef ANDROID_DEFAULT_CODE
        if( NULL == pC->mVideoSource.get() )
        {
            ALOGW("VideoEditor3gpReader_getNextAu pC->mVideoSource NULL");
            err = M4ERR_BAD_CONTEXT;
            goto exit_here;
        }
#endif //ANDROID_DEFAULT_CODE
        ...
        if(pC->mVideoSeeking == M4OSA_TRUE) {
            flag = M4OSA_TRUE;
            ALOGV("VideoEditor3gpReader_getNextAu seek: %lld",pC->mVideoSeekTime);
            options.setSeekTo(pC->mVideoSeekTime, MediaSource::ReadOptions::SEEK_PREVIOUS_SYNC);
            do
            {
                if (mMediaBuffer != NULL) {
                ...
                ALOGV("VE3gpReader_getNextAu mVideoSource MediaBuffer %x , error %d", mMediaBuffer, error);
                if (mMediaBuffer != NULL)
                {
                    if (mMediaBuffer->meta_data()->findInt32(kKeyIsSyncFrame, &i32Tmp) && i32Tmp) {
    ...
    if (mMediaBuffer != NULL) {
        if( (pAu->dataAddress == NULL) ||  (pAu->size < \
            ...
            if(pAu->dataAddress != NULL) {
                //ALOGD("size = %d ,range_length = %d", pAu->size, mMediaBuffer->range_length() );
                free((M4OSA_Int32*)pAu->dataAddress);
                pAu->dataAddress = NULL;
            }
            ALOGV("Buffer lenght = %d ,%d",(mMediaBuffer->range_length() + 3) & ~0x3,(mMediaBuffer->range_length()));

#ifndef ANDROID_DEFAULT_CODE
            /*
             ALPS00812279 vend_ck.wang @20130619
             If video file is corrupted, the returned mediabuffer may have zero length.
             */
            if (0 == mMediaBuffer->range_length())
            {
                ALOGD("Video file might be corrupted.");
                mMediaBuffer->release();
                mMediaBuffer = NULL;
        ...
        ALOGV("VideoEditor3gpReader_getNextAu CTS = %lld, range_length %d, %p",pAu->CTS, mMediaBuffer->range_length(), mMediaBuffer->meta_data().get());
...
#ifndef ANDROID_DEFAULT_CODE	
       if (pAccessUnit->m_dataAddress_preallocated != NULL)
       {
 //          memcpy((void *)pAccessUnit->m_dataAddress_preallocated,
 //                        (void *)pAccessUnit->m_dataAddress, pAccessUnit->m_size);
//           ALOGD("memcpy 3gp preallocated");
       }
#endif
        ...
        ALOGI("VideoEditor3gpReader_getNextAu: M4WAR_NO_MORE_AU (EOS) reached");
        pAccessUnit->m_size = 0;
...
#endif
    if(temp >= trackCount) {
        ALOGW("VideoEditor3gpReader_getNextStreamHandler error = %x",
                    ...
                    if (meta->findData(kKeyESDS, &type, &data, &size)) {
                        ESDS esds((const char *)data, size);
                            ...
                            if (OK != esds.InitCheck()) {
                            	ALOGE ("[ERROR] esds init check failed!");
                            	(*pStreamHandler)->m_pDecoderSpecificInfo = M4OSA_NULL;
                            	(*pStreamHandler)->m_ESDSInfoSize = 0;
                                (*pStreamHandler)->m_pESDSInfo = M4OSA_NULL;
                                (*pStreamHandler)->m_pH264DecoderSpecificInfo = M4OSA_NULL;
...
#endif
		{
            ALOGD("VideoEditor3gpReader_getNextStream audio getTrack called %s", mime);
            pC->mAudioSource = pC->mExtractor->getTrack(pC->mCurrTrack);
            ALOGD("VideoEditor3gpReader_getNextStream pC->mAudioSource %p", pC->mAudioSource.get());
...
Error:
    ALOGE("VideoEditor3gpReader_getNextStreamHandler end error = %x",err);
    return err;
}
    ...
    M4_StreamHandler *pStreamHandler, M4OSA_Int32* pTime)
{
    ...
    mMediaBuffer->meta_data()->findInt64(kKeyTime, (int64_t*)&tempTime64);
    ALOGV("VideoEditor3gpReader_getPrevRapTime read time %lld, %x", tempTime64,
        mMediaBuffer);
