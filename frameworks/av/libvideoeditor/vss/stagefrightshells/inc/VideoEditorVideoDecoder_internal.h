...
#define VIDEOEDITOR_VIDEC_SHELL_MAX_MVA_BUFFER_CNT     0x30
typedef struct MTK_VEPLATFORM_PRIVATE {
    unsigned long mM4UBufferPaB;
    unsigned long mM4UBufferVaA;
    //add next private members
} MTK_VEPLATFORM_PRIVATE;
typedef struct MTK_VDEC_ACODEC_MVA_ADDR_MAPPING {
    //OMX_U32 mM4UBufferVaA[VIDEOEDITOR_VIDEC_SHELL_MAX_MVA_BUFFER_CNT];
    //OMX_U32 mM4UBufferPaB[VIDEOEDITOR_VIDEC_SHELL_MAX_MVA_BUFFER_CNT];
    MTK_VEPLATFORM_PRIVATE mM4UBuffers[VIDEOEDITOR_VIDEC_SHELL_MAX_MVA_BUFFER_CNT];

    OMX_U32 mM4UBufferCount;
    OMX_U32 mInputBufferPopulatedCnt;
    OMX_U32 mOutputBufferPopulatedCnt;
    OMX_U32 ownNum;
    void*  mkM4UBufferHandle;
    OMX_U32 mM4UBufferSize;
...
typedef struct {
    ...
    MTK_VDEC_ACODEC_MVA_ADDR_MAPPING mMVAParamSrc;
    M4OSA_Bool              mAllocatedMVABUffer;

#ifdef USE_VIDEO_ION
    //for color convert
    int mIonDevFd;
    VideoIonBufferInfo mIonBufInfo[MAX_ION_BUF_COUNT];
#endif //USE_VIDEO_ION
#endif //ANDROID_DEFAULT_CODE
