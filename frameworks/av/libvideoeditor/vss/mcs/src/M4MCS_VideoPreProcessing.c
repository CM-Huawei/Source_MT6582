...
#include <utils/Log.h>
#define LOG_TAG "M4MCS_VideoPreProcessing"
#endif


#ifndef ANDROID_DEFAULT_CODE
extern M4OSA_ERR M4VSS3GPP_intRotateVideo(M4VIFI_ImagePlane* pPlaneIn,
        M4OSA_UInt32 rotationDegree);
extern M4OSA_ERR M4VSS3GPP_intSetYUV420Plane(M4VIFI_ImagePlane* planeIn,
...
/* This part is used only if video codecs are compiled*/
    ...
    if(pC->isRenderDup == M4OSA_FALSE)
    {
        ...
        if (M4OSA_NULL != pC->pPreResizeFrame)
        {
...
#ifndef ANDROID_DEFAULT_CODE
            M4OSA_UInt32 yuvFrameWidth = pC->pPreResizeFrame[0].u_width;
            M4OSA_UInt32 yuvFrameHeight = pC->pPreResizeFrame[0].u_height;
            if (pC->InputFileProperties.videoRotationDegrees != 0) {
                ALOGV("do rotation %ld", pC->InputFileProperties.videoRotationDegrees);
                err = M4VSS3GPP_intRotateVideo(pC->pPreResizeFrame, pC->InputFileProperties.videoRotationDegrees);
                if (M4NO_ERROR != err) {
                    M4OSA_TRACE1_1("M4MCS_intApplyVPP: M4VSS3GPP_intRotateVideo returns 0x%x!", err);
                    return err;
                }
            }
#endif
                ...
                if(pC->MediaRendering == M4MCS_kBlackBorders)
                {
                    for(i=0; i<pPlaneOut[0].u_height; i++)
                    {
                        memcpy(   (void *)pOutPlaneY,
                                        ...
                                        pPlaneOut[0].u_width);
#ifndef ANDROID_DEFAULT_CODE
                        pInPlaneY += pPlaneOut[0].u_stride;
#else
                    ...
                    for(i=0; i<pPlaneOut[1].u_height; i++)
                    {
                        memcpy(   (void *)pOutPlaneU,
                                        ...
                                        pPlaneOut[1].u_width);
#ifndef ANDROID_DEFAULT_CODE
                        pInPlaneU += pPlaneOut[1].u_stride;
#else
                    ...
                    for(i=0; i<pPlaneOut[2].u_height; i++)
                    {
                        memcpy(   (void *)pOutPlaneV,
                                        ...
                                        pPlaneOut[2].u_width);
#ifndef ANDROID_DEFAULT_CODE
                        pInPlaneV += pPlaneOut[2].u_stride;
#else
            ...
            if (0 != pC->InputFileProperties.videoRotationDegrees &&
                    180 != pC->InputFileProperties.videoRotationDegrees) {
                M4VSS3GPP_intSetYUV420Plane(pC->pPreResizeFrame,
