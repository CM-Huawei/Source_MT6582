...
#ifndef ANDROID_DEFAULT_CODE // Pass the check for some reserved profile_and_level indication in Table G-1, ALPS00317802 (the indication is useless in VE)
    if (i == length) {
    	ALOGD("OOps, unrecognized profileAndLevel(%d), bypass it", profileAndLevel);
        *pProfile = OMX_VIDEO_MPEG4ProfileSimple;
        *pLevel = OMX_VIDEO_MPEG4Level1;
    }
#endif
    return M4NO_ERROR;
}
