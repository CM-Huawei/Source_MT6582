    ...
    M4VSS3GPP_InternalAudioMixingContext *pC )
{
        ...
        pC->ChannelConversion > 0)
    {
        ...
        * Prepare output buffer */
        ...
        M4OSA_TRACE2_0("[AudioMixingCopyAdded]K **** blend AUs");
        /**
    ...
    M4VSS3GPP_InternalAudioMixingContext *pC )
{
...
#ifdef DUMPMIXINGPCM
	FILE *fp = fopen("/sdcard/audiodec_mix_output.pcm", "ab");

	if(fp)
	{
		fwrite(pEncInBuffer.pTableBuffer[0], 1, pEncInBuffer.pTableBufferSize[0], fp);
		fclose(fp);
	}
    ...
    * Prepare output buffer */
    ...
    M4OSA_TRACE2_0("[intAudioMixingDoMixing]K **** blend AUs");
            ...
            * Decode/encode up to the wanted position */
            while( pC->pInputClipCtxt->iAudioFrameCts < iCurrentCts )
            {
...
#ifdef MIX_TRANS_DUMP_PCM_TO_FILE
				
				FILE *fp = fopen("sdcard/mix_transInput.pcm", "ab");
                ...
                * Encode the PCM audio */
				...
				FILE *fp1 = fopen("sdcard/mix_transoutput.pcm", "ab");
