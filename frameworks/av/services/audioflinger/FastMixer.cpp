...
bool FastMixer::threadLoop()
{
    ...
    for (;;) {
        ...
        audio_devices_t OutputDevice = next->mOutputDevice;
#endif
        if (next != current) {
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
                    mixer = new AudioMTKMixer(frameCount, sampleRate);                    
                ...
                while (addedTracks != 0) {
                    ...
                    if (mixer != NULL) {
                        ...
                        mixer->setParameter(
                                    name,
                                    AudioMixer::TRACK,
                                    AudioMixer::FORMAT, (void *)fastTrack->mFormat);
                            
                        ALOGD("set resampler");
                        mixer->setParameter(name, AudioMixer::RESAMPLE,
                                AudioMixer::SAMPLE_RATE, (void*) fastTrack->mSampleRate);
                            
                        // no DRC and DC removal for fasttrack[0]
                        mixer->setParameter(
                                    name,
                                    AudioMTKMixer::TRACK,
                                    AudioMTKMixer::DO_POST_PROC, (void *) IsMixerDoPostProc);
                        ...
                        mixer->setParameter(
                            name,
                            AudioMTKMixer::TRACK,
                            AudioMTKMixer::STREAM_TYPE, (void *)fastTrack->mStreamType);
                        mixer->setParameter(
                            name,
                            AudioMTKMixer::DRC,
                            AudioMTKMixer::DEVICE, (void *)OutputDevice);
     #endif
#else
        ...
        if (rc == 0) {
            ...
            if (oldTsValid) {
                ...
                if (isWarm) {
                    ...
                    sleepNs = -1;   //overwrite the sleep time, 
#endif
                    } else {
                        ignoreNextOverrun = false;
                    }
...
status_t FastMixer::readyToRun()
{
    ...
    sched_p.sched_priority = RTPM_PRIO_FASTMIXER_THREAD;
        if(0 != sched_setscheduler(0, SCHED_RR, &sched_p)) {
            ALOGD("pid [%d] sched_setscheduler to default: %d", getpid(), errno);
        }
        else {
            sched_p.sched_priority = RTPM_PRIO_FASTMIXER_THREAD;
            sched_getparam(0, &sched_p);
            ALOGD("sched_setscheduler to rt, priority: %d", sched_p.sched_priority);
        }
    }
    return NO_ERROR;
}
