...
public final class InboundSmsTracker {
    ...
    public InboundSmsTracker(byte[] pdu, long timestamp, int destPort, boolean is3gpp2,
            String address, int referenceNumber, int sequenceNumber, int messageCount,
            boolean is3gpp2WapPdu, int simId) {
        this(pdu, timestamp, destPort, is3gpp2, address, referenceNumber, sequenceNumber, messageCount, is3gpp2WapPdu);
        mSimId = simId;
        mRecvTime = System.currentTimeMillis();
        mUploadFlag = IConcatenatedSmsFwkExt.UPLOAD_FLAG_NEW;
    }
    // MTK-END
