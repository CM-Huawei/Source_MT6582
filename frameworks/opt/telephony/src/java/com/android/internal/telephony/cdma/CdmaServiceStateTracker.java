...
//support by via start,according to MTK'S DESIGNING
...
public class CdmaServiceStateTracker extends ServiceStateTracker {
    ...
    //add by VIA for network info[begin][ALPS00384791]
    private String mSid;
    private String mNid;
    //add by VIA for network info[end][ALPS00384791]
   ...
   // add for time problem by via
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            //just for MTK,because sim id = Phone.GEMINI_SIM_1, so do nothing here;
            else if (intent.getAction().equals("com.mtk.TEST_TRM")){ //ALPS00242220 by mtk
    ...
    private ContentObserver mAutoTimeObserver = new ContentObserver(new Handler()) {
        ...
        public void onChange(boolean selfChange) {
            ...
            /*Changed by via for syn net work time*/
    ...
    protected CdmaServiceStateTracker(CDMAPhone phone, CellInfo cellInfo) {
        super(phone, phone.mCi, cellInfo, phone.getMySimId());
    ...
    public void handleMessage (Message msg) {
        ...
        case EVENT_RUIM_READY:
            ...
            if (DBG) log("via EVENT_RUIM_READY isSubscriptionFromRuim" + mIsSubscriptionFromRuim);
            //via-support start,modified for ALPS00395417
            ...
            mPhone.prepareEri();
            Rlog.d(LOG_TAG, "after pin unlock get NitzTime!!!!");
            // via support to add this API invocation
            queryCurrentNitzTime();
            //via-support end
        ...
        case EVENT_NV_READY:
            ...
            // subscription info.
            if (DBG) log("via EVENT_NV_READY isSubscriptionFromRuim" + mIsSubscriptionFromRuim);
            //via-support start,modified for ALPS00395417
       ...
       case EVENT_GET_NITZ_TIME:
            ...
            if (!(mCi.getRadioState().isOn()) || (mCi.getRadioState().isGsm()) || networkType==0) {
                Rlog.d(LOG_TAG, "EVENT_GET_NITZ_TIME ignore: isOn()=" +mCi.getRadioState().isOn() +
                            ", isGsm()="+mCi.getRadioState().isGsm() +
                            ", mRegistrationState="+mRegistrationState +
            ...
            Rlog.d(LOG_TAG, "EVENT_GET_NITZ_TIME  nitzStringSolicite = " + nitzStringSolicite + "nitzReceiveTimeSolicite = " + nitzReceiveTimeSolicite);
       ...
       // via support end
        ...
        case EVENT_RUIM_RECORDS_LOADED:
            Rlog.d(LOG_TAG, "EVENT_RUIM_RECORDS_LOADED get NitzTime!!!!");
            // via support to add this API invocation
            queryCurrentNitzTime();
            updatePhoneObject();
            updateSpnDisplay();
        ...
        case EVENT_NETWORK_TYPE_CHANGED:
            if (DBG) log("handleMessage (EVENT_NETWORK_TYPE_CHANGED)");
            ar = (AsyncResult)msg.obj;
            if(ar.exception == null){
                ints = (int[]) ar.result;
                //0: no network, 2:1x only mode, 4: Do only mode,8:1x/Do mode
                mCdmaNetWorkMode = ints[0];
                if (DBG) log(" mCdmaNetWorkMode = "  + mCdmaNetWorkMode);
                if(mCdmaNetWorkMode == ServiceState.RIL_CDMA_NETWORK_MODE_EVDO_ONLY) {
                   if (DBG) log("ServiceState.RIL_CDMA_NETWORK_MODE_EVDO_ONLY pollState ");
                   pollState();
                }
            }
            break;
    ...
    protected void setPowerStateToDesired() {
        //Modify by via begin [ALPS00374383]
        log("setPowerStateToDesired mDesiredPowerState:" + mDesiredPowerState +
            " current radio state:" + mCi.getRadioState() + ", mFirstRadioChange = " + mFirstRadioChange);

        // If we want it on and it's off, turn it on
        if (mDesiredPowerState
            && mCi.getRadioState() == CommandsInterface.RadioState.RADIO_OFF) {
            if (!FeatureOption.MTK_GEMINI_SUPPORT) {
                log("setPowerStateToDesired cm.setRadioPower(true, null)");
        ...
        //Modify by via end [ALPS00374383]
    }
    ...
    private boolean isSimInserted(int slotId) {
        ...
        if (phone != null) {
            //try {
                simInserted = true;//phone.isSimInsert(slotId);
            //} catch (RemoteException e) {
            //    e.printStackTrace();
            //}
        }
        Rlog.d(LOG_TAG, "isSimInserted(" + slotId + "), SimInserted=" + simInserted);
        return simInserted;
    }
    //VIA-START SET ETS
    ...
    protected void updateSpnDisplay(boolean forceUpdate) {
        ...
        if (plmn == null || plmn.equals("")) {
           ...
           if(plmn != null) {
               showPlmn = true;
               //add by via begin [ALPS00374383]
               if(plmn.equals("")) {
                    Rlog.d(LOG_TAG, "add by via"); 
                    plmn = null;
               }
               //add by via end [ALPS00374383]
           }
        }
    ...
    */
    protected void handlePollStateResultMessage(int what, AsyncResult ar){
        ...
        switch (what) {
            ...
            case EVENT_POLL_STATE_REGISTRATION_CDMA: // Handle RIL_REQUEST_REGISTRATION_STATE.
                ...
                //via support start
                if (DBG) log("newSS.setRegState(registrationState);");
                mNewSS.setRegState(registrationState);
    
                //via support[ALPS00421033]
                if (DBG) log("newSS.setCdmaNetworkMode;");
                ...
                //via support start
                if (DBG) log("setSystemProperty(cdma.operator.sid) = " + Integer.toString(systemId));
                SystemProperties.set(TelephonyPlusCode.PROPERTY_OPERATOR_SID, Integer.toString(systemId));
                //via support end
                ...
                if (opNames != null && opNames.length >= 3) {
                    ...
                    if (DBG) {
                        log("RIL_REQUEST_OPERATOR.opNames[0], " + opNames[0] + 
                            ", opNames[1] = '" + opNames[1] + ", opNames[2] = " + opNames[2]);
                    }

                    //move this from ril.java lookupOperatorName(). for MTK'S reason.--20130715
                    // via support start
                    ...
                    if(numeric.startsWith("2134") && numeric.length() == 7){
                       ...
                       opNames[0] = mCi.lookupOperatorName(numeric, true);
                       opNames[1] = mCi.lookupOperatorName(numeric, false);
                    }
                    // via support end
    
                    //via-support start
                    //if (!isSubscriptionFromRuim) {
                    if (mCi.getNvState().isNVReady()) {
                    //via-support end
                    ...
                    //via support start
                    if((opNames[2] != null) && opNames[2].length() >= 5) {
                        String mcc_ncc = opNames[2];
    
                        //via support start
                        if(opNames[2].length() == 7) {
                        if (DBG) log("for MCC = (1111), setSystemProperty(cdma.operator.mcc) = " + mcc_ncc.substring(0,4));
                            SystemProperties.set(TelephonyPlusCode.PROPERTY_OPERATOR_MCC, mcc_ncc.substring(0,4));
                        } else {
                            if (DBG) log("setSystemProperty(cdma.operator.mcc) = " + mcc_ncc.substring(0,3));
                            SystemProperties.set(TelephonyPlusCode.PROPERTY_OPERATOR_MCC, mcc_ncc.substring(0,3));
                        }
                        //via support end
    
                        //via support begin [ALPS00430267]
                        if (!mcc_ncc.equals(mSS.getOperatorNumeric())) {
                            if (DBG) log("broadcast " + TelephonyIntents.ACTION_MCC_MNC_CHANGED + ":" + mcc_ncc);
                            Intent intent = new Intent(TelephonyIntents.ACTION_MCC_MNC_CHANGED);
                            intent.addFlags(Intent.FLAG_RECEIVER_REPLACE_PENDING);
                            intent.putExtra(TelephonyIntents.EXTRA_MCC_MNC_CHANGED_MCC, mcc_ncc.substring(0,3));
                            intent.putExtra(TelephonyIntents.EXTRA_MCC_MNC_CHANGED_MNC, mcc_ncc.substring(3,mcc_ncc.length()));
                            mPhone.getContext().sendStickyBroadcast(intent);
                        }
                        //via support end [ALPS00430267]
                    }
                    //via support end
                } else {
    ...
    protected void pollStateDone() {
        ...
        if (hasRilDataRadioTechnologyChanged) {
            // via support:query network time if Network Type is changed to a valid state
            if (newNetworkType!=0) queryCurrentNitzTime();
            if (PhoneConstants.GEMINI_SIM_1 == mPhone.getMySimId()) {
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_DATA_NETWORK_TYPE,
                    ServiceState.rilRadioTechnologyToString(mSS.getRilDataRadioTechnology()));
            } else {
                mPhone.setSystemProperty(TelephonyProperties.PROPERTY_DATA_NETWORK_TYPE_2,
                ServiceState.rilRadioTechnologyToString(mSS.getRilDataRadioTechnology()));
            }
        }
        ...
        if (hasChanged) {
            if ((mCi.getRadioState().isOn()) && (!mIsSubscriptionFromRuim)) {
                log("via pollStateDone isSubscriptionFromRuim = " + mIsSubscriptionFromRuim);
                String eriText;
            ...
            if (PhoneConstants.GEMINI_SIM_1 == mPhone.getMySimId()) {
                ...
                operatorNumeric = mSS.getOperatorNumeric();
                mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_NUMERIC_2, operatorNumeric);
            ...
            if (operatorNumeric == null) {
                ...
                // via modified.
                if(PhoneConstants.GEMINI_SIM_1 == mPhone.getMySimId()) {
                    mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY, isoCountryCode);
                } else {
                    mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY_2, isoCountryCode);
                }
            ...
            if (hasRegStateChanged) {
                if (mSS.getRegState() == ServiceState.REGISTRATION_STATE_UNKNOWN
                    && (1 == Settings.System.getInt(mPhone.getContext().getContentResolver(), Settings.System.AIRPLANE_MODE_ON, -1))) {
   ...
   /*Poll NitzTime by via*/
    private void
    queryCurrentNitzTime() {
         if (mCi.getRadioState().isGsm()) {
            return;
        }
    ...
    void setTimeFromNITZString (String nitz, long nitzReceiveTime)
    {
        ...
        try {
            ...
            if (DBG) log("setSystemProperty(cdma.operator.ltmoffset) = " + ltmoffset);
            SystemProperties.set(TelephonyPlusCode.PROPERTY_TIME_LTMOFFSET, Integer.toString(ltmoffset));
            //via support end
            ...
            if(mPhone.getMySimId() == PhoneConstants.GEMINI_SIM_1) {
                ...
                log("[NITZ] mSimId = Phone.GEMINI_SIM_2");
                iso = SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY_2);
            }

            log("[NITZ] setTimeFromNITZString, iso = " + iso + ", mGotCountryCode = " + mGotCountryCode);
            //via added.
            ...
            try {
                ...
                // Modify for time problem by via [ALPS00395281]
                ...
                // Modify for time problem by via [ALPS00395281]
                    ...
                    //via support start
                    //Use updateZone with the check condition when occur change time zone only.
                    boolean updateZone = false;

                    //via added
                    log("[NITZ] setTimeFromNITZString, OldTimeZone = " + mSavedTimeZone);
                    log("[NITZ] setTimeFromNITZString, NewTimeZone = " + (zone!=null ? zone.getID() : "NULL"));
                    ...
                    // via support to modify the check condition 
                    if ((mSavedAtTime == 0) || (timeSinceLastUpdate > nitzUpdateSpacing)
                            || (Math.abs(gained) > nitzUpdateDiff)|| mAutoTimeChanged || updateZone) {
                        if (DBG) {
                    ...
                    //via support end
                        ...
                        //support by via start
                        if (zone != null) {
                        //modify by VIA fix CRTS#19675 [ALPS00416347]
                            if (getAutoTimeZone()) {
                                setAndBroadcastNetworkSetTimeZone(zone.getID());
                            }
                            saveNitzTimeZone(zone.getID());
                        }
                        log("NITZ:before ===== modify zone c.getTimeInMillis() = " + c.getTimeInMillis());	
                        c.add(Calendar.MILLISECOND, (int)(-1)*tzOffset);
                        //support by via end
                        log("NITZ: (int)(-1)*tzOffset = " + (int)(-1)*tzOffset + ", dst = "  + dst);
                        setAndBroadcastNetworkSetTime(c.getTimeInMillis());
                        log("NITZ:after ====== modify zone c.getTimeInMillis() = " + c.getTimeInMillis());	
                    } else {
                        if (DBG) {
                ...
                mWakeLock.release();
                mAutoTimeChanged = false;//Added by via zyl
            }
        } catch (RuntimeException ex) {
    ...
    public void setMdnNumber(String mdn, Message onComplete) {
        //mMdn = mdn;
        mCi.setMdnNumber(mdn, null);
        //Read mdn
        if (DBG) log("setMdnNumber : read mdn after wirte");
        mCi.getCDMASubscription(obtainMessage(EVENT_POLL_STATE_CDMA_SUBSCRIPTION, onComplete));
    }
    ...
    //add by VIA for network info[begin][ALPS00384791]
    /** Returns null until receive EVENT_POLL_STATE_REGISTRATION_CDMA */
    public String getSid() {
        if (DBG) log("getSid: mSid=" +mSid);
        return mSid;
    }

    public String getNid() {
        if (DBG) log("getNid: mNid=" +mNid);
        return mNid;
    }
    //add by VIA for network info[end][ALPS00384791]
    /**
    ...
    protected void hangupAndPowerOff() {
        ...
        //Modify by via begin [ALPS00374383]
        if (!FeatureOption.MTK_GEMINI_SUPPORT) {  
            log("VIA hangupAndPowerOff cm.setRadioPower(false, null);");
            mCi.setRadioPower(false, null);
        }
        //Modify by via end [ALPS00374383]
    }
    ...
    protected void onUpdateIccAvailability() {
        ...
        if (mUiccApplcation != newUiccApplication) {
            ...
            if (newUiccApplication != null) {
                ...
                //delete by yangli,because can't get isSubscriptionFromRuim value from cdmaSubscription.
                //so, we have to delete it ,else we can't get ruim_ready event.
                //if (isSubscriptionFromRuim) {
    ...
    public void setRadioPowerOn() {
        ...
        //Modify by gfzhu VIA end [ALPS00396584][ALPS00369509]

        Rlog.e(LOG_TAG, "VIA setRadioPowerOn() mDesiredPowerState " + mDesiredPowerState);
    ...
    public void setRadioPower(boolean power, boolean shutdown) {
        Rlog.d(LOG_TAG, "setRadioPower " + power + " shutdown = " + shutdown +
            " mPendingRadioPowerOffAfterDataOff = " + mPendingRadioPowerOffAfterDataOff);
        mDesiredPowerState = power;
        setPendingRadioPowerOff();
        if (shutdown) {
