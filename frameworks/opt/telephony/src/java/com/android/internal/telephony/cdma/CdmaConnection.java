...
public class CdmaConnection extends Connection {
    ...
    boolean isVideo;
    //Added to indicate if the MO call has been accepted by remote side.
    boolean isRealConnected;
    /*
    ...
    CdmaConnection(Context context, String dialString, CdmaCallTracker ct, CdmaCall parent) {
        ...
        mIndex = -1;
        //Add flag to indicated if remote side has answer the MO call(CTA case)
    ...
    CdmaConnection(Context context, CdmaCallWaitingNotification cw, CdmaCallTracker ct,
        ...
        Rlog.d(LOG_TAG, "before formate number = " + cw.number);
        if(mAddress != null && mAddress.length() > 0) {
            String number = mAddress;
            number = PlusCodeToIddNddUtils.removeIddNddAddPlusCode(cw.number);
            if(number != null) {
                mAddress = number;
                if(cw.numberType == 1 && number!= null && number.length() > 0 && number.charAt(0) != '+') {
                    mAddress = "+" + number;
                }
            }
        }
        Rlog.d(LOG_TAG, "after formate number = " + mAddress);
    ...
    public void hangup(DisconnectCause discRingingConnCause) throws CallStateException {
        if (!mDisconnected) {
            ...
            if (mParent == mOwner.mRingingCall && mOwner.mRingingCall.hasConnection(this)) { 			
                ...
                Rlog.d(LOG_TAG,"[CdmaConn] hangup RingingConn with cause=" + discRingingConnCause + ", and ringCall state=" + mParent.mState); 				
    ...
    disconnectCauseFromCode(int causeCode) {
        ...
        switch (causeCode) {
            ...
            default:
                ...
                //via support start -20120107
                AppState uiccAppState = AppState.APPSTATE_UNKNOWN;

                Rlog.d(LOG_TAG, "[CDMAConn] UiccController.getInstance() before");
                if(UiccController.getInstance(phone.getMySimId()) != null) {
                    Rlog.d(LOG_TAG, "[CDMAConn] UiccController.getInstance() != null");
                    ...
                    if(tempApplication != null) {
                        Rlog.d(LOG_TAG, "[CDMAConn] tempApplication != null");
                        uiccAppState = tempApplication.getState();
                    }
                }
                //via support end -20120107
    ...
    onConnectedInOrOut() {
        ...
        if (!mIsIncoming) {
            // outgoing calls only
            //via support start, send DTMF the CDMAcall is really accepted. 
            //processNextPostDialChar();
            //via support end
        } else {
