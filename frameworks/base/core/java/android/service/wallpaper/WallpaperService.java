...
public abstract class WallpaperService extends Service {
    ...
    public class Engine {
        ...
        boolean mDestroyed;
        /// M: declare stereo 3D flag @{
        boolean mSetFlagsEx;
        boolean mAlignCenter = false;
        int mClientWidth = 0;
        /// @}
        ...
        /// M: set stereo 3D flag to enable 3D wallpaper @{
        /** {@hide} */
        public void setFlagsEx(int flagsEx, int mask) {
            if (FeatureOption.MTK_S3D_SUPPORT) {
                int f = (mLayout.flagsEx & ~mask) | (flagsEx & mask);

                if (mLayout.flagsEx != f) {
                    mSetFlagsEx = true;
                    mLayout.flagsEx = f;
                    if (DEBUG) Log.v(TAG, "Set flagsEx = 0x " + Integer.toHexString(mLayout.flagsEx)
        ...
        /// M: set wallpaper default position to center @{
        /** @hide */
        public void setAlignCenter(int width) {
            mAlignCenter = true;
            mClientWidth = width;
        }
        /// @}
        ...
        void updateSurface(boolean forceRelayout, boolean forceReport, boolean redrawNeeded) {
            ...
            // / M: ALPS01103663 workaround for SmartBook, image wallpaper will restart by systemUI @{
            if (FeatureOption.MTK_SMARTBOOK_SUPPORT && IMAGEWALLPAPER.equals(mCurrentWallpaper)) {
                int desiredWidth = mWallpaperManager.getDesiredMinimumWidth();
                int desiredHeitht = mWallpaperManager.getDesiredMinimumHeight();
                if (myWidth != desiredWidth || myHeight != desiredHeitht) {
                    Log.w(TAG, "Image wallpaper----myWidth: " + myWidth + "myHeight: " + myHeight);
                    Log.w(TAG, "Image wallpaper----desiredWidth: " + desiredWidth + "desiredHeitht: " + desiredHeitht);
                    myWidth = desiredWidth;
                    myHeight = desiredHeitht;
                    mSurfaceHolder.setFixedSize(myWidth, myHeight);
                }           
            }
            // / @}
            ...
            if (forceRelayout || creating || surfaceCreating || formatChanged || sizeChanged
                ...
                try {
                    ...
                    /// M: for S3D, set initial wallpaper position
                    mLayout.x = mAlignCenter && (myWidth > 0) ? (mClientWidth - myWidth)/2 : 0;
                    mLayout.y = 0;
                    ...
                    try {
                        ...
                        /// M: check stereo 3D flag
                        if (didSurface && !mReportedVisible && !mSetFlagsEx) {
    ...
    class IWallpaperEngineWrapper extends IWallpaperEngine.Stub
        ...
        public void setFlagsEx(int flagsEx, int mask) {
            if (mEngine != null) {
                mEngine.setFlagsEx(flagsEx, mask);
            }
        }
        ...
        public void executeMessage(Message message) {
            switch (message.what) {
                case DO_ATTACH: {
                    ...
                    /// M: for S3D @{
                    if (mAlignCenter) {
                        mEngine.setAlignCenter(mClientWidth);
                    }
                    /// @}
                    engine.attach(this);
                    return;
    ...
    /// M: for S3D, get all active engines of this wallpaper service
    /** {@hide} */
    public ArrayList<Engine> getActiveEngines() {
        return mActiveEngines;
    }
