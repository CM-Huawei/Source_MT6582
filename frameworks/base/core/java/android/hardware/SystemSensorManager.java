...
public class SystemSensorManager extends SensorManager {
	...
	/// M: register new sensor service implement for sensor toast new feature@{
	private static int callerPid = 0;
	private static Context mAppContextImpl;
	private static boolean SensorFirstUsed = false;
	private static long first_time_stamp = 0;

    private static String getCurProcessName(Context context,int callerPid) 
	{  
		int pid = callerPid;  
    ...
    private static boolean needToast()
	{
	        callerPid= Binder.getCallingPid();
	        String callerName = getCurProcessName(mAppContextImpl,callerPid);
			boolean needtoast = false;
			Log.d(TAG,"caller name =" + callerName);

			if(callerName.contains("compass")|| callerName.contains("zhinanzhen"))
			{
			  //Log.d(TAG,"have compass app ");
			  needtoast = true;
			}
    ...
    private static abstract class BaseEventQueue {
        ...
        private int enableSensor(
            ...
            if (sensor == null) throw new NullPointerException();
				int res = 0;
            res = nativeEnableSensor(nSensorEventQueue, sensor.getHandle(), rateUs,
                    maxBatchReportLatencyUs, reservedFlags);
			if(FeatureOption.MTK_SHOW_MSENSOR_TOAST_SUPPORT){
				if(res == 0){
					SensorFirstUsed = true;
					first_time_stamp = SystemClock.elapsedRealtime();
					Log.d(TAG, "enableSensor: " + sensor.getName());
				}
			}
            	return res;
        }
    ...
    static final class SensorEventQueue extends BaseEventQueue {
        ...
        protected void dispatchSensorEvent(int handle, float[] values, int inAccuracy,
            ...
            switch (t.sensor.getType()) {
                ...
                case Sensor.TYPE_ORIENTATION:
					...
					if(FeatureOption.MTK_SHOW_MSENSOR_TOAST_SUPPORT){	
					if(SensorFirstUsed){
						if((SystemClock.elapsedRealtime() - first_time_stamp) > 3000)
							{
								Log.d(TAG,"first_time_stamp =" + first_time_stamp);
								SensorFirstUsed = false;
								//Log.d(TAG,"accuracy =" + t.accuracy);
								if(t.accuracy != 3){
									//Log.d(TAG,"accuracy2 =" + t.accuracy);
									showtoast(t.sensor.getType(), 3);}
							}
						}
					}
					///@}
                    if ((t.accuracy >= 0) && (accuracy != t.accuracy)) {
						...
						/// M: register new sensor service implement for sensor toast new feature@{
						if(FeatureOption.MTK_SHOW_MSENSOR_TOAST_SUPPORT){
							Log.d(TAG,"accuracy changed and toast will performed old =" + accuracy + " ,new =" + t.accuracy);
							showtoast(t.sensor.getType(), accuracy);
						}
						///@}
                    }
                    break;
		...
		/// M: register new sensor service implement for sensor toast new feature@{
		private void showtoast(int sensortype, int oldaccuracy){
			//Log.d(TAG,"showtoast! oldaccuracy: " + oldaccuracy);
			//Log.d(TAG,"showtoast! sensortype: " + sensortype);
			if(oldaccuracy == 3){
				if(sensortype == 2){
					if(mHandler != null){
						mHandler.post(new Runnable(){
						public void run(){
						Log.d(TAG,"Toast function would appear msensor!");
						if((mAppContextImpl != null) && needToast())
						//if(mAppContextImpl != null)
						{
							Log.d(TAG,"show toast msensor!");
							Toast.makeText(mAppContextImpl.getApplicationContext(),
								com.mediatek.internal.R.string.msensor_toast_text,Toast.LENGTH_SHORT).show();
							//mToastView = (LinearLayout)mSensorToast.getView();
							//mImage = new ImageView(mAppContextImpl.getApplicationContext());
							//mImage.setImageResource();
							//mToastView.addView(mImage,0);
							//mSensorToast.show();
						}else{
							Log.d(TAG, "Error: Sensor toast mAppContextImpl is null");	
							}
						}
						});//modification
					}else{
						Log.e(TAG, "Error: Sensor toast handler is null");
					}
				}
				if(sensortype == 3){
					if(mHandler != null){
						mHandler.post(new Runnable(){
						public void run(){
						Log.d(TAG,"Toast function would appear orientation!");
						if((mAppContextImpl != null) && needToast())
						//if(mAppContextImpl != null)
						{
							Log.d(TAG,"show toast orientation!");
							Toast.makeText(mAppContextImpl.getApplicationContext(),
								com.mediatek.internal.R.string.msensor_toast_text,Toast.LENGTH_SHORT).show();
							//mToastView = (LinearLayout)mSensorToast.getView();
							//mImage = new ImageView(mAppContextImpl.getApplicationContext());
