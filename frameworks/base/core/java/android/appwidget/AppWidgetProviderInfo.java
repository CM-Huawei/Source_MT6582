...
public class AppWidgetProviderInfo implements Parcelable {
    ...
    /// M:
    /// Whether appWidget application has used customer View, if true, checkSignature will be done 
    /// in AppWidgetHostView, app that passed checkSignature will obtain enlarged Context permissions
    /// ; otherwise, app will run default flow with default Context permissions.
    /// 
    ///  <p>This field corresponds to the <code>android:hasUsedCustomerView</code> attribute in
    ///  the AppWidget meta-data file.
    ///         
    /// @{
    /**
     * @hide
     */
    public boolean hasUsedCustomerView;
    ...
    public AppWidgetProviderInfo(Parcel in) {
        ...
        /// M:  add for using customer view and add specified to default launcher @{
        boolean [] temp = in.createBooleanArray();
        Log.v("AppWidget", "array = " + temp);
        if(temp != null && temp.length > 0){
            this.hasUsedCustomerView = temp[0];
        }
        /// @}
    }
    ...
    public void writeToParcel(android.os.Parcel out, int flags) {
        ...
        /// M: add for using customer view and add specified widget to default launcher
        out.writeBooleanArray(new boolean[]{this.hasUsedCustomerView});
    }
    ...
    public AppWidgetProviderInfo clone() {
        ...
        that.hasUsedCustomerView = this.hasUsedCustomerView;
        return that;
    }
