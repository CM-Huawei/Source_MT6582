...
public class NetworkStatsHistory implements Parcelable {
    ...
    public void recordData(long start, long end, NetworkStats.Entry entry) {
        ...
        for (int i = startIndex; i >= 0; i--) {
            ...
            final long curEnd = bucketStart[i] + bucketDuration;
            ...
            if(duration <=0){
                ...
                Xlog.d(TAG,"recordData error i="+i+" duration="+ duration +" start="+start+" end="+
                    end+" overlap="+ overlap +" curEnd="+curEnd+" curStart="+curStart +" bucketDuration="+bucketDuration);

                Xlog.d(TAG,"bucket bucketCount="+ bucketCount +" startIndex="+getIndexAfter(end));
                for(int k=getIndexAfter(end); k >= 0; k--){
                    Xlog.d(TAG,"bucket bucketStart["+ k+"]="+bucketStart[k]);
                }
                break;
            }
