...
public class SurfaceView extends View {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case UPDATE_WINDOW_MSG: {
                    Log.i(TAG, "updateWindow -- UPDATE_WINDOW_MSG");
    ...
    final ViewTreeObserver.OnScrollChangedListener mScrollChangedListener
            = new ViewTreeObserver.OnScrollChangedListener() {
                    ...
                    public void onScrollChanged() {
                        Log.i(TAG, "updateWindow -- OnScrollChangedListener");
                        updateWindow(false, false);
                    }
            };
    ...
    /// M: Save the value of flagesEx what app wants
    int mFlagsEx = WindowManager.LayoutParams.FLAG_EX_INIT_VALUE;
    ...
    private final ViewTreeObserver.OnPreDrawListener mDrawListener =
            new ViewTreeObserver.OnPreDrawListener() {
                ...
                public boolean onPreDraw() {
                    ...
                    Log.i(TAG, "updateWindow -- OnPreDrawListener" +
    ...
    /**
     * Set specific flagsEx for SurfaceFlinger 
     *
     * @hide
     */
    /// M: Public API to set flagsEx for SurfaceFlinger @{
    public void setFlagsEx(int flagsEx, int mask) {
        if (FeatureOption.MTK_S3D_SUPPORT) {
            Log.d(TAG, "Set flagEx = 0x" + Integer.toHexString(flagsEx)
                    + " mask = 0x" + Integer.toHexString(mask));
            mFlagsEx = (mFlagsEx & ~mask) | (flagsEx & mask);
    ...
    protected void onDetachedFromWindow() {
        ...
        Log.i(TAG, "updateWindow -- onDetachedFromWindow");
        updateWindow(false, false);
        mHaveFrame = false;
        if (mWindow != null) {
            try {
    ...
    protected boolean setFrame(int left, int top, int right, int bottom) {
        ...
        Log.i(TAG, "updateWindow -- setFrame");
        updateWindow(false, false);
        return result;
    }
        ...
        if (force || creating || formatChanged || sizeChanged || visibleChanged
            ...
            try {
                ...
                if (mWindow == null) {
                    ...
                    if (LOG_UPDATE_WINDOW) {
                        Trace.traceBegin(Trace.TRACE_TAG_VIEW, "SurfaceView addToDisplayWithoutInputChannel");
                    }
                ...
                /// M: set flagsEx @{
                if (FeatureOption.MTK_S3D_SUPPORT) {
                    mLayout.flagsEx = visible ? mFlagsEx
                           : WindowManager.LayoutParams.FLAG_EX_INIT_VALUE;
                }
                /// @}
                ...
                try {
                    ...
                    if (mSurfaceCreated && (surfaceChanged || (!visible && visibleChanged))) {
                        ...
                        if (mSurface.isValid()) {
                            if (DEBUG) Log.i(TAG, "Callback --> surfaceDestroyed");
                            callbacks = getSurfaceCallbacks();
                            for (SurfaceHolder.Callback c : callbacks) {
                                if (DEBUG) Log.i(TAG, "surfaceDestroyed callback +");
                    ...
                    if (visible && mSurface.isValid()) {
                        if (!mSurfaceCreated && (surfaceChanged || visibleChanged)) {
                            ...
                            mIsCreating = true;
                            if (DEBUG) Log.i(TAG, "Callback --> surfaceCreated");
                            if (callbacks == null) {
                                callbacks = getSurfaceCallbacks();
                            }
                            for (SurfaceHolder.Callback c : callbacks) {
                                if (DEBUG) Log.i(TAG, "surfaceCreated callback +");
                        ...
                        if (creating || formatChanged || sizeChanged
                            ...
                            for (SurfaceHolder.Callback c : callbacks) {
                                if (DEBUG) Log.i(TAG, "surfaceChanged callback +");
                    ...
                    if (LOG_UPDATE_WINDOW) {
                        Trace.traceBegin(Trace.TRACE_TAG_VIEW, "SurfaceView performDeferredDestroy");
                    }
    ...
    void handleGetNewSurface() {
        Log.i(TAG, "updateWindow -- handleGetNewSurface");
        updateWindow(false, false);
    }
    ...
    private final SurfaceHolder mSurfaceHolder = new SurfaceHolder() {
        ...
        public void setFormat(int format) {
            ...
            if (mWindow != null) {
                Log.i(TAG, "updateWindow -- setFormat");
                updateWindow(false, false);
            }
        }
