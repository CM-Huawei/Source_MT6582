...
public class RemoteViews implements Parcelable, Filter {
    ...
    /// M: add for notifying the incorrect usage of application developer,refer to ALPS00740044
    private boolean mHasTransferredToHost = false;
    /**
    ...
    public void writeToParcel(Parcel dest, int flags) {
        /** M:  notifying the incorrect usage of application developer. @{ */
        if(mHasTransferredToHost) {
           Log.e(LOG_TAG,"ANR Warning,RemoteViews can only be used once ,if not ,it may cause ANR in hosts such as Laucher,SystemUI. keys for search <ANR Exception MSG   History>");
        }
        mHasTransferredToHost = true;
        /** @} */
        if (!hasLandscapeAndPortraitLayouts()) {
