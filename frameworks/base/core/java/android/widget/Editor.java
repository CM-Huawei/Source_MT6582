...
public class Editor {
    ...
    void prepareCursorControllers() {
        ...
        if (!mSelectionControllerEnabled) {
            ...
            if (mSelectionModifierCursorController != null) {
                mSelectionModifierCursorController.onDetached();
                /// M: ALPS01243943 need to hide first if no action bar in the window
                if (mTextView.length() == 0) mSelectionModifierCursorController.hide();
    ...
    boolean startSelectionActionMode() {
        ...
        /// M: selection mode should be started if the textview is selectable
        if ((!willExtract)||(mTextView.isTextSelectable())) {
