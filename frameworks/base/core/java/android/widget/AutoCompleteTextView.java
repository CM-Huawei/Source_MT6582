    ...
    private void performCompletion(View selectedView, int position, long id) {
        if (isPopupShowing()) {
            ...
            if (mItemClickListener != null) {
                ...
                int clickedPosition = position; /// M: Record position
                if (selectedView == null || position < 0) {
                    selectedView = list.getSelectedView();
                    position = list.getSelectedItemPosition();
                    id = list.getSelectedItemId();
                }
                /// M: When clicked on item in MoreSuggestionView generated by IME, if clicked position is bigger than amount 
                ///    of displayed items in ListPopupWindow, there's no corresponding selectedItem & position in ListPopupWindow.
                ///    Set position to the position indicated by IME to fix this problem. @{
                if (selectedView == null && position < 0) {
                    if (clickedPosition < mAdapter.getCount()) {
