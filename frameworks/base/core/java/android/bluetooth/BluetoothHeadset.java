...
///M: to handle bind service in multi-user mode fail case (when onwer process try to bind with user process)
...
public final class BluetoothHeadset implements BluetoothProfile {
    ...
    boolean doBind() {
        ...
        int foregroundUser =  um.getSwitchedUserId();
        int callingUser = um.getUserHandle();
        
        if (VDBG) Log.d(TAG,"Binding service..., foregroundUser:" + foregroundUser + ", callingUser:" + callingUser);
		
        Intent intent = new Intent(IBluetoothHeadset.class.getName());
        ComponentName comp = intent.resolveSystemService(mContext.getPackageManager(), 0);
        intent.setComponent(comp);

        if( foregroundUser != callingUser){
            if (comp == null || !mContext.bindServiceAsUser(intent, mConnection, 0 , UserHandle.CURRENT)) {
