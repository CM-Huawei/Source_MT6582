...
public class Process {
    ...
    public static final long getZramExtraAvailableSize()
    {
        ...
        if (SystemProperties.getBoolean("mediatek.virtual_zram_extra", false) ) {
            // virtual zram extra memory
            // compressed zram size could be up to total memory / 4
            long compTotalSize = getTotalMemory() / 4;
            long origTotalSize = (long)(compTotalSize * getZramCompressRatio());
            long origAvailableSize = origTotalSize - Debug.getOrigZram();
            if (origAvailableSize > 0) {
                return (long)(origAvailableSize * (getZramCompressRatio() - 1.0f) / getZramCompressRatio());
            }
            return 0;
        } else {
