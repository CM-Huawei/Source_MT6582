...
public final class PowerManager {
   ...
   /**
    ...
    /**
     * Forces the device to go to sleep with reason.
     * <p>
     * Overrides all the wake locks that are held.
     * </p><p>
     * Requires the {@link android.Manifest.permission#DEVICE_POWER} permission.
     ...
     * {@hide}
     */
    public void goToSleepEx(long time, int reason) {
        try {
            mService.goToSleep(time, reason);
        } catch (RemoteException e) {
        }
    }
    ...
    /**
     * [Smart Book] Forces the device to wake up from sleep.
     * <p>
     * If the device is currently asleep, wakes it up, otherwise does nothing.
     * This is what happens when the power key is pressed to turn on the screen.
     * </p><p>
     ...
     * {@hide}
     */
    public void sbWakeUp(long time) {
        try {
            mService.sbWakeUp(time);
        } catch (RemoteException e) {
        }
    }

    /**
     * [Smart Book] Forces the device to go to sleep.
     * <p>
     * Overrides all the wake locks that are held.
     * This is what happens when the power key is pressed to turn off the screen.
     * </p><p>
     ...
     * {@hide}
     */
    public void sbGoToSleep(long time) {
        try {
            mService.sbGoToSleep(time, GO_TO_SLEEP_REASON_USER);
        } catch (RemoteException e) {
        }
    }

    /**
     * [Smart Book] Forces smart book to turn on screen backlight.
     * <p>
     * Requires the {@link android.Manifest.permission#DEVICE_POWER} permission.
     * </p>
     *
     * @param timeout The timeout is used to set the screen timout for smartbook individually.
     * It will set to 0 if smartbook use phone screen timeout. 
     *
     * @see #userActivity
     * @see #wakeUp
     *
    ...
    public void sbScreenOnCtrl(int timeout) {
        try {
            mService.sbScreenOnCtrl(timeout);
        } catch (RemoteException e) {
        }
    }

    /**
     * [Smart Book] Forces smart book to turn off screen backlight.
     * <p>
     * Requires the {@link android.Manifest.permission#DEVICE_POWER} permission.
     * </p>
     *
    ...
    public void sbScreenOffCtrl(int timeout) {
        try {
            mService.sbScreenOffCtrl(timeout);
        } catch (RemoteException e) {
        }
    }
