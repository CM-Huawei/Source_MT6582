...
public interface IActivityManager extends IInterface {
    ...
    int NOTIFY_WINDOW_TIMEOUT = IBinder.FIRST_CALL_TRANSACTION + 183;   /// M: dump message history and future messages for app transition timeout or app freeze timeout cases
    int SET_WALLPAPER_PROCESS = IBinder.FIRST_CALL_TRANSACTION + 184;   /// M: LCA, set wallpaper process
    int UPDATE_WALLPAPER_STATE = IBinder.FIRST_CALL_TRANSACTION + 185;  /// M: LCA, update wallpaper process adj

    /// M: ALPS00431397. When running window animation, halt the activity
    /// launchigng. Otherwise, restore the activity state @{
    int HALT_ACTIVITY_RESUMING = IBinder.FIRST_CALL_TRANSACTION + 186;
    int RESTORE_ACTIVITY_RESUMING = IBinder.FIRST_CALL_TRANSACTION + 187;
    /// @}

    /// M: ALPS00404470. LCA check-in
    int GET_PROCESS_PSWAP_TRANSACTION = IBinder.FIRST_CALL_TRANSACTION+188;
    /// @}

    /// M: ALPS01257806, Force kill AP for IPO without stopping the package @{
    int FORCE_KILL_PACKAGE_TRANSACTION = IBinder.FIRST_CALL_TRANSACTION+189;
    /// @}
