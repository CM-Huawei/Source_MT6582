...
void OpenGLRenderer::setViewport(int width, int height) {
    ATRACE_CALL();
    ALOGD("setViewport %dx%d <%p>", width, height, this);
...
status_t OpenGLRenderer::prepareDirty(float left, float top,
        float right, float bottom, bool opaque) {
    ATRACE_CALL();
    ALOGD("prepareDirty (%.2f, %.2f, %.2f, %.2f) opaque %d <%p>", left, top, right, bottom, opaque, this);
...
status_t OpenGLRenderer::clear(float left, float top, float right, float bottom, bool opaque) {
    ...
    if (!opaque || mCountOverdraw) {
        ALOGD("clear (%.2f, %d - %.2f = %.2f, %.2f - %.2f = %.2f, %.2f - %.2f = %.2f) opaque %d <%p>", left, mSnapshot->height, bottom,
            mSnapshot->height - bottom, right, left, right - left, bottom, top, bottom - top, opaque, this);
...
void OpenGLRenderer::eventMark(const char* name) const {
    if (g_HWUI_debug_egl_trace) {
#ifdef LOG_TAG
#undef LOG_TAG
#endif
#define LOG_TAG "libEGL"
        ALOGD("eventMark %s", name);
#undef LOG_TAG
#define LOG_TAG "OpenGLRenderer"
    }
...
void OpenGLRenderer::updateLayers() {
    ...
    if (count > 0) {
        LAYER_LOGD("--updateLayers %d <%p>", count, this);
        ...
        endMark();
        LAYER_LOGD("--updateLayers end <%p>", this);
    }
}
...
void OpenGLRenderer::flushLayers() {
    ...
    int count = mLayerUpdates.size();
    if (count > 0) {
        LAYER_LOGD("--flushLayers %d <%p>", count, this);
        ...
        ATRACE_BEGIN("glBindFramebuffer");
        glBindFramebuffer(GL_FRAMEBUFFER, getTargetFbo());
        ATRACE_END();

        endMark();
        LAYER_LOGD("--flushLayers end <%p>", this);
    }
}
...
void OpenGLRenderer::pushLayerUpdate(Layer* layer) {
    ...
    if (layer) {
        ...
        LAYER_LOGD("push layer %p, textureSize (%d, %d), layerSize (%.2f,%.2f) <%p>", layer,
            layer->getWidth(), layer->getHeight(), layer->layer.getWidth(), layer->layer.getHeight(), this);
...
void OpenGLRenderer::cancelLayerUpdate(Layer* layer) {
    ATRACE_CALL();
    if (layer) {
        LAYER_LOGD("cancel layer %p, textureSize (%d, %d), layerSize (%.2f,%.2f) <%p>", layer,
            layer->getWidth(), layer->getHeight(), layer->layer.getWidth(), layer->layer.getHeight(), this);
        for (int i = mLayerUpdates.size() - 1; i >= 0; i--) {
            if (mLayerUpdates.itemAt(i) == layer) {
...
void OpenGLRenderer::clearLayerUpdates() {
    ...
    if (count > 0) {
        ...
        for (size_t i = 0; i < count; i++) {
            LAYER_LOGD("clear layer %p (%d/%d) <%p>", mLayerUpdates.itemAt(i), i, count, this);
        ...
        if (!bounds.isEmpty()) {
            if (layer->isEmpty()) {
                ...
                TT_ADD(layer->getTexture(), layer->getWidth(), layer->getHeight(),
                                         GL_RGBA, GL_UNSIGNED_BYTE, String8("createLayer"), "[3D][OpenGLRenderer.cpp] createLayer +");

                layer->setEmpty(false);
            }

            ATRACE_BEGIN("glCopyTexSubImage2D");
...
bool OpenGLRenderer::createFboLayer(Layer* layer, Rect& bounds, Rect& clip, GLuint previousFbo) {
    ...
    if (layer->isEmpty()) {
        ...
        layer->setEmpty(false);
        TT_ADD(layer->getTexture(), layer->getWidth(), layer->getHeight(), GL_RGBA, GL_UNSIGNED_BYTE, String8("layer"), "[OpenGLRenderer.cpp] allocateTexture +");
    }
...
void OpenGLRenderer::composeLayer(sp<Snapshot> current, sp<Snapshot> previous) {
    ...
    if (!mCaches.layerCache.put(layer)) {
        LAYER_LOGD("Deleting layer %p <%p>", layer, this);
...
status_t OpenGLRenderer::drawDisplayList(DisplayList* displayList, Rect& dirty,
    ...
    if (!Caches::getInstance().isDisplayListActive(displayList)) {
        ALOGE("ERROR: The displaylist %p has been destroyed!!", displayList);
    ...
    if (displayList && displayList->isRenderable()) {
        ...
        ATRACE_BEGIN("defer drawing commands");
...
status_t OpenGLRenderer::drawLayer(Layer* layer, float x, float y) {
    ...
    LAYER_LOGD("draw layer %p at (%.2f, %.2f), textureSize (%d, %d), layerSize (%.2f, %.2f), isIdentity %d <%p>",
        layer, x, y, layer->getWidth(), layer->getHeight(), layer->layer.getWidth(),
        layer->layer.getHeight(), transform ? transform->isIdentity() : 1, this);
...
status_t OpenGLRenderer::drawRects(Vertex* vertices, int count, SkPaint* paint) {
    ATRACE_NAME("drawRectsVertex");
    ...
    setupDrawShader();
    setupDrawColorFilter();
    setupDrawBlending(getXfermode(paint->getXfermode()));
    setupDrawProgram();
    setupDrawDirtyRegionsDisabled();
    setupDrawModelView(0.0f, 0.0f, 1.0f, 1.0f, true, true);
    setupDrawColorUniforms();
    setupDrawShaderUniforms();
    setupDrawColorFilterUniforms();

    drawIndexedQuads(&vertices[0], count);
