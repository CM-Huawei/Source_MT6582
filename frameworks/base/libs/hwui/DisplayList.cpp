...
void DisplayList::outputViewProperties(const int level) {
    updateMatrix();
    if (mLeft != 0 || mTop != 0) {
        ALOGD("%*sTranslate (left, top) %d, %d by property", level * 2, "", mLeft, mTop);
    }
    if (mStaticMatrix) {
        ALOGD("%*sConcatMatrix (static) %p: " MATRIX_STRING " by property",
                level * 2, "", mStaticMatrix, MATRIX_ARGS(mStaticMatrix));
    }
    if (mAnimationMatrix) {
        ALOGD("%*sConcatMatrix (animation) %p: " MATRIX_STRING " by property",
                level * 2, "", mAnimationMatrix, MATRIX_ARGS(mAnimationMatrix));
    }
    if (mMatrixFlags != 0) {
        if (mMatrixFlags == TRANSLATION) {
            ALOGD("%*sTranslate %f, %f by property", level * 2, "", mTranslationX, mTranslationY);
        } else {
            ALOGD("%*sConcatMatrix %p: " MATRIX_STRING " by property",
    ...
    if (mAlpha < 1) {
        if (mCaching) {
            ALOGD("%*sSetOverrideLayerAlpha %.2f by property", level * 2, "", mAlpha);
            clipToBoundsNeeded = false; // clipping done by layer
        } else if (!mHasOverlappingRendering) {
            ALOGD("%*sScaleAlpha %.2f by property", level * 2, "", mAlpha);
        } else {
            ...
            ALOGD("%*sSaveLayerAlpha %.2f, %.2f, %.2f, %.2f, %d, 0x%x by property", level * 2, "",
                    (float) 0, (float) 0, (float) mRight - mLeft, (float) mBottom - mTop,
                    (int)(mAlpha * 255), flags);
        }
    }
    if (clipToBoundsNeeded) {
        ALOGD("%*sClipRect %.2f, %.2f, %.2f, %.2f by property", level * 2, "", 0.0f, 0.0f,
...
void DisplayList::iterate(OpenGLRenderer& renderer, T& handler, const int level) {
    ...
    mClipRectOp->viewName = mName;
    mSaveLayerOp->viewName = mName;
    mSaveOp->viewName = mName;
    mRestoreToCountOp->viewName = mName;

    int restoreTo = renderer.getSaveCount();
    handler(mSaveOp->reinit(SkCanvas::kMatrix_SaveFlag | SkCanvas::kClip_SaveFlag),
            PROPERTY_SAVECOUNT, mClipToBounds);

    DISPLAY_LIST_LOGD("%*sSave %d %d by property", (level + 1) * 2, "",
            SkCanvas::kMatrix_SaveFlag | SkCanvas::kClip_SaveFlag, restoreTo);

    setViewProperties<T>(renderer, handler, level + 1);

    if (mClipToBounds && renderer.quickRejectNoScissor(0, 0, mWidth, mHeight)) {
        DISPLAY_LIST_LOGD("%*sRestoreToCount %d by property", (level + 1) * 2, "", restoreTo);
...
#if DEBUG_DISPLAY_LIST
        if (g_HWUI_debug_display_list) {
            //op->output(level + 1, DisplayListOp::kOpLogFlag_JSON); or
            if (strcmp(op->name(), "DrawDisplayList") != 0)
              op->output(level + 1, DisplayListOp::kOpLogFlag_Recurse);
        }
#endif
