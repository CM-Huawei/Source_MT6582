...
public:
    ...
    virtual status_t multiDraw(OpenGLRenderer& renderer, Rect& dirty,
        ...
        for (unsigned int i = 0; i < ops.size(); i++) {
            ...
            ATRACE_BEGIN(ops[i].op->viewName.string());
...
#define SET_TEXTURE(ptr, posRect, offsetRect, texCoordsRect, xDim, yDim) \
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw bitmap %p at %f %f <%p>", mBitmap, mLocalBounds.left, mLocalBounds.top, this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw bitmap %p matrix "MATRIX_STRING" <%p>", mBitmap, MATRIX_ARGS(mMatrix), this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw bitmap %p <%p>", mBitmap, this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw bitmap %p mesh %d x %d <%p>", mBitmap, mMeshWidth, mMeshHeight, this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw patch "RECT_STRING" <%p>", RECT_ARGS(mLocalBounds), this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw color %#x, mode %d <%p>", mColor, mMode, this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const{
        if (CC_LIKELY(mPaint)) {
            OP_LOG("Draw Rect "RECT_STRING" paint %p id %d style %d AA %d color %p <%p>", RECT_ARGS(mLocalBounds), mPaint,
                            mPaint->getGenerationID(), mPaint->getStyle(), mPaint->isAntiAlias(), (void*)mPaint->getColor(), this);
        } else {
            OP_LOG("Draw Rect "RECT_STRING" <%p>", RECT_ARGS(mLocalBounds), this);
        }
    }
    ...
    virtual status_t multiDraw(OpenGLRenderer& renderer, Rect& dirty,
            const Vector<OpStatePair>& ops, const Rect& bounds) {
        if (CC_LIKELY(mPaint)) {
            ALOGD("drawRect multiDraw %s paint %p id %d style %d AA %d color %p", (ops[0].op)->viewName.string(),
                mPaint, mPaint->getGenerationID(), mPaint->getStyle(), mPaint->isAntiAlias(), (void*)mPaint->getColor());
        }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw RoundRect "RECT_STRING", rx %f, ry %f <%p>", RECT_ARGS(mLocalBounds), mRx, mRy, this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw Circle x %f, y %f, r %f <%p>", mX, mY, mRadius, this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw Oval "RECT_STRING" <%p>", RECT_ARGS(mLocalBounds), this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw Arc "RECT_STRING", start %f, sweep %f, useCenter %d <%p>",
                RECT_ARGS(mLocalBounds), mStartAngle, mSweepAngle, mUseCenter, this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw Path %p in "RECT_STRING" <%p>", mPath, RECT_ARGS(mLocalBounds), this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw Lines count %d <%p>", mCount, this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw Points count %d <%p>", mCount, this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw some text, %d bytes <%p>", mBytesCount, this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        if (CC_LIKELY(mPaint)) {
            OP_LOG("Draw Text of count %d, bytes %d, alpha %d, color %d, paint %p <%p>",
                mCount, mBytesCount, mPaint->getAlpha(), mPaint->getColor(), mPaint, this);
        } else {
            OP_LOG("Draw Text of count %d, bytes %d <%p>", mCount, mBytesCount, this);
        }
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw Display List %p, flags %#x <%p>", mDisplayList, mFlags, this);
...
public:
    ...
    virtual void output(int level, uint32_t logFlags) const {
        OP_LOG("Draw Layer %p at %f %f <%p>", mLayer, mX, mY, this);
    }
