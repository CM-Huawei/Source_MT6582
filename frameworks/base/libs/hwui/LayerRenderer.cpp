...
void LayerRenderer::updateTextureLayer(Layer* layer, uint32_t width, uint32_t height,
    ...
    if (layer) {
        LAYER_RENDERER_LOGD("Updating layer %p, %dx%d fbo = %d, texture = %d, renderTarget = 0x%x",
                layer, width, height, layer->getFbo(), layer->getTexture(), renderTarget);
...
bool LayerRenderer::copyLayer(Layer* layer, SkBitmap* bitmap) {
    ...
    if (layer && bitmap->width() <= caches.maxTextureSize &&
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, format, bitmap->width(), bitmap->height(),
                0, format, type, NULL);
        TT_ADD(texture, bitmap->width(), bitmap->height(), format, type, String8("copyLayer"), "[LayerRenderer.cpp] copyLayer +");
        if ((error = glGetError()) != GL_NO_ERROR) goto error;
...
error:
        ...
        TT_REMOVE(texture, "[LayerRenderer.cpp] copyLayer");
