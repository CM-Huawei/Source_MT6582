...
#ifdef ANDROID_ENABLE_RENDERSCRIPT
    /// M: clear RenderScript elements
    if (mRs != 0) {
        ALOGD("~FontRenderer(): clear RenderScript elements");
        mRs->clearElements();
    }
#endif /* ANDROID_ENABLE_RENDERSCRIPT */
}
...
void checkTextureUpdateForCache(Caches& caches, Vector<CacheTexture*>& cacheTextures,
    ...
    for (uint32_t i = 0; i < cacheTextures.size(); i++) {
        ...
        if (cacheTexture->isDirty() && cacheTexture->getPixelBuffer()) {
            ...
            DUMP_ALPHA_TEXTURE(cacheTexture->getWidth(), cacheTexture->getHeight(), cacheTexture->getPixelBuffer()->getMappedPointer(), prefix, SkBitmap::kA8_Config);
        }
    }
}
