...
public class LocationController extends BroadcastReceiver {
    ...
    private void refreshViews() {
        if (mAreActiveLocationRequests) {
            if(mPrivacy) {/// M: FOR ALPS00096196. @{
                Log.d(TAG, "[MtkAgpsManagerService] This session is privacy overwrite, don't show the GPS icon in status bar");
            } else { 
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        Log.d(TAG, "[MtkAgpsManagerService] visible=" + mAreActiveLocationRequests + " mPrivacy=" + mPrivacy);
        if (LocationManager.HIGH_POWER_REQUEST_CHANGE_ACTION.equals(action)) {
            updateActiveLocationRequests();
        } else if (action.equals("android.location.PRIVACY_OVERWRITE")) { /// M: FOR ALPS00096196. @{
            Log.d(TAG, "[MtkAgpsManagerService] recv android.location.PRIVACY_OVERWRITE");
            mPrivacy = true;
        }/// M: end
    }
}
