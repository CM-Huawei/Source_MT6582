...
public class CommandQueue extends IStatusBar.Stub {
    ...
    private static final int MSG_SHOW_SIM_INDICATOR = 18 << MSG_SHIFT;
    private static final int MSG_HIDE_SIM_INDICATOR = 19 << MSG_SHIFT;
    /// @}
    /// M: [SystemUI]Show application guide for App. @{
    private static final int MSG_SHOW_APPLICATION_GUIDE = 20 << MSG_SHIFT;
    /// @}
    ...
    /// M: [SystemUI] Enhancement to replace old update notification.
    private static final boolean bReplaceUpdateNotification = true;
    HashMap<IBinder,NotificationQueueEntry> mNotifications
            = new HashMap<IBinder,NotificationQueueEntry>();
    ...
    public void updateNotification(IBinder key, StatusBarNotification notification) {
        synchronized (mList) {
            ...
            /// M: [SystemUI] Enhancement to replace old update notification.
            if (bReplaceUpdateNotification) {
                NotificationQueueEntry mQueueEntry = mNotifications.remove(key);
                if (mQueueEntry != null) {
                    mHandler.removeMessages(MSG_UPDATE_NOTIFICATION, mQueueEntry);
                }
                mNotifications.put(key, ne);
            }
