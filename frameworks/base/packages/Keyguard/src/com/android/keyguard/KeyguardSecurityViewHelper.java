    ...
    public static void hideBouncer(SecurityMessageDisplay securityMessageDisplay,
        ...
        if (ecaView != null) {
            ...
            if (duration > 0) {
                ...
                anim.setDuration(duration);
                /// M: [ALPS00786657] fix emergency call area is hidden after hide bouncer animation
                anim.addListener(new AnimatorListenerAdapter() {
                    private boolean mCanceled;
                    @Override
                    public void onAnimationCancel(Animator animation) {
                        // Fail safe and hide the emergency button in onAnimationEnd()
                        mCanceled = true;
                        ecaView.setAlpha(0f);
                    }
                    @Override
                    public void onAnimationEnd(Animator animation) {
                        ecaView.setVisibility(mCanceled ? View.INVISIBLE : View.VISIBLE);
                    }
                });
                anim.start();
            } else {
