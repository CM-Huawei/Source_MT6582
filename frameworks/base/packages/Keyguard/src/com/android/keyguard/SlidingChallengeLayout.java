...
public class SlidingChallengeLayout extends ViewGroup implements ChallengeLayout {
    ...
    /// M: flag to check current resolution res support rotation or not    
    ...
    public SlidingChallengeLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        enableScreenRotation = KeyguardUtils.shouldEnableScreenRotation(context);
    ...
    public void onDetachedFromWindow() {
        ...
        /// M: clean up KeyguardLayer
        cleanUpKeyguardLayer();
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        /// M: Dispatch touch events to dataUsage view anyway when the ChallengeView is showing.
        /// This is to prevent from dropping touch events in some cases.
        if (isChallengeShowing() && mKeyguardLayerForegroundView != null) {
            View parent = this;
            View target = mKeyguardLayerForegroundView;
            int[] locationParent = {0, 0}; 
            int[] locationTarget = {0, 0};

            // [ALPS01247455] Convert to target view's coordinate
            parent.getLocationOnScreen(locationParent);
            target.getLocationOnScreen(locationTarget);
            
            final float offsetX = locationParent[0] - locationTarget[0];
            final float offsetY = locationParent[1] - locationTarget[1];
            
            ev.offsetLocation(offsetX, offsetY);
            mKeyguardLayerForegroundView.dispatchTouchEvent(ev);
            ev.offsetLocation(-offsetX, -offsetY);
        }
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (lp.childType == LayoutParams.CHILD_TYPE_CHALLENGE) {
                ...
                ///M: [ALPS00570009] set challenge margin as 0 only for glowPad view
        ...
        if (mChallengeView != null && mChallengeView.getVisibility() != View.GONE) {
            ...
            if (root != null) {
                ...
                ///M: for phone, enlarge challenge height for full screen challenge style @{
                if(DEBUG) Log.d(TAG, "before adjust maxChallengeHeight = "+ maxChallengeHeight
                ...
                if(!enableScreenRotation) {
                    ...
                    if (mAppWidgetContainer != null && mAppWidgetContainer.getVisibility() != View.VISIBLE) {
                        maxChallengeHeight = Math.min(insetHeight, fullScreenChallengeHeight);
                    }
                }
                if(DEBUG) Log.d(TAG, "after ajust maxChallengeHeight = "+ maxChallengeHeight);
                // @}
            ...
            if (lp.childType == LayoutParams.CHILD_TYPE_WIDGETS) {
                ...
                if (root != null) {
                    ...
                    parentHeightSpec = MeasureSpec.makeMeasureSpec(
                       ...
                       || lp.childType == LayoutParams.CHILD_TYPE_KEYGUARD_LAYER_BACKGROUND) {
                // Allow scrim views to extend into the insets
                /// M: also enlarge background layer to parent size
    ...
    private void cleanUpKeyguardLayer() {
    ...
    public void updateKeyguardLayerVisibility(boolean show) {
        updateKeyguardLayerView(show);
        if (mKeyguardLayerForegroundLayout != null) {
            mKeyguardLayerForegroundLayout.setVisibility(show ? View.VISIBLE : View.INVISIBLE);
        }
        if (mKeyguardLayerBackgroundLayout != null) {
            mKeyguardLayerBackgroundLayout.setVisibility(show ? View.VISIBLE : View.INVISIBLE);
        }
    }
