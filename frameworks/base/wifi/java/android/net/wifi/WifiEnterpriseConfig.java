...
public class WifiEnterpriseConfig implements Parcelable {
    ...
    ///M: @{    
    private static final String CA_CERT2_PREFIX = KEYSTORE_URI + Credentials.WAPI_AS_CERTIFICATE;
    private static final String CLIENT_CERT2_PREFIX = KEYSTORE_URI + Credentials.WAPI_USER_CERTIFICATE;
    private static final String USER_PRIVATE_KEY2_PREFIX =  KEYSTORE_URI + Credentials.WAPI_USER_CERTIFICATE;
    ///@}
    ...
    boolean installKeys(android.security.KeyStore keyStore, String name) {
        ...
        ///M: modify for WAPI@{
        String privKeyName2= USER_PRIVATE_KEY2_PREFIX + name;
        String userCertName2 = Credentials.WAPI_USER_CERTIFICATE + name;
        String caCertName2 = Credentials.WAPI_AS_CERTIFICATE + name;
        if (mClientCertificate != null) {
            byte[] privKeyData2 = mClientPrivateKey.getEncoded();
            ret = keyStore.importKey(privKeyName2, privKeyData2, Process.WIFI_UID,
                            KeyStore.FLAG_ENCRYPTED);
            if (ret == false) {
                return ret;
            }

            ret = putCertInKeyStore(keyStore, userCertName2, mClientCertificate);
            if (ret == false) {
                // Remove private key installed
                keyStore.delKey(privKeyName2, Process.WIFI_UID);
                return ret;
            }
        }

        if (mCaCert != null) {
            ret = putCertInKeyStore(keyStore, caCertName2, mCaCert);
            if (ret == false) {
                if (mClientCertificate != null) {
                    // Remove client key+cert
                    keyStore.delKey(privKeyName2, Process.WIFI_UID);
                    keyStore.delete(userCertName2, Process.WIFI_UID);
                }
                return ret;
            }
        }
        ...
        // Set alias names
        if (mClientCertificate != null) {
            setClientCertificate2Alias(name);
            mClientPrivateKey = null;
            mClientCertificate = null;
        }

        if (mCaCert != null) {
            setCaCertificate2Alias(name);
            mCaCert = null;
        }
        ///@}
    ...
    void removeKeys(KeyStore keyStore) {
        ...
        ///M: for WAPI@{
        String client2 = getFieldValue(CLIENT_CERT_KEY, CLIENT_CERT2_PREFIX);
        // a valid client certificate is configured
        if (!TextUtils.isEmpty(client2)) {
            keyStore.delKey(Credentials.WAPI_USER_CERTIFICATE + client2, Process.WIFI_UID);
            keyStore.delete(Credentials.WAPI_USER_CERTIFICATE + client2, Process.WIFI_UID);
        }
        
        String ca2 = getFieldValue(CA_CERT2_KEY, CA_CERT2_PREFIX);
        // a valid ca2 certificate is configured
        if (!TextUtils.isEmpty(ca2)) {
            keyStore.delete(Credentials.WAPI_AS_CERTIFICATE + ca2, Process.WIFI_UID);
        }
        ///@}
    }
    ...
    void migrateCerts(android.security.KeyStore keyStore) {
        ...
        // a valid client certificate is configured
        if (!TextUtils.isEmpty(client)) {
            if (!keyStore.contains(Credentials.WAPI_USER_CERTIFICATE + client2, Process.WIFI_UID)) {
                keyStore.duplicate(Credentials.WAPI_USER_CERTIFICATE + client2, -1,
                        Credentials.WAPI_USER_CERTIFICATE + client2, Process.WIFI_UID);
                keyStore.duplicate(Credentials.USER_CERTIFICATE + client2, -1,
                        Credentials.USER_CERTIFICATE + client2, Process.WIFI_UID);
            }
        }
        String ca2 = getFieldValue(CA_CERT2_KEY, CA_CERT2_PREFIX);
        // a valid ca certificate is configured
        if (!TextUtils.isEmpty(ca2)) {
            if (!keyStore.contains(Credentials.CA_CERTIFICATE + ca2, Process.WIFI_UID)) {
                keyStore.duplicate(Credentials.CA_CERTIFICATE + ca2, -1,
                        Credentials.CA_CERTIFICATE + ca2, Process.WIFI_UID);
            }
        }
        ///@}
    }
   ...
   /**
     ...
     * @internal
     */
    public void setCaCertificate2Alias(String alias) {
     ...
     * @internal
     */
    public String getCaCertificate2Alias() {
     ...
     * @internal
     */
    public void setClientCertificate2Alias(String alias) {
     ...
     * @internal
     */
    public String getClientCertificate2Alias() {
