                        ...
                        M4OSA_Void* pCallbackUserData)
{
    ...
    M4OSA_TRACE1_3("inside VBcallback w %d, h %d, stride %d", pPlane->u_width, pPlane->u_height, pPlane->u_stride);
                             ...
                             M4OSA_UInt32* pTimeMS, M4OSA_UInt32 tolerance)
{
...
#ifndef ANDROID_DEFAULT_CODE
    M4OSA_TRACE1_1("ThumbnailGetPixels: *pTimeMS=%d", *pTimeMS);
#endif //ANDROID_DEFAULT_CODE
    ...
    if (M4NO_ERROR != err)
    {
        M4OSA_TRACE1_1("ThumbnailGetPixels: err=0x%x", err);
    }

    // Display frame only when decoded successfully.
    if ((M4NO_ERROR == err) && (pC->m_bRender != M4OSA_TRUE)) {
#else
