...
#ifndef ANDROID_DEFAULT_CODE
bool bEnableVeAutotest = false; // ck.wang
#endif
                ...
                M4VSS3GPP_ClipSettings*                       pSettings)
{
    ...
    ALOGD("audioChannels = %d",pSettings->ClipProperties.uiNbChannels);
...
#ifndef ANDROID_DEFAULT_CODE
   fid = pEnv->GetFieldID(clazz,"audioRealDuration","I");
   pSettings->ClipProperties.uiClipAudioRealDuration = pEnv->GetIntField(object,fid);
   M4OSA_TRACE1_1("audioRealDuration = %d",
                   pSettings->ClipProperties.uiClipAudioRealDuration);
#endif
                              ...
                              jobject  thiz)
{
...
#ifndef ANDROID_DEFAULT_CODE
    if (true == bEnableVeAutotest)
    {
        ALOGD("videoEditor_stopPreview: BUSY 1");
        FILE *pFile;
        pFile = fopen("/sdcard/ve.autotest.jni.txt", "a");
        if (0 != pFile)
        {
            fputs ("BUSY 1 videoEditor_stopPreview\r\n",pFile);
            fclose(pFile);
        }
    }
#endif
    ...
    if (true == bEnableVeAutotest)
    {
        ALOGD("videoEditor_stopPreview: BUSY 0 videoEditor_stopPreview");
        FILE *pFile;
        pFile = fopen("/sdcard/ve.autotest.jni.txt", "a");
        if (0 != pFile)
        {
            fputs ("BUSY 0 videoEditor_stopPreview\r\n",pFile);
            fclose(pFile);
        }
    }
#endif
...
#ifndef ANDROID_DEFAULT_CODE
    VIDEOEDIT_LOG_FUNCTION(ANDROID_LOG_INFO, "VIDEO_EDITOR", "videoEditor_stopPreview()done with %d", lastProgressTimeMs);
    ...
    /*+ Handle the image files here */
      if (pContext->pEditSettings->pClipList[iCurrentClipIndex]->FileType ==
        ...
        if (result != M4NO_ERROR) {
            free(pixelArray);
            ThumbnailClose(tnContext);
            ALOGE("videoEditor_renderPreviewFrame: decode result=0x%x", (unsigned int)result);
            return -1;
        }
                ...
                jobject                 audioSettingObject)
{
    ...
    if (audioSettingObject != M4OSA_NULL) {
        ...
        ALOGD("uiNbChannels = %d",pContext->mAudioSettings->uiNbChannels);
    ...
    if (pContext->pEditSettings != NULL)
    {
        ...
        for(int i=0; i<pContext->pEditSettings->uiClipNumber; i++)
        {
            if( (NULL!=pContext->mPreviewController->mOuterClipList)&&(NULL!=&(pContext->mPreviewController->mOuterClipList[i]->ClipProperties)))
            {
                ...
                {
                    pContext->mPreviewController->mOuterClipList[i]->ClipProperties.uiClipAudioDuration =
                        pContext->pEditSettings->pClipList[i]->ClipProperties.uiClipAudioRealDuration;
                    pContext->mPreviewController->mOuterClipList[i]->ClipProperties.uiClipVideoDuration =
                        pContext->pEditSettings->pClipList[i]->ClipProperties.uiClipVideoDuration;
                }

                ALOGD("i = %d uiClipNumber = %d, a %d, v %d", i, pContext->pEditSettings->uiClipNumber,
                    pContext->pEditSettings->pClipList[i]->ClipProperties.uiClipAudioRealDuration,
                    pContext->pEditSettings->pClipList[i]->ClipProperties.uiClipVideoDuration);
            }
        }
                ...
                jboolean                loop)
{
    ...
    if (true == bEnableVeAutotest)
    {
        ALOGD("videoEditor_startPreview: BUSY 1");
        FILE *pFile;
        pFile = fopen("/sdcard/ve.autotest.jni.txt", "a");
        if (0 != pFile)
        {
            fputs ("BUSY 1 videoEditor_startPreview\r\n",pFile);
            fclose(pFile);
        }
    }
#endif
    ...
    if (true == bEnableVeAutotest)
    {
        ...
        if (M4NO_ERROR != result)
        {
            ALOGD("videoEditor_startPreview: ERROR 1");
            if (0 != pFile)
            {
                fputs ("ERROR 1 videoEditor_startPreview\r\n",pFile);
            }
        }

        ALOGD("videoEditor_startPreview: BUSY 0");
        if (0 != pFile)
        {
            fputs ("BUSY 0 videoEditor_startPreview\r\n",pFile);
            fclose(pFile);
        }
    }
#endif
...
#ifndef ANDROID_DEFAULT_CODE
    VIDEOEDIT_LOG_FUNCTION(ANDROID_LOG_INFO, "VIDEO_EDITOR", "videoEditor_startPreview()done");
#endif //ANDROID_DEFAULT_CODE
}
                ...
                jobject                 callback)
{
    ...
    M4OSA_Context       mContext = M4OSA_NULL;
#ifndef ANDROID_DEFAULT_CODE
    VIDEOEDIT_LOG_FUNCTION(ANDROID_LOG_INFO, "VIDEO_EDITOR", "videoEditor_getPixelsList()");
#endif //ANDROID_DEFAULT_CODE
    const char *pString = env->GetStringUTFChars(path, NULL);
    if (pString == M4OSA_NULL) {
        jniThrowException(env, "java/lang/RuntimeException", "Input string null");
    ...
    ALOGV("videoEditor_getPixelsList: startTime=%lld, endTime=%lld, duration=%lld, noOfThumbnails=%d",
        startTime, endTime, duration, noOfThumbnails);
    ...
    for (int i = 0; i < len; i++) {
        ...
        ALOGV("videoEditor_getPixelsList: k=%d, timeMS=%d, i=%d, len=%d", k, timeMS, i, len);
#endif
        err = ThumbnailGetPixels32(mContext, ((M4OSA_Int32 *)m_dst32),
            width, height, &timeMS, tolerance);
        if (err != M4NO_ERROR) {
            ALOGV("videoEditor_getPixelsList: err=0x%x", err);
            break;
        }
#ifndef ANDROID_DEFAULT_CODE
        VIDEOEDIT_LOG_FUNCTION(ANDROID_LOG_INFO, "VIDEO_EDITOR", "ThumbnailGetPixels32() timeMS %d, w:%d, h:%d", timeMS, width, height);
#endif //ANDROID_DEFAULT_CODE
...
#ifndef ANDROID_DEFAULT_CODE
    // Not to throw exception when no new decoded frame to render
    if (err != M4NO_ERROR && err != M4WAR_VIDEORENDERER_NO_NEW_FRAME && !env->ExceptionCheck()) {
#else
...
#ifndef ANDROID_DEFAULT_CODE
    VIDEOEDIT_LOG_FUNCTION(ANDROID_LOG_INFO, "VIDEO_EDITOR", "videoEditor_getPixelsList done");
#endif //ANDROID_DEFAULT_CODE
    return err;
}
                ...
                jstring                             libraryPath)
{
    ...
    property_get("mtk.ve.enable.autotest", value, "0");
    bEnableVeAutotest = (atoi(value) != 0);
#endif
...
videoEditor_generateClip(
    ...
    if (true == bEnableVeAutotest)
    {
        ALOGD("videoEditor_generateClip: BUSY 1");
        FILE *pFile;
        pFile = fopen("/sdcard/ve.autotest.jni.txt", "a");
        if (0 != pFile)
        {
            fputs ("BUSY 1 videoEditor_generateClip\r\n",pFile);
            fclose(pFile);
        }
    }
#endif
    ...
    if (true == bEnableVeAutotest)
    {
        ...
        if (M4NO_ERROR != result)
        {
            ALOGD("videoEditor_generateClip: ERROR 1");
            if (0 != pFile)
            {
                fputs ("ERROR 1 videoEditor_generateClip\r\n",pFile);
            }
        }

        ALOGD("videoEditor_generateClip: BUSY 0");
        if (0 != pFile)
        {
            fputs ("BUSY 0 videoEditor_generateClip\r\n",pFile);
            fclose(pFile);
        }
    }
#endif
