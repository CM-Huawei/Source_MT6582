...
public class AudioTrack {
    ...
    AudioTrack(VideoEditor editor, String audioTrackId, String filename,
        ...
        switch (mMANativeHelper.getAudioCodecType(properties.audioFormat)) {
            ...
            case MediaProperties.ACODEC_AAC_PLUS:/// M: add AAC plus
            case MediaProperties.ACODEC_ENHANCED_AAC_PLUS:/// M: add enhanced AAC plus
                break;
            default:
        ...
        if (audioWaveformFilename != null) {
            try {
                ...
                invalidate();
                Log.w(TAG, "Can not get waveform data from " + audioWaveformFilename, e);
            }
        } else {
            mWaveformData = null;
        }
    ...
    throws IOException {
        if (mAudioWaveformFilename == null) {
            ...
            switch (codecType) {
                ...
                case MediaProperties.ACODEC_ENHANCED_AAC_PLUS: {
                    frameDuration = (MediaProperties.SAMPLES_PER_FRAME_AAC * 1000 * 2)
                    / MediaProperties.DEFAULT_SAMPLING_FREQUENCY;
                    sampleCount = MediaProperties.SAMPLES_PER_FRAME_AAC * 2;
                    break;
                }
                /// @}
