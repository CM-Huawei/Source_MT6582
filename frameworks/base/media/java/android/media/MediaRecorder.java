...
public class MediaRecorder
{
    ...
    {
        Xlog.w(TAG, "setAuxiliaryOutputFile(FileDescriptor) is no longer supported.");
    }
    ...
    {
        Xlog.w(TAG, "setAuxiliaryOutputFile(String) is no longer supported.");
    }
    ...
    public void prepare() throws IllegalStateException, IOException
    {
        ...
        if (FeatureOption.MTK_MOBILE_MANAGEMENT) {
            Xlog.d(TAG, "prepare>>>: check permission");
            ...
            try {
                if (moms != null && (moms.checkPermission(permission, uid) != PackageManager.PERMISSION_GRANTED)) {
                    Xlog.d(TAG, "prepare<<<: user denied permission " + permission + " for uid " + uid);
                    throw new IOException();
                }
                Xlog.d(TAG, "prepare<<<: user granted permission " + permission + " for uid " + uid);
    ...
    private class EventHandler extends Handler
    {
        ...
        public void handleMessage(Message msg) {
            if (mMediaRecorder.mNativeContext == 0) {
                Xlog.w(TAG, "mediarecorder went away with unhandled events");
                return;
            }
            switch(msg.what) {
            ...
            default:
                Xlog.e(TAG, "Unknown message type " + msg.what);
                return;
            }
        }
    }
