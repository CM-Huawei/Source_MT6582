...
void TouchInputMapper::dispatchVirtualKey(nsecs_t when, uint32_t policyFlags,
    ...
    ALOGD_READER("Latency info debug, dispatchVirtualKey now(ms): %lld",now/1000000);
...
void TouchInputMapper::dispatchTouches(nsecs_t when, uint32_t policyFlags) {
    ...
    if (currentIdBits == lastIdBits) {
        if (!currentIdBits.isEmpty()) {
            ...
            ALOGD_READER("Latency info debug, MOVE now(ms): %lld",now/1000000);
        ...
        while (!upIdBits.isEmpty()) {
	    	...
	    	ALOGD_READER("Latency info debug, POINTER UP now(ms): %lld",now/1000000);
            dispatchMotion(when, policyFlags, mSource,
        ...
        if (moveNeeded) {
            ...
            ALOGD_READER("Latency info debug, POINTER MOVE, now(ms): %lld",now/1000000);
            dispatchMotion(when, policyFlags, mSource,
        ...
        while (!downIdBits.isEmpty()) {
            ...
            ALOGD_READER("Latency info debug, POINTER DOWN now(ms): %lld",now/1000000);
            dispatchMotion(when, policyFlags, mSource,
...
void MultiTouchInputMapper::syncTouch(nsecs_t when, bool* outHavePointerIds) {
    ...
    for (size_t inIndex = 0; inIndex < inCount; inIndex++) {
		...
		if(outPointer.id!= trackingId){
			ALOGW("outer ID:%d, tracking ID:%d, pointer count:%d",outPointer.id, trackingId, inCount);
		}
        }
	...
	if(outCount != inCount){
		ALOGW("inCount:%d, pointer count:%d", inCount, outCount);
	}
