...
class AppWidgetService extends IAppWidgetService.Stub
{
    ...
    public void systemRunning(boolean safeMode) {
        ...
        /// M: Register for SMB toggle
        if (FeatureOption.MTK_SMARTBOOK_SUPPORT) {
            IntentFilter sbFilter = new IntentFilter();
            sbFilter.addAction(Intent.ACTION_SMARTBOOK_PLUG);
            mContext.registerReceiver(new SmartBookToggleReceiver(), sbFilter, null, null);	
        }
        /// @}
    ...
    /// M: add for smartbook
    private final class SmartBookToggleReceiver extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {
            ...
            Slog.d(TAG, "SmartBookToggleReceiver: sbPlugState=" + sbPlugState);
            for (int i = 0; i < mAppWidgetServices.size(); i++) {
                AppWidgetServiceImpl service = mAppWidgetServices.valueAt(i);
                service.onSmartBookToggle(sbPlugState);
            }
        }
    }
}
