...
final class Notifier {
    ...
    private void updatePendingBroadcastLocked() {
        if (DEBUG) {
            Slog.d(TAG, "updatePendingBroadcastLocked mBroadcastInProgress = " + mBroadcastInProgress
                        + ", mActualPowerState = " + mActualPowerState + ", mPendingWakeUpBroadcast = " + mPendingWakeUpBroadcast
                        + ", mPendingGoToSleepBroadcast = " + mPendingGoToSleepBroadcast + ", mBroadcastedPowerState = " + mBroadcastedPowerState);
        }
    ...
    private void sendWakeUpBroadcast() {
        if (DEBUG) {
            Slog.d(TAG, "Sending wake up broadcast - screenTurningOn");
        }
        ...
        if (DEBUG) {
            Slog.d(TAG, "Sending wake up broadcast - ActivityManagerNative wakingUp");
        }
        ...
        if (ActivityManagerNative.isSystemReady()) {
            if (DEBUG) {
                Slog.d(TAG, "Sending wake up broadcast - sendOrderedBroadcastAsUser");
            }
    ...
    private void sendGoToSleepBroadcast(int reason) {
        ...
        if (DEBUG) {
            Slog.d(TAG, "Sending go to sleep broadcast - ActivityManagerNative goingToSleep");
        }
        ...
        if (ActivityManagerNative.isSystemReady()) {
            if (DEBUG) {
                Slog.d(TAG, "Sending go to sleep broadcast - sendOrderedBroadcastAsUser");
            }
