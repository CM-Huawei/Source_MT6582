...
public class LocationManagerService extends ILocationManager.Stub {
    ...
    // MTK add [ALPS00489982] Dynamic Bind Fused Location Provider
    private LocationProviderProxy mFusedLocationProvider = null;;
    //MTK end
        ...
        mContext.getContentResolver().registerContentObserver(
                ...
                new ContentObserver(mLocationHandler) {
                    ...
                    public void onChange(boolean selfChange) {
                        synchronized (mLock) {
                            if (D) Log.d(TAG, "LOCATION_PROVIDERS_ALLOWED : " + Settings.Secure.LOCATION_PROVIDERS_ALLOWED);
                            updateProvidersLocked();
                        }
                    }
                }, UserHandle.USER_ALL);
    ...
    private void locationIPOcreateProvider(){
        ...
        */
        ...
        GeofenceProxy provider = GeofenceProxy.createAndBind(mContext,
                ...
                com.android.internal.R.array.config_locationProviderPackageNames,
                mLocationHandler,
                ((GpsLocationProvider)mRealProviders.get(LocationManager.GPS_PROVIDER)).getGpsGeofenceProxy(),
                flpHardwareProvider.getGeofenceHardware());
        if (provider == null) {
            Slog.e(TAG,  "no geofence provider found");
        }
    ...
    private LocationProviderProxy loadFusedLocationProvider() {
        ...
        if (pkgs != null) providerPackageNames.addAll(Arrays.asList(pkgs));
        ensureFallbackFusedProviderPresentLocked(providerPackageNames);
        LocationProviderProxy fusedLocationProvider = null;
        Log.d(TAG, "load FUSED_PROVIDER");
        if(mFusedLocationProvider != null) {
            fusedLocationProvider = mFusedLocationProvider;
        } else {
            fusedLocationProvider = LocationProviderProxy.createAndBind(
                mContext,
                LocationManager.FUSED_PROVIDER,
                FUSED_LOCATION_SERVICE_ACTION,
                com.android.internal.R.bool.config_enableFusedLocationOverlay,
    ...
    private void unloadFusedLocationProvider() {
        ...
        if (mFusedLocationProvider != null) {
            ...
            LocationProviderInterface realProvider = mRealProviders.get(LocationManager.FUSED_PROVIDER);
            if(realProvider != null) {
                LocationProviderProxy fusedProvider = (LocationProviderProxy)realProvider;
                mRealProviders.remove(LocationManager.FUSED_PROVIDER);
                mEnabledProviders.remove(LocationManager.FUSED_PROVIDER);
                mProxyProviders.remove(fusedProvider);
                removeProviderLocked(realProvider);
            }
            ...
            Slog.e(TAG, "unload fused location fail Provider = null");
        }
    }
    ...
    private void switchUser(int userId) {
        //MTK add in case of double init
        if (mCurrentUserId == userId) {
            Log.d(TAG, "user id no changes " + userId);
            return;
        }
    ...
    public void requestLocationUpdates(LocationRequest request, ILocationListener listener,
        ...
        //moms permision check
        if(FeatureOption.MTK_MOBILE_MANAGEMENT == true) {
            try {
                ...
                int result = mMobileManager.checkPermission(SubPermissions.ACCESS_LOCATION,uid);
                Log.d(TAG, "moms requestLocationUpdates pkg:" + packageName + " uid:" + uid + " result:" + result);
                if(result != PackageManager.PERMISSION_GRANTED) {
                    return;
                }
            } catch (RemoteException e) {
                Log.e(TAG, "chcek permission failed :" +  SubPermissions.ACCESS_LOCATION + " package:" + packageName);
                return;
            }
        }
        ...
        //MTK add [ALPS00489982] Dynamic Bind Fused Location Provider
        if (FeatureOption.MTK_LCA_RAM_OPTIMIZE) {
            Log.d(TAG, "isFused ="+ request.getProvider().equals(LocationManager.FUSED_PROVIDER) + " isInited=" + ((mFusedLocationProvider==null)?false:true) );
    ...
    public Location getLastLocation(LocationRequest request, String packageName) {
        ...
        if(FeatureOption.MTK_MOBILE_MANAGEMENT == true) {
            try {
                IBinder binder = ServiceManager.getService(Context.MOBILE_SERVICE);
                IMobileManagerService mMobileManager = IMobileManagerService.Stub.asInterface(binder);
                int result = mMobileManager.checkPermission(SubPermissions.ACCESS_LOCATION,Binder.getCallingUid());
                Log.d(TAG, "moms getLastLocation pkg:" + packageName + " result:" + result);
                if(result != PackageManager.PERMISSION_GRANTED) {
                    return null;
                }
            } catch (RemoteException e) {
        ...
        try {
            ...
            synchronized (mLock) {
                ...
                //MTK add [ALPS00489982] Dynamic Bind Fused Location Provider
                if (FeatureOption.MTK_LCA_RAM_OPTIMIZE && (name == LocationManager.FUSED_PROVIDER) ) {
                    Log.d(TAG, "allow query mlastlocation");
                } else {
