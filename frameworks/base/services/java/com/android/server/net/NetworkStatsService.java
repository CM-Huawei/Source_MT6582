            ...
            public long getMobileTotalBytes(
                ...
                if ( FeatureOption.EVDO_DT_SUPPORT || FeatureOption.MTK_DT_SUPPORT || FeatureOption.MTK_GEMINI_SUPPORT ){
                    ...
                    if(iTelephonyEx!=null){
                         try{
                             subscriberId= iTelephonyEx.getSubscriberId(subSim);
                             templete = buildTemplateMobileAll(subscriberId);
                             totalBytes = getNetworkTotalBytes(templete, start_time, end_time);
                        }catch (RemoteException e) {
                            Slog.v(TAG, "getMobileTotalBytes Itelphony is not ready");
                        }
                    }
                }else{ //always return slot 1
                    ...
                    if (telephony.getSimState() != SIM_STATE_READY) {
                        Slog.v(TAG,"getMobileTotalBytes sim is not ready");
                        return 0;
                    }
                    if(subsId==null){
                        Slog.v(TAG,"getMobileTotalBytes subId=null");
                        return 0;
                    }
                    templete = buildTemplateMobileAll(subscriberId);
                    totalBytes = getNetworkTotalBytes(templete, start_time, end_time);
                }
                Slog.v(TAG, "getMobileTotalBytes subSim="+subSim+" subscriberId"+subscriberId+" start_time="+start_time+" end_time"+end_time+" totalBytes="+totalBytes);
                return totalBytes;
            }
