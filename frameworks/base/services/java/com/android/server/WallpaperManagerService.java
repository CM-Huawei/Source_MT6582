...
class WallpaperManagerService extends IWallpaperManager.Stub {
    ...
    public void setWallpaperComponent(ComponentName name) {
        ...
        synchronized (mLock) {
            ...
            try {
                wallpaper.imageWallpaperPending = false;
                /// M: force to create a new window when Stereo3DWallpaperService changes to a different wallpaper 
                /// in order to avoid half black screen shown during orientation transition @{
                if ("com.mediatek.stereo3dwallpaper.Stereo3DWallpaperService".equals(name.getClassName())
                    || "com.mediatek.stereo3dwallpaper.Stereo3DPairWallpaperService".equals(name.getClassName())) {
                    if (DEBUG) Slog.v(TAG, "Force to create a new window for " + name.getClassName());
                    bindWallpaperComponentLocked(name, true, true, wallpaper, null);
                } else {
