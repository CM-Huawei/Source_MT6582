...
public class WindowAnimator {
    ...
    private void updateWindowsLocked(final int displayId) {
        ...
        /// M: Handle Keyguard sub windows visibility
        ArrayList<WindowState> keyguardSubWindows = null;
        ...
        for (int i = windows.size() - 1; i >= 0; i--) {
            ...
            if (winAnimator.mSurfaceControl != null) {
                ...
                /// M: Handle Keyguard sub windows visibility @{
                        ...
                        mPolicy.doesForceHide(win.mAttachedWindow, win.mAttachedWindow.mAttrs)) {
                    if (keyguardSubWindows == null) {
                        keyguardSubWindows = new ArrayList<WindowState>();
                    }
                    keyguardSubWindows.add(win);
                /// @}
        ...
        /// M: Handle Keyguard sub windows visibility @{
        if (keyguardSubWindows != null) {
            if (mForceHiding == KEYGUARD_SHOWN ||
                    mForceHiding == KEYGUARD_ANIMATING_IN ||
                    mForceHiding == KEYGUARD_ANIMATING_OUT) {
                for (int i = keyguardSubWindows.size() - 1; i >= 0; i--) {
                    final WindowState win = keyguardSubWindows.get(i);
    ...
    private void updateWallpaperLocked(int displayId) {
        ...
        for (int i = windows.size() - 1; i >= 0; i--) {
            ...
            if (appAnimator != null && appAnimator.animation != null
                ...
                /// M: Fix ALPS01267762, due to dialog activity set its background dim layer alpha to 1.0.
                /// We should check if this window is fullscreen before setAnimationBackground @{
                final DisplayInfo displayInfo = win.mDisplayContent.getDisplayInfo();
                int dw = displayInfo.logicalWidth;
                int dh = displayInfo.logicalHeight;
                /// @}

                if (color != 0 && win.isFullscreen(dw, dh) && win.mAttrs.format != PixelFormat.TRANSPARENT) {
                    win.getStack().setAnimationBackground(winAnimator, color);
                }
            }
        } // end forall windows
                ...
                if (mService.mSbParams.getPlugState() == WindowManagerService.SmartBookParams.SMARTBOOK_HAS_PLUGGED_IN) {
                    if (mService.mSbParams.checkWindowInSmartBookWhiteList()) {
                        if (!mService.IS_USER_BUILD)
                            Slog.d(TAG, "Win:" + mService.mSbParams.mTurnOnScreenWin + " display ready, call sbWakeUp");
                        mService.mPowerManager.sbWakeUp(SystemClock.uptimeMillis());
                    }
                }
