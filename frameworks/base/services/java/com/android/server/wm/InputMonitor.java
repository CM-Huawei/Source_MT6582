    ...
    public long notifyANR(InputApplicationHandle inputApplicationHandle,
        ...
        if (appWindowToken != null && appWindowToken.appToken != null) {
            try {
                ...
                //     int userId = (windowState != null) ? UserHandle.getUserId(windowState.mOwnerUid) : UserHandle.USER_NULL;

                //     long extendTime = ANRManager.extendANRTime(userId, appWindowToken.inputDispatchingTimeoutNanos/MILLI_TO_NANO);
                //     if (extendTime > 0) {
                ...
                if ( ANRManager.DISABLE_ALL_ANR_MECHANISM != ANRManager.enableANRDebuggingMechanism()) {
                    ...
                    {
                        abort = appWindowToken.appToken.keyDispatchingTimedOut(reason);
                    }
                    /// @}
                }
                else {
            ...
            try {
                ...
                if(FeatureOption.MTK_MOBILE_MANAGEMENT) {
                    ...
                    long extendTime = ANRManager.extendANRTime(userId, DEFAULT_INPUT_DISPATCHING_TIMEOUT_NANOS/MILLI_TO_NANO);
                    if (extendTime > 0) {
                        Slog.w(WindowManagerService.TAG, "Skip INPUT_DISPATCH_TIMEOUT ANR due to user confirm blocking");
                        return extendTime*MILLI_TO_NANO;
                    }
