...
public final class DisplayManagerService extends IDisplayManager.Stub {
    ...
    /// MTK: for WFD request @{
    /**
     * Called by the power manager to blank specified display.
     *
     *@param displayId The display need to blank
     */
    public void blankDisplayFromPowerManager(int displayId) {
        synchronized (mSyncRoot) {
            LogicalDisplay display = mLogicalDisplays.get(displayId);
            if (display != null) {
   ...
   /// MTK: for WFD request @{
    /**
     * Called by the power manager to unblank specified display.
     *
     *@param displayId The display need to unblank
     */
     public void unblankDisplayFromPowerManager(int displayId) {
         synchronized (mSyncRoot) {
             LogicalDisplay display = mLogicalDisplays.get(displayId);
             if (display != null) {
                 DisplayDevice device = display.getPrimaryDisplayDeviceLocked();
   ...
   ///@}
    ...
    private void registerWifiDisplayAdapterLocked() {
        if (mContext.getResources().getBoolean(
                ...
                || FeatureOption.MTK_WFD_SUPPORT) { /*M: add for wifidisplay */
    ...
    private void updateDisplayBlankingLocked(DisplayDevice device) {
        ...
        Slog.d(TAG, "updateDisplayBlankingLocked for " + info.name + ", flag=" + info.flags);
        Slog.d(TAG, "mAllDisplayBlankStateFromPowerManager: " + mAllDisplayBlankStateFromPowerManager);
    ...
    private void addLogicalDisplayLocked(DisplayDevice device) {
        ...
        if (FeatureOption.MTK_SMARTBOOK_SUPPORT) {
            if (deviceInfo.subtype == DisplayDeviceInfo.EXTRA_TYPE_SMARTBOOK) {
                ...
                Slog.d(TAG, "Extra display " + deviceInfo.subtype + " plugged and display change to " + out.appHeight + " x " + out.appWidth + ", density:" + out.logicalDensityDpi);
                mWindowManagerFuncs.requestExtraDisplayDeviceRefresh(out.appHeight, out.appWidth, 160);
            ...
            if (!display.isValidLocked()) {
                ...
                if (FeatureOption.MTK_SMARTBOOK_SUPPORT) {
                    if (displayId != mExtraDisplayId) {
                        ...
                        mWindowManagerFuncs.requestExtraDisplayDeviceRefresh(0, 0, 0);
    ...
    public interface WindowManagerFuncs {
        ...
        /**
         * M:[SmartBook]
         * Request window manager to aware new extra display device added/removed and refresh size and density
         */
        void requestExtraDisplayDeviceRefresh(int width, int height, int density);
    }
