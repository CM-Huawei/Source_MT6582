        ...
        public void onDisplayDisconnecting() {    
            ...
            new Thread(new Runnable() {
                public void run() {
                    ...
                    mInDisconnectingThread = true;
                    while (true) {
                        // thread life time: max is 30*333 ms = 10 seconds
                        if (30 > counter && null != mActiveDisplay) {
                            Slog.i(TAG, "WfdDisConnThread: " + counter*333 + " ms");
            
                            if (0 == counter%2) {
                                iSmallIcon = com.mediatek.internal.R.drawable.ic_notify_wifidisplay_blink;
                            } else {
                                iSmallIcon = R.drawable.ic_notification_cast_on;
                            }
                            Notification notification = new Notification.Builder(context)
                            .setSmallIcon(iSmallIcon)
                            .setOngoing(true)
                            .build();
                    ...
                    mInDisconnectingThread = false; 

                }
            }, "WfdDisConnThread").start();
        }
        ///@}
    };
