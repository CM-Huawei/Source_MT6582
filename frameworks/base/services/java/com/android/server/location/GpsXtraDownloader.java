...
/// M: Ignore Http request WAP @{
import com.mediatek.common.featureoption.FeatureOption;
import com.mediatek.eposervice.EPOinfo;
/// }@
...
public class GpsXtraDownloader {
    ...
    GpsXtraDownloader(Context context, Properties properties) {
        ...
        if(FeatureOption.MTK_GPS_SUPPORT) {
            if(getURLwithSSLKey() != null) {
                Log.e(TAG, "getURLwithSSLKey Support retry:" + RETRY_TIMES);
                mNextServerIndex = RETRY_TIMES;
                mXtraServers = new String[RETRY_TIMES];
            } else {
                Log.e(TAG, "Warning: getURLwithSSLKey Not Support");
            }
        } else {
        /// }@
    ...
    byte[] downloadXtraData() {
        ...
        while (result == null) {
            /// M: Add URL with SSLKey @{
            if(FeatureOption.MTK_GPS_SUPPORT) { 
                result = doDownload(getURLwithSSLKey(), useProxy, proxyHost, proxyPort);
                mNextServerIndex--;
                if (mNextServerIndex <= 0) break;
            } else {/// }@
    ...
    protected static byte[] doDownload(String url, boolean isProxySet, 
        ...
        if (DEBUG) Log.d(TAG, "Downloading XTRA data from " + getKeyLog(10,url));
    ...
    String getURLwithSSLKey() {

        /// M: Ignore Http request WAP @{
        String url = "";
        if(FeatureOption.MTK_GPS_SUPPORT) {
            EPOinfo info = new EPOinfo();
            url = info.getSignedURL();
        }
        /// }@
        return url;
    }

    protected static String getKeyLog(int length, String TargetURL) {
        /// M: Ignore Http request WAP @{
        String url = new String(TargetURL);
        if(url.contains("key=")) {
            int index = url.indexOf("key=");
            url = url.substring(index+4,index+4+length);
        }
        /// }@
        return url;
    }
