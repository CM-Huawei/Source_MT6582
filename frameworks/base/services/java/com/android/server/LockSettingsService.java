...
public class LockSettingsService extends ILockSettings.Stub {
    ...
    private class DbCache {    
        ...
        public void clear(String key, int userId) {
            if(DEBUG_CACHE_PRINT) {
                Slog.i(TAG, "DbCache.clear() key = " + key + "userId = " + userId) ;
            }

            String cacheKey = key + Integer.toString(userId) ;
            mCache.remove(cacheKey);
        }
    
        public String get(String key, int userId) {            
            String cacheKey = key + Integer.toString(userId) ;            
            String value = mCache.get(cacheKey); 

            if(DEBUG_CACHE_PRINT) {
                Slog.i(TAG, "DbCache.get() key = " + key + " userId = " + userId + "value = " + value) ;
            }           
            
            return value ;
        }
    
        public void put(String key, int userId, String value) {
            if(DEBUG_CACHE_PRINT) {                
                Slog.i(TAG, "DbCache.put() key = " + key + " userId = " + userId + " value = " + value ) ;
            }
            
            String cacheKey = key + Integer.toString(userId) ;                        
            mCache.put(cacheKey, value);
        }
    }
    private DbCache mLockSettingCache ;
    private static final boolean DEBUG_CACHE_PRINT = false ;
    private static final boolean USE_DBCACHE = true ;
    private static String NULL_CACHED_VALUE = "NOT_EXIST_YET" ;
    ...
    private void writeToDb(SQLiteDatabase db, String key, String value, int userId) {
        if(USE_DBCACHE) {
            if(DEBUG_CACHE_PRINT) {
            Slog.i(TAG, "writeToDb() value = " + value) ;
        }
        
        /// M: Cache hits. 
        String cachedResult = mLockSettingCache.get(key,userId);
        if(cachedResult != null && cachedResult.equals(value)) {
                if(DEBUG_CACHE_PRINT) {
                Slog.i(TAG, "chache hits, The value = " + value + " already exists in cache.Skip write to db.") ;
            }
            return ;
        }
        }
        ...
        if(USE_DBCACHE) {
        /// M: To optimize the performance, we use cache here to get value if existed.
        mLockSettingCache.put(key,userId,value) ;
        }
    }
    ...
    private String readFromDb(String key, String defaultValue, int userId) {
        ...
        if(cachedResult != null) {
                if(DEBUG_CACHE_PRINT) {
                Slog.i(TAG, "chache hits, cachedResult = " + cachedResult) ;
            }

                if(cachedResult.equals(NULL_CACHED_VALUE)) {
                    // here means that the key/userId had been queried again, 
                    // so we should return default value since key/userId does not exist in DB yet.
                    return defaultValue ;                
                }
                else {
            return cachedResult ;
        ...
        if ((cursor = db.query(TABLE, COLUMNS_FOR_QUERY,
                ...
                null, null, null)) != null) {
            /// M: [ALPS00712408] Use try-catch to prevent from DB exception
            try {
                if (cursor.moveToFirst()) {
                    result = cursor.getString(0);
                }
            } catch (Exception e) {
                Slog.e(TAG, "Cannot read Db " + e);
            } finally {
                cursor.close();
            }
        }
        ...
        if(USE_DBCACHE) {
        /// M: Cache miss will reach here. This will happen if reading the data written in the last boot.
        if(result != null) {
            mLockSettingCache.put(key,userId,result) ;
            } 
            else {
                /// M: we should also remember the "result = null" case.
                mLockSettingCache.put(key,userId,NULL_CACHED_VALUE) ;
            }
        }
