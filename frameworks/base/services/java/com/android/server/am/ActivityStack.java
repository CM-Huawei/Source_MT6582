...
final class ActivityStack {
    ...
    final boolean finishActivityLocked(ActivityRecord r, int resultCode, Intent resultData,
        ...
        if (r.processName.equals("com.android.email")) {
            boolean updatePreviousProcess = true;
            for (int idx = activities.size()-1; idx >= 0; idx--) {
                ActivityRecord next = activities.get(idx);
                if (!next.finishing) {
                    updatePreviousProcess = false;
                    break;
                }
            }
            if (updatePreviousProcess && mService.mPreviousProcess2 == r.app) {
                Slog.d(TAG, "updatePreviousProcess = " + updatePreviousProcess + " mService.mPreviousProcess2 = " + mService.mPreviousProcess2);
                mService.mPreviousProcess2 = null;
            }
        }
        /// @}
    ...
    final boolean moveTaskToBackLocked(int taskId, ActivityRecord reason) {
        ...
        if (task == tr && task.mOnTopOfHome || numTasks <= 1) {
            /// M: ALPS01236387, Fix google KK issue, prevent task null pointer exception @{ 
            if (task != null) {
