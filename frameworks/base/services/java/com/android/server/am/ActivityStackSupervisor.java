...
public final class ActivityStackSupervisor {
    ...
    static final int HALT_RESUMING_TIMEOUT_MSG = FIRST_SUPERVISOR_STACK_MSG + 5;
    static final int RESTORE_RESUMING_MSG = FIRST_SUPERVISOR_STACK_MSG + 6;
    /// @}
    ...
    final int startActivityMayWait(IApplicationThread caller, int callingUid,
        ...
        if (FeatureOption.MTK_MOBILE_MANAGEMENT) {
            if (aInfo != null && aInfo.permission != null) {
                Slog.d(TAG, "startActivityMayWait() with permission: " + aInfo.permission);
                try {
                    Bundle data = new Bundle();
                    String permission = aInfo.permission;
                    IMobileManagerService mMobileManager = null;
                    ...
                    if (permission.equals("android.permission.CALL_PHONE") && intent != null) {
                        permission = "sub-permission.MAKE_CALL";
                        String phoneNumber = PhoneNumberUtils.getNumberFromIntent(intent, mContext);
                        data.putString(IMobileManager.PARAMETER_PHONENUMBER, phoneNumber);
                    }
                    int result = mMobileManager.checkPermissionWithData(permission, Binder.getCallingUid(), data);
                    if (result != PackageManager.PERMISSION_GRANTED) {
                        Slog.e(TAG, "startActivity() is not granted with permission: " +
                                aInfo.permission + " from uid: " + Binder.getCallingUid());
                        return ActivityManager.START_PERMISSION_USER_DENIED;
                    }
                } catch (Exception e) {
                    Slog.e(TAG, "Failed to chcek permission: " +
                            aInfo.permission + " with uid: " + Binder.getCallingUid());
    ...
    final int startActivityLocked(IApplicationThread caller,
        ...
        if (resultRecord != null && resultRecord.processName.equals("com.android.email") && mService.mPreviousProcess2 != resultRecord.app) {            
            Slog.d(TAG, "mPreviousProcess2 process = " + resultRecord.app);
            mService.mPreviousProcess2 = resultRecord.app;
        }
        /// @}
        ...
        if (r.resultTo == null && !addingToTask
            ...
            if (!addingToTask &&
                ...
                if (top != null) {
                    ...
                    top.setTask(sourceTask, sourceRecord.thumbHolder, false);
    ...
    void ensureActivitiesVisibleLocked(ActivityRecord starting, int configChanges) {
        ...
        synchronized (mHaltActivityLock) {
            final ActivityStack topStack = getFocusedStack();
            final ActivityRecord top = topStack.topRunningActivityLocked(null);
            if (mHaltActivity && top != null && top.state != ActivityState.RESUMED && checkIfBlockResumeTop(top)) {
