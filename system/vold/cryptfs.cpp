...
static void upgrade_crypt_ftr(int fd, struct crypt_mnt_ftr *crypt_ftr, off64_t offset)
{
    ...
    if ((crypt_ftr->major_version == 1) && (crypt_ftr->minor_version == 0)) {
        ...
        pdata = (struct crypt_persist_data *)malloc(CRYPT_PERSIST_DATA_SIZE);
        if (pdata == NULL) {
...
static int load_persistent_data(void)
{
    ...
    if (persist_data == NULL) {
        pdata = (struct crypt_persist_data *)malloc(crypt_ftr.persist_data_size);
        if (pdata == NULL) {
    ...
    if (i < WAIT_UNMOUNT_COUNT) {
      ...
      while (retries--) {
         ...
         if (retries == 1) { 
            ...
            int ret;
            if ((ret = system(umount_cmd)) < 0)
            {
                SLOGE("execute '%s' command, got error return code:%d", umount_cmd, ret);                      
            }
         }
         else {            
...
int cryptfs_restart(bool is_ipo_reboot, int boot_reason)
{
    ...
    if (! (rc = wait_and_unmount(DATA_MNT_POINT)) ) {
        if (wait_and_unmount("/protect_s")) {
            goto cryptfs_restart_error;
        }
        if (wait_and_unmount("/protect_f")) {
            goto cryptfs_restart_error;
        }
        if (is_ipo_reboot) {
                ...
                if (fs_mgr_do_tmpfs_mount(DATA_MNT_POINT)) {
                    ...
                    SLOGD("Successful: mount tmpfs for '%s'\n", DATA_MNT_POINT);
                     if (fs_mgr_do_tmpfs_mount("/protect_s")) {
                        SLOGE("Fail: mount tmpfs for '/protect_s' , errno=%d\n", errno);
                        goto cryptfs_restart_error;
                     }
                     if (fs_mgr_do_tmpfs_mount("/protect_f")) {
                        SLOGE("Fail: mount tmpfs for '/protect_f' , errno=%d\n", errno);
                        goto cryptfs_restart_error;
                     }
                }   
        }
               ...
               if (fs_mgr_do_mount(fstab, DATA_MNT_POINT, crypto_blkdev, 0)) {
                    ...
                    SLOGD("Successful: mount crypto_blkdev\n");
                     if (fs_mgr_do_mount(fstab, "/protect_s", "/emmc@protect_s", 0)) {
                         SLOGE("Fail: mount /protect_s, errno=%d\n", errno);
                         goto cryptfs_restart_error;
                     }

                     if (fs_mgr_do_mount(fstab, "/protect_f", "/emmc@protect_f", 0)) {
                         SLOGE("Fail: mount /protect_f, errno=%d\n", errno);
                         goto cryptfs_restart_error;
                     }
                }
        }
