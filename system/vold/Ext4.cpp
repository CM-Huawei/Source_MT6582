...
int Ext4::autoRepair(const char *fsPath) {
    ...
    rc = android_fork_execvp(ARRAY_SIZE(args), (char **)args, &status, false,
            true);
    if (rc != 0) {
        SLOGE("Filesystem (ext4) auto-repair failed due to logwrap error");
        errno = EIO;
        return -1;
    }

    if (!WIFEXITED(status)) {
        SLOGE("Filesystem (ext4) auto-repair did not exit properly");
        errno = EIO;
        return -1;
    }
    ...
    if (status == 0) {
        ...
        SLOGE("Auto-repair (ext4) failed (unknown exit code %d)", status);
        errno = EIO;
        return -1;
    }
    return 0;
