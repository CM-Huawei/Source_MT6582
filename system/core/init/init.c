...
int add_environment(const char *key, const char *val)
{
    ...
    for (n = 0; n < 31; n++) {
        if (ENV[n]) {
            if((ENV[n][strlen(key)]== '=') && (!strncmp(ENV[n], key, strlen(key)))) {
               break;
            }
        }
    }
    if(n < 31) {        
        ...
        NOTICE("%s: environment '%s' is over-written to '%s'\n", __FUNCTION__, ENV[n], entry);
        free(ENV[n]);
        ENV[n] = entry;     
        return 0;
    }
...
void execute_one_command(void)
{
    ...
    for(i=0 ;i<cur_command->nargs ; i++){
        if(buffer_size < strlen(buf) + strlen(cur_command->args[i])+1)
            break;
        snprintf( buf , buffer_size , "%s %s", buf, cur_command->args[i]);
    }
    NOTICE("command r=%d '%s'\n", ret, buf);
#else
...
/* end: multi-boot mode */
/* NEW FEATURE: dynamic DRAM size detect/config */
#define MAX_LINE 256
#define LOW_MEM_SIZE_KB 524288
...
static int queue_com_triggers_action()
{
    ...
    //    || (boot_mode == 6)) {
    if (((meta_com_id & 0x0001) != 0) && (boot_mode == 1 || boot_mode == 6 || boot_mode == 4))
    {
...
static void get_mobile_log_config()
{
    int meta_com_id = 0;

    meta_com_id = get_meta_com_id();

    /* If enter META mode and meta_com_id & 0x0002 == 1, disable mobile_log_d service, else enable mobile_log_d service
     * Please reference meta_init.rc
     */
    if ( (meta_com_id & 0x0002) != 0 )
    {
        property_set("sys.mtk.disable.mobile_log_d", "1");
    } else {
		property_set("sys.mtk.disable.mobile_log_d", "0");
    }
}
#endif
...
#ifdef MTK_INIT
    /* check if mem < 512MB and set property*/
    get_meminfo();
#endif
...
/**** End of Parsing .rc files ****/
    ...
    if ( (mt_boot_mode == MT_META_BOOT) ||(mt_boot_mode == MT_FACTORY_BOOT) || (mt_boot_mode == MT_ATE_FACTORY_BOOT) )      
    {
