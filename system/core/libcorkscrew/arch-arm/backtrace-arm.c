...
static bool try_pop_Stack_ForNotSaveInASM(const memory_t* memory, unwind_state_t* state,size_t frames) 
{
		uint32_t sp = state->gregs[R_SP];
		bool sp_updated = false;
		uint32_t TempValue=0;
		if (!try_get_word(memory, state->gregs[R_SP]-4, &TempValue)) 
		{
				return false;
		}
		...
		if (sp_updated) 
		{
				ALOGD("try_pop_Stack_ForNotSaveInASM tid=%d,frame:%d,sp=0x%08x",(int)(memory->tid),(int)frames,sp+4);
...
static ssize_t unwind_backtrace_common(const memory_t* memory,
    ...
    for (size_t index = 0; returned_frames < max_depth; index++) {
		...
		try_pop_Stack_ForNotSaveInASM(memory, state,returned_frames);
...
#ifdef HAVE_AEE_FEATURE
        bool ret;
        ret = aee_ioctl_get_reg(tid, &regs);
        if (!ret)
            return -1;
#else
        return -1;
