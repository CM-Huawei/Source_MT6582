...
static int usb_adb_write(usb_handle *h, const void *data, int len)
{
    ...
    if(bitdebug_enabled == 1){
        if(len == sizeof(amessage)){
            ...
            if(msg != NULL){
                switch(msg->command){
                    ...
                    case A_WRTE:
                    case A_AUTH:
                        //XLOGW("usb_adb_write msg (0x%x) (0x%x) (0x%x) (0x%x) (0x%x) (0x%x) \n", msg->command, msg->arg0, msg->arg1, 
        ...
        if(len == sizeof(debuginfo)){
            debuginfo *dbg = (debuginfo*) data;
            if(dbg != NULL){
                if( dbg->command == A_DBUG){
                    //XLOGW("usb_adb_write dbg (0x%x) (0x%x) (0x%x) \n", dbg->command, dbg->msg_check, dbg->data_check);
                }
            }
        }
    }
...
static int usb_adb_read(usb_handle *h, void *data, int len)
{
    ...
    if(bitdebug_enabled == 1){
        if(len == sizeof(amessage)){
            ...
            if(msg != NULL){
                switch(msg->command){
                    ...
                    case A_WRTE:
                    case A_AUTH:
                        //XLOGW("usb_adb_read msg (%d) (%d) (%d) (%d) (%d) (%d) \n", msg->command, msg->arg0, msg->arg1, 
                            //msg->data_length, msg->data_check, msg->magic);
                        break;
                }
            }
        ...
        if(len == sizeof(debuginfo)){
            ...
            if(dbg != NULL){
                if( dbg->command == A_DBUG){
                    //XLOGW("usb_adb_read dbg (0x%x) (0x%x) (0x%x) \n", dbg->command, dbg->msg_check, dbg->data_check);
                }
            }
        }
    }
