...
bool BatteryMonitor::update(void) {
    ...
    props.batteryStatus_2nd = BATTERY_STATUS_UNKNOWN;
    ...
    if (!mHealthdConfig->batteryPresentPath_2nd.isEmpty())
        props.batteryPresent_2nd = getBooleanField(mHealthdConfig->batteryPresentPath_2nd);
    else
        props.batteryPresent_2nd = false;

    props.batteryLevel = getIntField(mHealthdConfig->batteryCapacityPath);
    props.batteryLevel_2nd = getIntField(mHealthdConfig->batteryCapacityPath_2nd);
    ...
    if (readFromFile(mHealthdConfig->batteryStatusPath_2nd, buf, SIZE) > 0)
        props.batteryStatus_2nd = getBatteryStatus(buf);
    ...
    if (logthis) {
        ...
        snprintf(dmesgline, sizeof(dmesgline),
                 "battery l=%d l2=%d v=%d t=%s%d.%d h=%d st=%d st2=%d ext=%d",
                 props.batteryLevel, props.batteryLevel_2nd, 
                 props.batteryVoltage,
                 props.batteryTemperature < 0 ? "-" : "",
                 abs(props.batteryTemperature),
                 props.batteryHealth,
                 props.batteryStatus, props.batteryStatus_2nd,
                 props.batteryPresent_2nd);
...
void BatteryMonitor::init(struct healthd_config *hc, bool nosvcmgr) {
    ...
    if (dir == NULL) {
        ...
        while ((entry = readdir(dir))) {
            ...
            case ANDROID_POWER_SUPPLY_TYPE_BATTERY:
                ...
                if (mHealthdConfig->batteryStatusPath_2nd.isEmpty()) {
                    path.clear();
                    path.appendFormat("%s/%s/status_2nd", POWER_SUPPLY_SYSFS_PATH,
                                      name);
                    if (access(path, R_OK) == 0)
                        mHealthdConfig->batteryStatusPath_2nd = path;
                }
                ...
                if (mHealthdConfig->batteryPresentPath_2nd.isEmpty()) {
                    path.clear();
                    path.appendFormat("%s/%s/present_2nd", POWER_SUPPLY_SYSFS_PATH,
                                      name);
                    if (access(path, R_OK) == 0)
                        mHealthdConfig->batteryPresentPath_2nd = path;
                }
                ...
                if (mHealthdConfig->batteryCapacityPath_2nd.isEmpty()) {
                    path.clear();
                    path.appendFormat("%s/%s/capacity_2nd", POWER_SUPPLY_SYSFS_PATH,
                                      name);
                    if (access(path, R_OK) == 0)
                        mHealthdConfig->batteryCapacityPath_2nd = path;
                }
    ...
    if (mHealthdConfig->batteryStatusPath_2nd.isEmpty())
        KLOG_WARNING(LOG_TAG, "BatteryStatusPath_2nd not found\n");
    if (mHealthdConfig->batteryHealthPath.isEmpty())
        KLOG_WARNING(LOG_TAG, "BatteryHealthPath not found\n");
    if (mHealthdConfig->batteryPresentPath.isEmpty())
        KLOG_WARNING(LOG_TAG, "BatteryPresentPath not found\n");
    if (mHealthdConfig->batteryPresentPath_2nd.isEmpty())
        KLOG_WARNING(LOG_TAG, "BatteryPresentPath_2nd not found\n");
    if (mHealthdConfig->batteryCapacityPath.isEmpty())
        KLOG_WARNING(LOG_TAG, "BatteryCapacityPath not found\n");
    if (mHealthdConfig->batteryCapacityPath_2nd.isEmpty())
        KLOG_WARNING(LOG_TAG, "BatteryCapacityPath_2nd not found\n");
    if (mHealthdConfig->batteryVoltagePath.isEmpty())
