...
int NatController::setDefaults() {
    ...
    struct CommandsAndArgs defaultCommands[] = {
        ...
        {{IPTABLES_PATH, "-t", "nat", "-F", LOCAL_NAT_POSTROUTING}, 1},
        	///M:ALPS00711602. don't flush routing table
		/*
...
int NatController::disableNat(const int argc, char **argv) {
    ...
    if (--natCount <= 0) {
    	...
    	if(tableNumber != -1){
			// remove all secondary tables
	   		struct CommandsAndArgs secondaryCommands[] = {
		    ...
		    for (unsigned int cmdNum = 0; cmdNum < ARRAY_SIZE(secondaryCommands); cmdNum++) {
		        if (runCmd(ARRAY_SIZE(secondaryCommands[cmdNum].cmd), secondaryCommands[cmdNum].cmd) &&
		            secondaryCommands[cmdNum].checkRes) {
		                return -1;
		        }
		    }
		}
