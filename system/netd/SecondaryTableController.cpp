...
int SecondaryTableController::modifyRoute(SocketClient *cli, const char *action, char *iface,
    ...
    //don't set seconary table component for ipv6 test

    if(prefix > 32)
    {
      ALOGD(" modifyRoute :skip set seconary table for ipv6 network ");
      cli->sendMsg(ResponseCode::CommandOkay, "Route igored", false);
      return 0;
     }
...
int SecondaryTableController::modifyFromRule(int tableIndex, const char *action,
    ...
    if (strchr(addr, ':') != NULL)
    {
      ALOGD(" modifyFromRule :skip set seconary table for ipv6 network ");
      return 0;
     }
...
int SecondaryTableController::modifyLocalRoute(int tableIndex, const char *action,
    ...
    if (strchr(addr, ':') != NULL)
    {
      ALOGD(" modifyLocalRoute :skip set seconary table for ipv6 network ");
      return 0;
     }
...
int SecondaryTableController::setFwmarkRoute(const char* iface, const char *dest, int prefix,
   ...
   if(add)
   {
    ...
    ret = runCmd(ARRAY_SIZE(route_cmd), route_cmd);  
    ALOGE("ip route add 0.0.0.0/1 dev %s ret:%d,errno:%d", iface,ret,errno);  
    const char *route_cmd1[] = {
        IP_PATH,
        "route",
        "add" ,
        "128.0.0.0/1",
    ...
    ret = runCmd(ARRAY_SIZE(route_cmd1), route_cmd1);  
    ALOGE("ip route add 128.0.0.0/1 dev %s ret:%d,errno:%d", iface,ret,errno);                  
    }
