        ...
        buffer_handle_t handle)
{
    ...
    if (err == 0) {
        ...
        if (hw_get_module(GRALLOC_HARDWARE_MODULE_ID, (const hw_module_t **)&pmodule) == 0) {
            ...
            ALOGE("Could not get gralloc module for handle: 0x%x", (unsigned int)hnd);
        }
        ...
        if (ion_import(m->ion_client, hnd->fd, &hnd->ion_hnd)) {
            ALOGE( "Failed to ion_import( ion_client: %d share_fd: %d )", m->ion_client, hnd->fd);
            return -1;
        }
    }
#endif
        ...
        buffer_handle_t handle)
{
...
#ifdef MTK_ION_SUPPORT
    {
        ...
        if (ion_free(m->ion_client, hnd->ion_hnd)) {
